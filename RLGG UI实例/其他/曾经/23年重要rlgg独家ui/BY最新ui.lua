...
gggscro=luajava.loadlayout
							...
							{
								TextView,
								text = string.fromHtml(getTimeStamp(os.time()).." <font color=#97B9FF>".."BY 代理[小也][江东][苦瓜][李F][蕉灼][哀澈]".."</font>"),
								textSize = "12sp",
								textColor = color,
								gravity="left",
								layout_width = "wrap_content",
			...
			},
			{
				...
				leftbar,
				{
					...
					orientation = "vertical",
					{
						...
						layout_width="40dp",
						padding="10dp",
						background=getVerticalBG({0xffe5e5e5 ,0xffe5e5e5},30),
						onTouch=hanshu,
						onClick=searching,
						}
					},
		...
		for j=1,4 do
			for k=1,#searchlist[i][j] do
				if string.find(searchlist[i][j][k][1],tobesearch)~=nil then
					result[#result+1]={searchlist[i][j][k][1],i,j,k}
				end
			end
		end
	end
...
_ENV[nid]=luajava.loadlayout({
            ...
            background = "https://wp.td88.cyou/view.php/7bf44d7e23cb593f6a8ae64fd2850f2a.png",
            onClick = function() luajava.newThread(function() func() end):start() end,
        	},{
            LinearLayout,
            visibility="gone",
            ...
            background = "https://wp.td88.cyou/view.php/7c25f5efd626b40d8be94fbf779ca00b.png",
            onClick = function() luajava.newThread(function() func() end):start() end,
        	}
		})
...
function gg.edits(addr, Table, name)
    ...
    gg.addListItems(Table1[2])
    gg.setValues(Table1[1])
    gg.toast((name or "") .. "开启成功, 共修改"..#Table.."个值by")
end
...
---绘制需要全部写在这里否则会消失
draw.setSize(40)
draw.text('BY全防', 127,220)draw.setColor('#ff0000')
draw.text('TG:@BYNB688', 127,270)draw.setColor('#ff0000')
end

sname="BY全防"

stitle="欢迎使用by全防 祝你打穿检测"

--悬浮窗图标链接
BY悬浮窗 = "https://vip.kyun.top/down.php/d0816a66e2c99ba985de1c0d0e6bfeeb.png"

stab = {
---侧边标题和图标
	{
		"防封","https://vip.kyun.top/down.php/d0816a66e2c99ba985de1c0d0e6bfeeb.png"
	},
	{
		"功能","https://vip.kyun.top/down.php/d0816a66e2c99ba985de1c0d0e6bfeeb.png"
	},
	{
		"美化","https://vip.kyun.top/down.php/d0816a66e2c99ba985de1c0d0e6bfeeb.png"
	},
	{
		"音乐","https://vip.kyun.top/down.php/d0816a66e2c99ba985de1c0d0e6bfeeb.png"
	},
	{
		"启动","https://vip.kyun.top/down.php/d0816a66e2c99ba985de1c0d0e6bfeeb.png"
	},
	{
		"设置","https://vip.kyun.top/down.php/d0816a66e2c99ba985de1c0d0e6bfeeb.png"
	},
...
swits["第1页"]["第1板块"] = {
	...
	changan.switch(
		"腾讯①",
		function()
so=gg.getRangesList("libanogs.so")[1].start----过os注入检测
py=0x3c8ca0
setvalue(so+py,4,0)
gg.toast("开启成功")
string.toMusic('开启成功')
...
local tt = {0x1E08, 0x4B0, 0x360, 0x2F0, 0x32C}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x14018, 0x568, 0x1B0, 0xA8}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x6678, 0x530, 0x1A0, 0x130, 0x278}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x2AA0, 0x238, 0xA0, 0x30, 0x594}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x1E00, 0x568, 0x5A0, 0x480, 0x114}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x1E00, 0x568, 0x5A0, 0x4E0, 0x338}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x7A8, 0x4B0, 0x550, 0x440, 0xA8}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x1E00, 0x508, 0x400, 0x240, 0x594}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x1E00, 0x568, 0x5A0, 0x4E0, 0x158}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x7A8, 0x4B0, 0x550, 0x440, 0x38}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x1E00, 0x508, 0x440, 0x30, 0x38}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x2F98, 0x3A0, 0x240, 0x180, 0x204}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x1E00, 0x568, 0xF0, 0x5A0, 0x458}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x3A8, 0x410, 0x5B0, 0x110, 0x5C}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x3A8, 0x410, 0x5B0, 0x80, 0x68}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x1E00, 0x508, 0x1C0, 0x10, 0x68}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x1E00, 0x548, 0x498, 0xA0, 0x594}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x1E00, 0x508, 0x1C0, 0x10, 0x11C}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x1E00, 0x548, 0x498, 0xE0, 0x278}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x1E00, 0x508, 0x80, 0xC0, 0x8C}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x1E00, 0x548, 0x498, 0xE0, 0x378}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x1E00, 0x508, 0x80, 0x90, 0xEC}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x2818, 0x328, 0xA0, 0x240, 0x354}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x2F98, 0x330, 0x80, 0x330, 0x59C}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x3F0, 0x350, 0x310, 0x2B0, 0x114}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x1E00, 0x568, 0x538, 0x10, 0x594}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x1E00, 0x568, 0x460, 0x210, 0x444}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x1E00, 0x568, 0x460, 0x2A0, 0x3B4}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x6678, 0x590, 0x360, 0x360, 0x4E8}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x798, 0x100, 0x2A8, 0x260, 0x564}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x6678, 0x530, 0x1E0, 0x1C0, 0x38}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x14018, 0x508, 0x360, 0x250, 0xF8}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x1E00, 0x548, 0x230, 0x5A0, 0x5A8}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x14018, 0x568, 0x250, 0x8}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x1E00, 0x548, 0x230, 0x390, 0x5C4}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x14018, 0x568, 0x110, 0x68}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x1E00, 0x548, 0x230, 0x3F0, 0x594}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})
...
string.toMusic('开启成功')
	...
	changan.switch(
		...
		function()
local t = {"libanogs.so:bss", "Cb"}
local tt = {0x56E8, 0xE0, 0x40, 0x48}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = -698416192}})
...
string.toMusic('开启成功')---语音
		...
		"大厅③",
		function()
setvalue(gg.getRangesList('libanogs.so')[1].start+0x49f10,4,-698416192)

setvalue(gg.getRangesList('libanogs.so')[1].start+0x4a84c,4,-698416192)

setvalue(gg.getRangesList('libanogs.so')[1].start+0x4a86c,4,-698416192)

setvalue(gg.getRangesList('libanogs.so')[1].start+0x55590,4,-698416192)

setvalue(gg.getRangesList('libanogs.so:bss')[1].start+0x28c,4,-698416192)

setvalue(gg.getRangesList('libanogs.so:bss')[1].start+0x3d8,4,-698416192)
					
setvalue(gg.getRangesList('libPandoraVideo.so')[1].start+0x1910,4,-698416192)

setvalue(gg.getRangesList('libPandoraVideo.so')[1].start+0x19d0,4,-698416192)

setvalue(gg.getRangesList('libPandoraVideo.so')[1].start+0x1a50,4,-698416192)

setvalue(gg.getRangesList('libPandoraVideo.so')[1].start+0x1a98,4,-698416192)

setvalue(gg.getRangesList('libanogs.so')[1].start+0x1ff0,4,256)

setvalue(gg.getRangesList('libanogs.so')[1].start+0x20cc,4,256)

setvalue(gg.getRangesList('libanogs.so')[1].start+0x2118,4,256)

setvalue(gg.getRangesList('libanogs.so')[1].start+0x2128,4,256)

setvalue(gg.getRangesList('libanogs.so')[1].start+0x2158,4,256)

setvalue(gg.getRangesList('libanogs.so')[1].start+0x21b0,4,256)

setvalue(gg.getRangesList('libanogs.so')[1].start+0x21d0,4,256)

setvalue(gg.getRangesList('libanogs.so')[1].start+0x21d8,4,256)

setvalue(gg.getRangesList('libanogs.so')[1].start+0x2270,4,256)

setvalue(gg.getRangesList('libanogs.so')[1].start+0x2288,4,256)

setvalue(gg.getRangesList('libanogs.so')[1].start+0x2298,4,256)

setvalue(gg.getRangesList('libanogs.so')[1].start+0x22c8,4,256)

setvalue(gg.getRangesList('libanogs.so')[1].start+0x22cc,4,256)

setvalue(gg.getRangesList('libanogs.so')[1].start+0x2364,4,256)

setvalue(gg.getRangesList('libanogs.so')[1].start+0x23e4,4,256)

setvalue(gg.getRangesList('libanogs.so')[1].start+0x246c,4,256)

setvalue(gg.getRangesList('libanogs.so')[1].start+0x24c8,4,256)

setvalue(gg.getRangesList('libanogs.so')[1].start+0x250c,4,256)

setvalue(gg.getRangesList('libanogs.so')[1].start+0x2568,4,256)

setvalue(gg.getRangesList('libanogs.so')[1].start+0x265c,4,256)

setvalue(gg.getRangesList('libanogs.so')[1].start+0x2660,4,256)

setvalue(gg.getRangesList('libanogs.so')[1].start+0x2738,4,256)

setvalue(gg.getRangesList('libanogs.so')[1].start+0x2758,4,256)

setvalue(gg.getRangesList('libanogs.so')[1].start+0x2788,4,256)

setvalue(gg.getRangesList('libanogs.so')[1].start+0x27c8,4,256)

setvalue(gg.getRangesList('libanogs.so')[1].start+0x2844,4,256)

setvalue(gg.getRangesList('libanogs.so')[1].start+0x2890,4,256)

setvalue(gg.getRangesList('libanogs.so')[1].start+0x28fc,4,256)
gg.toast("开启成功")
string.toMusic('开启成功')---语音
end),
changan.switch(
...
local tt = {0xAF0C}
local ttt = S_Pointer(t, tt, true)
gg.addListItems({{address = ttt, flags = 4, value = -698416192, freeze = true}})
...
Modify(Read('libanogs.so','rw-p')+0x4ee60,-698416192,4)

Modify(Read('libanogs.so','rw-p')+0x50178,-698416192,4)

Modify(Read('libanogs.so','rw-p')+0x514c0,-698416192,4)

Modify(Read('libanogs.so','rw-p')+0x522a0,-698416192,4)

Modify(Read('libanogs.so','rw-p')+0x55ba0,-698416192,4)

Modify(Read('libanogs.so','rw-p')+0x56e30,-698416192,4)

Modify(Read('libanogs.so','rw-p')+0x57380,-698416192,4)

Modify(Read('libanogs.so','rw-p')+0x573e8,-698416192,4)

Modify(Read('libanogs.so','rw-p')+0x57748,-698416192,4)

Modify(Read('libanogs.so','rw-p')+0x578c0,-698416192,4)
...
Modify(Read('libgcloud.so','r-xp')+0x422588,-698416192,4)
Modify(Read('libgcloud.so','r-xp')+0x4955e8,-698416192,4)
Modify(Read('libanogs.so','r-xp')+0x3d2fb8,-698416192,4)
gg.setValues(Table)
	 gg.toast("全局离线开启成功")
	 gg.clearResults()
...
Modify(Read('libgcloud.so','r-xp')+0x422588,1033081774,4)
Modify(Read('libgcloud.so','r-xp')+0x4955e8,1033081774,4)
Modify(Read('libanogs.so','r-xp')+0x3d2fb8,1033081774,4)
gg.setValues(Table)
	 gg.toast("全局离线关闭成功")
	 gg.clearResults()
string.toMusic('全局离线关闭成功')---语音	
		end),
	changan.switch(
	"离线②",
		function()		
...
string.toMusic('全局离线关闭成功')---语音
		...
		"离线③",
		function()
so=gg.getRangesList('libgcloud.so')[1].start---离线开
py=0x423230
...
function()
so=gg.getRangesList('libgcloud.so')[1].start---离线关
py=0x423230
setvalue(so+py,4,690584402)
	 gg.toast("全局离线关闭成功")
	 gg.clearResults()
string.toMusic('全局离线关闭成功')---语音
		end),
...
Modify(Read('libanogs.so','rw-p')+0x57c48,64,4)
Modify(Read('libanogs.so','rw-p')+0x57f08,64,4)
gg.setValues(Table)
gg.toast("开启成功")
string.toMusic('开启成功')	
		end),
...
local tt = {0x13B0}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 17664}})
gg.toast("开启成功")
string.toMusic('开启成功')	
...
local tt = {0x159170, 0x0}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 8340150}})
gg.toast("开启成功")
string.toMusic('开启成功')	
...
function()
local t = {"libanogs.so:bss", "Cb"}
local tt = {0x1EA0,0x208,0x160,0x2B0,0x418,0x514}
local ttt = S_Pointer(t, tt, true)
gg.addListItems({{address = ttt, flags = 16, value = 2.80259693e-45, freeze = true}})
gg.toast("开启成功") 
end),
changan.switch("自改帧率",
function()
...
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 16, value = 9999}})--关
...
local tt = {0x34C688, 0x4}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 16, value = 0.10000000149}})
gg.toast("关闭成功") 
end),
changan.switch("马克菠萝",
function()
so=gg.getRangesList('libUE4.so:bss')[1].start--so模块
while(true)do--循环
local addr = yf(yf(yf(so+0x39488)+0x0)+0x458)+0x18C--锁链
XGDZ=gg.getValues({{address=addr, flags=16}})[1].value
if XGDZ==360 then XGDZ=-90 end
gg.setValues({{address = addr, flags = 16, value = XGDZ+23}}) 
gg.alert("开启成功")
string.toMusic('开启成功')
end
		end,
...
function()
local t = {"libUE4.so:bss", "Cb"}
local tt = {0x469C50,0x410,0x2F0,0x1E0,0x2CC}
local ttt = S_Pointer(t, tt, true)
...
--动态基址配置
ddsl = readPointer('libUE4.so:bss', {0x3AD38,0x0,0x460,0x1B4}, 1)--已更新
ddsl1 = readPointer('libUE4.so:bss', {0x3AD38,0x0,0x460,0x1B8}, 1)--已更新
--定点锁链
lhsl = readPointer('libUE4.so:bss', {0x2A4498,0x40,0x28,0x38}, 1)--已更新
--灵魂锁链
ttsl = readPointer('libUE4.so:bss', {0x3AD38,0x0,0x458,0xA10,0x4B0,0x570}, 1)--不知道对不对
--探头锁链
local jznbsl = readPointer('libUE4.so:bss', {0x476588,0x20,0x270}, 1)--矩阵头-已更新
--矩阵南北
local jzdxsl = readPointer('libUE4.so:bss', {0x476588,0x20,0x27C}, 1)--+C--已更新
--矩阵东西
local jzsxsl = readPointer('libUE4.so:bss', {0x476588,0x20,0x29C}, 1)--+2C--已更新
--矩阵上下
zbxsl = readPointer('libUE4.so:bss', {0x3AD38,0x0,0x1B0,0x1C0}, 1)---已更新
--坐标x实体
zbysl = readPointer('libUE4.so:bss', {0x3AD38,0x0,0x1B0,0x1C4}, 1)---已更新
--坐标y实体
zbzsl = readPointer('libUE4.so:bss', {0x3AD38,0x0,0x1B0,0x1C8}, 1)--已更新
--坐标z实体
...
if tt < 1000 then--不探头
gg.edits(lhsl, {{10000, 4, 0, false}})
gg.edits(ddsl, {{1, 16, 0, false}})
gg.edits(ddsl1, {{443, 16, 0, false}})
...
function()
local t = {"libUE4.so:bss", "Cb"}
local tt = {0x3AD38, 0x0, 0x198, 0x1018}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 16, value = 2}})
local t = {"libUE4.so:bss", "Cb"}
local tt = {0x3AD38, 0x0, 0x198, 0x101C}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 16, value = 640}})
gg.toast("开启成功")
end,
function()
local t = {"libUE4.so:bss", "Cb"}
...
changan.switch("自改画质",
function()
kk = gg.prompt({"1流畅 2均衡 3高清 4HDR 5超高清  6未开放 7超流畅"},{3},{"number"})
local addr = readPointer("libUE4.so:bss", {0x458870, 0xF8, 0x520, 0x420, 0x6BC},1)
gg.edits(addr, {{kk[1], 4, 0, false}})
gg.toast("开启成功")
end),
changan.switch("卡金砖",
function()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("3010011", gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1, 0)
gg.getResults(1000, nil, nil, nil, nil, nil, nil, nil, nil)
gg.editAll("3001067", gg.TYPE_DWORD)
gg.toast("BY ☑️")
gg.toast("开启成功")
string.toMusic('开启成功')
gg.clearResults()
...
zhiqiu=gg.prompt({
"上半身初始值1400129\n血鸦1405870  海洋之王1405983  血渊圣王1406872  黄木乃伊1405623  \n白木乃伊1400687  蓝木乃伊1406891  暗星陨落1406878   \n晨星光耀1406879  齐天大圣1406327   明日香1406387  秘渊领主1406970\n请输入你的衣服代码：",
},cfg,{"text","text"})
if zhiqiu == nil then else
gg.saveVariable(zhiqiu, cfg_file)
local t = {"libUE4.so:bss", "Cb"}
local tt = {0x3AD38, 0x0, 0x330, 0xCA8, 0x388, 0x118}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = zhiqiu[1]}})
...
local t = {"libUE4.so:bss", "Cb"}
local tt = {0x3AD38, 0x0, 0x390, 0xA0, 0x388, 0x118}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = zhiqiu[1]}})
gg.toast("自改人物开启成功")
end
...
swits["第3页"]["第2板块"] = {
changan.text2("手持枪械美化"),
changan.switch("手持枪械美化",
function ()
local cfg_file = gg.EXT_FILES_DIR.."/"..gg.getFile():match("[^/]+$")..'(全部枪械.).lua'
local chunk = loadfile(cfg_file)
local cfg = nil
if chunk ~= nil then
 cfg = chunk()
...
if cfg == nil then
cfg = {1101004209, 1010042082, 1010042093, 1010042071, 1010042039, 1010042046,  true}
end
h= gg.prompt({
"枪械=修改",
"机瞄＝修改",
...
if h[4] == true then h4() end
if h[5] == true then h5() end
if h[6] == true then h6() end
...
local t = {"libUE4.so:bss", "Cb"}
local tt = {0x3AD38,0x0,0xDD0,0x60,0x9B8,0xA4}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = h[2]}})
local t = {"libUE4.so:bss", "Cb"}
local tt = {0x3AD38,0x0,0xDD0,0x60,0x9B8,0x7C}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = h[3]}})
local t = {"libUE4.so:bss", "Cb"}
local tt = {0x3AD38,0x0,0xDD0,0x60,0x9B8,0x54}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = h[4]}})
local t = {"libUE4.so:bss", "Cb"}
local tt = {0x3AD38,0x0,0xDD0,0x60,0x9B8,0x04}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = h[5]}})
local t = {"libUE4.so:bss", "Cb"}
local tt = {0x3AD38,0x0,0xDD0,0x60,0x9B8,0x2C}
local ttt = S_Pointer(t, tt, true)
...
changan.switch("动作美化",
function()
DN = gg.prompt({"电摇2201201 奈子会动12219624 宗师12219532 木乃伊12215503 艳后12219412 剩下的进群拿全部代码自己找，懒得扣了 "},nil,{"number"})
	if DN ~= nil then


local t = {"libUE4.so:bss", "Cb"}--4/8500
local tt = {0x484030,0x30,0x2F0,0x1A0,0x358,0x234}
local ttt = S_Pointer(t, tt, true)
gg.addListItems({{address = ttt, flags = 4, value = DN[1], freeze = true}})
...
changan.switch("击杀播报",
function()
DN = gg.prompt({"AKM华丽浮雕击杀播报 1101001042 AKM怒海争锋击杀播报1101001063 AKM白虎咆哮击杀播报 1101001068 AKM冰霜核心击杀播报 1101001089 AKM积木龙骨击杀播报1101001103 AKM奇异之夜击杀播报 1101001116 AKM金牌海盗击杀播报 1101001143 AKM吉利龙王击杀播报 1101001128 AKM奇幻工坊击杀播报 1101001154 AKM部落之王击杀播报1101001174 AKM星海提督击杀播报 1101001213 AKM俏皮兔宝击杀播报 1101001231M16A4血风暴击杀播报1101002029 M16A4极光脉冲击杀播报 1101002056 M16A4缤纷利刃击杀播报 1101002068 M16A4猩红血月击杀播报 1101002081M16A4古堡龙卫击杀播报1101002103死噶梦幻水枪击杀播报 1101003057死噶魔法南瓜击杀播报1101003070死噶翌日行动击杀播报 1101003080 死噶诡秘之夜击杀播报 1101003099 死噶魔力结晶击杀播报 1101003119死噶邪能植物击杀播报 1101003146 死噶血魂魔皇击杀播报 1101003167M416冰霜核心击杀播报1101004046 M416愚人小丑击杀播报 1101004062M416异域游者击杀播报 1101004078 M416萌龙咆哮击杀播报1101004086M416野性呼唤击杀播报 1101004098 M416科技核心击杀播报 1101004138 M416潮鸣宫廷击杀播报 1101004163 M416庇护之潮击杀播报1101004209Groza幻夜音乐击杀播报1101005025 Groza绚烂之战1101005043 Groza冥河烈焰1101005052AUG流浪马戏团 1101006033 AUG第四使徒 1101006044 QBZ极致绝杀 1101007036 QBZ瑰绮灵姬1101007046 M762比特宝莉 1101008026"},nil,{"n1101001042"})
	if DN ~= nil then


local t = {"libUE4.so:bss", "Cb"}--4/8500
local tt = {0x39488,0x0,0x1A0,0x8,0x9C8,0x11C}
local ttt = S_Pointer(t, tt, true)
gg.addListItems({{address = ttt, flags = 4, value = DN[1], freeze = true}})

local t = {"libUE4.so:bss", "Cb"}--4/8500
local tt = {0xC36A488,0x0,0x430,0x15A0}
local ttt = S_Pointer(t, tt, true)
gg.addListItems({{address = ttt, flags = 4, value = DN[1], freeze = true}})
end
end),
