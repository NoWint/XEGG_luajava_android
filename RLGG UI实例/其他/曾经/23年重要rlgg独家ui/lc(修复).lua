...
font-size: 15px;
letter-spacing: 5px;
background: linear-gradient(to right,#5D6269,#ffffff,#71767B);
...
if panduan("/sdcard/XG/图片/"..tmp) ~= true then
gg.toast("正在下载资源"..ii.."/"..#ckimg.."\n请耐心等待")
luajava.download("https://escape2020-1303126286.cos.ap-shenzhen-fsi.myqcloud.com/"..tmp,"/sdcard/XG/图片/"..tmp)
else
if file.length("/sdcard/XG/图片/"..tmp) <= 1 then
gg.toast("正在下载资源"..ii.."/"..#ckimg.."\n请耐心等待")
luajava.download("https://escape2020-1303126286.cos.ap-shenzhen-fsi.myqcloud.com/"..tmp,"/sdcard/XG/图片/"..tmp)
end
end
if panduan("/sdcard/XG/图片/"..tmp) ~= true then
gg.toast("正在下载资源"..ii.."/"..#ckimg.."\n请耐心等待")
luajava.download("https://rl-1303126286.cos.ap-beijing-fsi.myqcloud.com/"..tmp,"/sdcard/XG/图片/"..tmp)
else
	if file.length("/sdcard/XG/图片/"..tmp) <= 1 then
gg.toast("正在下载资源"..ii.."/"..#ckimg.."\n请耐心等待")
luajava.download("https://rl-1303126286.cos.ap-beijing-fsi.myqcloud.com/"..tmp,"/sdcard/XG/图片/"..tmp)
...
摇一摇=true
if panduan("/sdcard/XG/图片/opo1")~=true then
	file.unzip("/sdcard/XG/图片/opo","/sdcard/XG/图片/")
end
function 获取图片(txt)
txt = string.url(txt,"de")
ntxt = string.sub(string.gsub(txt,"/","-"),-10,-1)
if string.find(tostring(txt),"http") ~= nil then
if panduan("/sdcard/XG/图片/"..ntxt) == false then
file.download(txt,"/sdcard/XG/图片/"..ntxt)
else
	if file.length("/sdcard/XG/图片/"..ntxt) <= 1 then
file.download(txt,"/sdcard/XG/图片/"..ntxt)
end
end
txt = "/sdcard/XG/图片/"..ntxt
end
return luajava.getBitmapDrawable(txt)
end
function getRes(x)
return 获取图片("/sdcard/XG/图片/"..x)
end
...
dexloader=dex.loadfile('/sdcard/XG/图片/classes3.dex')
...
-- 设置录音源、输出格式和编码器
			...
			textSize="13sp",
				...
				layout_height="20dp",
				layout_width="40dp",
				onClick=function() luajava.newThread(function() 发送聊天(XG.getedit2("聊天框")) end):start() end
				},
		}}
	for i=1,#stab do
...
lxytp1=获取图片('/sdcard/XG/图片/star1')
lxytp2=获取图片('/sdcard/XG/图片/star2')
	ckou={
		LinearLayout,
...
draw.remove()
luajava.runUiThread(function()
	XG.controlSmall(floatWindow,400)
	end)
gg.sleep(400)
luajava.runUiThread(function()
	floatWindow:setVisibility(View.GONE)
...
if panduan("/sdcard/XG/配置文件/MGkami.lua") ~= false then
dofile("/sdcard/XG/配置文件/MGkami.lua")
隐藏状态 = 8
隐藏长度 = "0dp"
隐藏长度1 = '20dp'
隐藏状态1 = 0
...
luajava.post(function()
	...
	layout_marginRight='4dp',
				...
				layout_height = "30dp",
				layout_width = "160dp",
				onClick = function() luajava.newThread(function() login(XG.getedit("卡密")) end):start() end,
			},{
				TextView,
				background = luajava.loadlayout {
					GradientDrawable,
...
ktmp = "kam='"..ret.kami.."'"
luajava.newThread(function()
	file.write("/sdcard/XG/配置文件/MGkami.lua",ktmp)
end):start()
end
...
switchs={}
tcheck=10
function XG.intcheck(name,func1,func2,ii,gid,pic)
if pic~=nil then
	tocheck=获取图片(pic)
else
if tcheck==56 then
...
function XG.intcard(name,name2,func1,func2,ii,gid)
if tcheck==56 then
	tcheck=1
else
	tcheck=tcheck+1
...
luajava.runUiThread(function()
	...
	--XG.controlWater(switchs[nid],300)
	end)
_ENV[name] = "开"
...
if not name then name = "未设置" end
local gid=guid()
rstt = XG.intcard(name,name2,func1,func2,1,gid)
...
if not name then name = "未设置" end
rstt = XG.intcheck(name,func1,func2,i,gid,pic)
if i<9 then
rest[#rest+1] = rstt
end
end
if #cklist>=9 then
rstt = XG.checkbox(gid,#cklist+1)
rest[#rest+1] = rstt
end
...
XG.controlRotation9 = function(control, time,t)
luajava.runUiThread(function()
...
XG.controlRotation9(boxpic[tid],0,90)
boxpic[tid]:setColorFilter(控件颜色)
colorvs[tid]={true,"box"}
else
	tview : setVisibility (View.GONE)
XG.controlWater (_ENV [tid.."6"] , 200)
XG.controlRotation9(boxpic[tid],90,0)
...
firadio [# firadio + 1] = {
	...
	gg.alert ("XG.box的table内第一个元素必须是string") os.exit ()
end
radios = {
	LinearLayout ,
...
buts={}
heir=getRes("heir")
function XG.button(txt,func,txtc)
...
function XG.image(img,height,width,pad,func)
...
function XG.text(txt,color,size,t)
...
addchat(XG.chat(localres[#localres]))
end
end
...
-- 设置录音源、输出格式和编码器
					...
					local savePath = "/sdcard/XG/test.mp3"
					recorder:setOutputFile(savePath)
		            recorder:prepare()
					recorder:start()
					redstart=os.time()
		...
		},
		{
			...
			gravity="left",
			{
				...
				onClick=function()
					...
					luajava.newThread(
					function()
						if panduan("/sdcard/XG/音频/"..playPath)~=true then
						file.download(tostring("http://47.101.220.155/"..channel.."/".."audio/"..playPath),"/sdcard/XG/音频/"..playPath)
						end
						gg.playMusic("/sdcard/XG/音频/"..playPath)
						end):start()
					end,
				padding = "3dp",
				layout_width = "wrap_content",
...
悬浮窗图标="http://twjht.kkkkoo.cn/view.php/34ea66084c18475d8f36bf269372dc1a.png"
右上角标题='XGUI'
stab={
	"主页",
	"防封",
...
XG.menu({
	...
	{---第二页
		...
		XG.button(
			"调用输入框",
			function()
				local tmp=XG.getedit("输入框例子")--名字写输入框名字
				gg.alert(tmp)
			end),
		XG.seek("拉条例子","ltlz",1,100,1),--名字，变量名字，最小，最大，初始
		XG.button(
			"调用拉条",
			function()
...
XG.text('点击进入官方群聊',nil,'11sp',true),
XG.text("\n"),
XG.switch('流星开关',
function()
		luajava.post(function()
