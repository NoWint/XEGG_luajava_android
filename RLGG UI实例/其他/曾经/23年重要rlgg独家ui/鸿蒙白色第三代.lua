...
getSelector:addState({
	android.R.attr.state_pressed
}, getVerticalBG({0xFFffffff,0xFFffffff},15,0,0xaa232323)) -- 点击时候的背景
getSelector:addState({
	-android.R.attr.state_pressed
}, getVerticalBG({0xFFffffff,0xFFffffff},15,0,0xaa232323)) -- 没点击的背景
...
if i< #stab then
	gund[#gund+1] = luajava.loadlayout({
		...
		background=getVerticalBG({0xFFffffff,0xFFffffff},15,0,0xaa232323)
	})
end
end
cebian[#cebian+1] = {
...
luajava.runUiThread(function()
	for i = 1,#stab do
	_ENV["jm"..i.."t"]:setTextColor(0xff201E1E)--未选中的字体颜色
	_ENV["layout"..i]:setVisibility(View.GONE)
	_ENV["jm"..i]:setBackground(_ENV["jmb"..i])--未选中的颜色
	end
	_ENV["layout"..当前ui]:setVisibility(View.VISIBLE)
	_ENV["jm"..当前ui.."t"]:setTextColor(0xff201E1E)--选中的字体颜色
	_ENV["jm"..当前ui]:setBackground(getVerticalBG({0xff00F4FF,0xff006DFF},15,0,0xaa242937))--选中后颜色
...
-----------------------------------------------------
		{----1
-----------------------------------------------------
			changan.text("HarmonyOS","#008EFF","20sp"),  
			changan.switch("功能",
			function()
			退出UI()
			end,
