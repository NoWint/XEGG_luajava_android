...
if panduan("/sdcard/FUNC20/图片/"..tmp) ~= true then
gg.toast("正在加载资源"..tmp)
download("https://escape2020-1303126286.cos.ap-shenzhen-fsi.myqcloud.com/"..tmp,"/sdcard/FUNC20/图片/"..tmp)
end
end
function 获取图片(txt)
ntxt=string.sub(string.gsub(txt,"/","."),-10,-1)
if string.find(tostring(txt),"http")~=nil then
    if panduan("/sdcard/FUNC20/图片/"..ntxt)==false then
    download(txt,"/sdcard/FUNC20/图片/"..ntxt)
    end
    txt="/sdcard/FUNC20/图片/"..ntxt
end
...
ttview:setBackground(luajava.getBitmapDrawable("/sdcard/FUNC20/图片/hsj"))
else
tview:setVisibility(View.GONE)
ttview:setBackground(luajava.getBitmapDrawable("/sdcard/FUNC20/图片/sj"))
end
end
...
firadio[#firadio+1] = {
	...
	background = getSelector3(),
	{
		...
		background = "/sdcard/FUNC20/图片/hsj",
		layout_width = "20dp",
		layout_height = "20dp",
...
YH.menu(
	{
		{----1
			--YH.text("欢迎使用FUNC20UI","#008EFF","20sp"),
...
function getRes(x)
return 获取图片("/sdcard/FUNC20/图片/"..x)
end
...
if string.find(tostring(txt),"http") ~= nil then
if panduan("/sdcard/FUNC20/图片/"..ntxt) == false then
luajava.download(txt,"/sdcard/FUNC20/图片/"..ntxt)
end
txt = "/sdcard/FUNC20/图片/"..ntxt
...
local mian_layout = {
	...
	layout_height = "match_parent",
	orientation = "vertical",
	background = luajava.getBitmapDrawable("/sdcard/FUNC20/图片/cow"),
	{
		LinearLayout,
