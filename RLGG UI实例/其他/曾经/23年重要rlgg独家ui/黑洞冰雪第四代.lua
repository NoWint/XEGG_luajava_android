...
local function simpleTagCloudView(data)
	...
	for i=1,#stab do
		local textView = luajava.loadlayout({
			...
			padding="3dp",
			gravity = "center",
			background=getVerticalBG({0x78000000,0x78000000},25,2,0x44000000),
...
local tagCloudView = simpleTagCloudView(stab)
			...
			{LinearLayout,
				...
				{LinearLayout,
					...
					{LinearLayout,
						...
						{
								TextView,
								text = string.fromHtml("<font color=#ffffff>".."欢迎使用云烛落日横屏ui 群965284990","</font>"),
								textSize = "12sp",
								textColor = "#545454",
								gravity="left",
								layout_width = "match_parent",
...
function 隐藏()
luajava.runUiThread(function()
	if tonumber(tostring(titlebar:getVisibility())) == 8.0 then
	control:setVisibility(View.GONE)
	if not yunzhuk then yunzhug=600 yunzhuk=600 end----默认长宽
    	mainLayoutParams.height=yunzhug
    	mainLayoutParams.width=yunzhuk
    	window:updateViewLayout(floatWindow, mainLayoutParams)
	...
	Yunzhu.controlBig(floatWindow,500)
		...
		yunzhug=mainLayoutParams.height
		yunzhuk=mainLayoutParams.width
...
--id = luajava.newId(tid),
				...
				src="/storage/emulated/0/云烛/图片/heir.png",
				layout_height="14dp",
				layout_width="14dp",
				gravity = "center_vertical" ,
...
pcall(func1)
	...
	vibra:vibrate(9)
luajava.runUiThread(function()
	luajava.getIdValue(nid.."t"):setTextColor(控件颜色[2])
	luajava.getIdValue(nid.."p"):setColorFilter(控件颜色[2])
	luajava.getIdValue(nid.."t2"):setTextColor(控件颜色[2])
