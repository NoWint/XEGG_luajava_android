...
ckou = {
	...
	orientation = "vertical",
	{
		...
		gravity="center_vertical",
		{
			...
			onClick = function() 
			fh=当前ui
			切换(1) cebian:setVisibility(View.GONE) vibra:vibrate(6) end,
			onTouch = hanshu,
		},
		cebian,
		{ImageView,
...
floatWindow = {
	...
	motion,
	{
		...
		layout_height = "40dp",
		onTouch = hanshu,
		onClick = function() 隐藏() vibra:vibrate(6)
if 当前ui == 1 then cebian:setVisibility(View.GONE) end end,
	}
}
...
beij:setColors(({0xcc000000,0xff00e8ff,0xff006dff,0xcc000000}))--背景
...
YBY.controlWater = function(control,time)
luajava.runUiThread(function()
...
gg.toast("已隐藏")
luajava.runUiThread(function()
	YBY.controlSmall(floatWindow,400)
	end)
gg.sleep(400)
luajava.runUiThread(function()
...
luajava.runUiThread(function()
	YBY.controlBig(floatWindow,400)
	end)

end
end
end
YBY.controlSmall = function(control,time)
luajava.runUiThread(function()
	import "android.animation.ObjectAnimator"
	ObjectAnimator():ofFloat(control,"scaleX", {
		1, 0.7, 0.4, 0
...
YBY.controlBig = function(control,time)
luajava.runUiThread(function()
	import "android.animation.ObjectAnimator"
	ObjectAnimator():ofFloat(control,"scaleX", {
		0, 0.4, 0.7, 1
...
YBY.controlFlip2 = function(control,time)
	luajava.runUiThread(function()
...
YBY.controlRotation2 = function(control, time)
	luajava.runUiThread(function()
		import "android.view.animation.Animation"
		import "android.animation.ObjectAnimator"
...
chazhi={} chajv={}
function YBY.seek(name,bian,smin,smax,nows)
_ENV[bian] =nows
thum=lt3()
thum:setSize(80, 80)
...
checkbg2 = getShape2(
	...
	4,0xff00)
function YBY.intcheck(name,func1,func2,qdtp)
	nid = name..guid()
...
function YBY.image(tpgd,tpkd,tup,func)
return luajava.loadlayout({
LinearLayout,
layout_height = tpgd,
layout_width = tpkd,
background = tup,
onClick = function() vibra:vibrate(6) cebian:setVisibility(View.VISIBLE) luajava.newThread(function() pcall(func) end):start() end,
})
end
function YBY.line()
return luajava.loadlayout({
...
function YBY.button(name,func1,func2)
...
function YBY.check(cklist)
	...
	for i = 1,#cklist do
		...
		if not name then name = "未设置" end
		rstt = YBY.intcheck(name,func1,func2,qdtp)
		rest[#rest+1] = rstt
	end
	return luajava.loadlayout({
		LinearLayout,rest
...
function YBY.switch(cklist)
		...
		for i = 1,#cklist do
			...
			local func2 = cklist[i][3]
			if not name then name = "未设置" end
			rstt = YBY.switch(name,func1,func2)
				rest[#rest+1] = rstt
			end
			return luajava.loadlayout({
				LinearLayout,rest
		...
		4,0x00fffffff)
		function YBY.switch(name,func1,func2,miaoshu)
				nid = name..guid()
				local func = 开关3(name,func1,func2,nid)
				if not name then name = "未设置" end
				local rest = luajava.loadlayout({
			...
			textColor = "#A5A5A5",
		},
						{
							...
							background = "https://wp.td88.cyou/view.php/20b23874707b8a8db32becbbe64abac5.png",
							padding = "0dp",
							layout_height = "20dp" ,
							layout_width = "45dp" ,
							{
								...
								background = "https://wp.td88.cyou/view.php/3a0c0f742863c56423863bc2b41f23b3.png",
								layout_height = "18dp" ,
						    	layout_width = "23dp" ,
							}, {
								LinearLayout,
		...
		anniu:setStroke(8,"0xff222222")
			function YBY.button(txt,func)
				...
				return luajava.loadlayout(
				{
					...
					layout_hight = "30dp", {
						...
						layout_marginBottom = "5dp",
						background = anniu,
						onClick = function() vibra:vibrate(6) luajava.newThread(function() pcall(func) end):start() end,
						{
							TextView,
							--id = luajava.newId(tid),
							text = txt,
			...
			function YBY.text(txt,color,size)
		...
		gravity = "left",
			...
			function YBY.text1(txt,color,size)
				if not txt then txt = "未设置文字" end
				if not color then color = "#ffffff" end
				if not size then size = "18sp" end
				return luajava.loadlayout(
		...
		gravity = "center",
			...
			corb = true
			function YBY.setedit(name,txt)
				txt = tostring(txt)
				luajava.runUiThread(function()
					luajava.getIdValue(_ENV[name]):setText(txt)
				end)
			end
			function YBY.getedit(name)
				--gg.alert(edit)
...
firadio[#firadio+1] = {
	...
	gg.alert("YBY.box第一个参数必须是string") os.exit()
end
radios = {
	LinearLayout,
...
颜色2 = "545454"--用于文本
bglist={
	getVerticalBG({0xff2000ff,0xffff0000},25,8,0xff232323),
	getVerticalBG({0xcc000000,0xff00e8ff,0xff006dff,0xcc000000},25,8,0xff232323),
	getVerticalBG({0xcc000000,0xff2000EE,0xcc000000},25,8,0xff232323),
}
stitle = "    "
stab = {
...
YBY.menu(
	{
			{
			YBY.image("130dp","130dp","https://cdn-community.codemao.cn/47/community/d2ViXzEwMDFfMTAwMTQwOTNfMTAwMTQwOTNfMTcwNjUyNTg1MTY4NV80ZmRmNjBjMw.png",          function() 
			if not fh or fh == 1 then fh=2 end
			切换(fh) 
			end),--高度,宽度,图片链接 这里的功能改了会出错
			
			particle("yzNB","YZ全防-特别版",{"#2000ff","#ff0000","#2000ff"}),
			YBY.text1("Protection Server 1","#FFFFFF","13sp"),
----------------------------启动功能↓---------------------------
		    YBY.intcheck( "全球服",
										function()
										end,
...
----------------------------启动功能↑---------------------------		
		...
		{--1
			YBY.text("GAME防封","#FFFFFF","15sp"),
			YBY.text("如果使用了启动游戏的过检测 可以不开防封 也可以自行搭配","#545454","10sp"),
			YBY.text("也以不开过检测直接开防如果开过检测了可以直接奔放或者搭配防封","#545454","10sp"),
					YBY.switch("LOGO防封",
						function()
							end,
							function()
		...
		}, {YBY.seek("拉条演示","ltys",1,500000,6),--名字，变量名字，最小值，最大值，初始值
--2
		}, {
...
--3
		...
		YBY.text("\n\n\n"),		
YBY.image("50dp","50dp","https://wp.td88.cyou/view.php/f08db9c31f0f2cd8cf8e6a17c91f9488.png", function() app.openUrl("https://t.me/XEWLYZ") end),--高度,宽度,图片链接
			YBY.text("\n"),
			YBY.button("EXIT",function()
				exit()
				end),
				YBY.switch("缩放UI",
						function()
