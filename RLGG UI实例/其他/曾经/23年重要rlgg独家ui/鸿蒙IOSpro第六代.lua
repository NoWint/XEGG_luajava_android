...
function 变色()
if android.R.attr.state_pressed then beij:setStroke(8,"0xFF3875F2") end
end
...
getSelector:addState({
	android.R.attr.state_pressed
}, luajava.getBitmapDrawable("https://cdn-community.codemao.cn/47/community/d2ViXzUwMDFfMTAwMTQwOTNfMTQ2NzVfMTcwNjg0NjE0ODY4Nl8xYThhNzUzYw.png")) -- 点击时候的背景
getSelector:addState({
	-android.R.attr.state_pressed
}, luajava.getBitmapDrawable("https://cdn-community.codemao.cn/47/community/d2ViXzUwMDFfMTAwMTQwOTNfMTQ2NzVfMTcwNjg0NjE0ODY4Nl8xYThhNzUzYw.png")) -- 没点击的背景
...
if i< #stab then
	gund[#gund+1] = luajava.loadlayout({
		...
		layout_height="match_parent",
		layout_width="1.3dp",
		background="https://cdn-community.codemao.cn/47/community/d2ViXzUwMDFfMTAwMTQwOTNfMTQ2NzVfMTcwNjg0NjE0ODY4Nl8xYThhNzUzYw.png",
	})
end
end
cebian[#cebian+1] = {
...
FrameLayout,
	...
	{LinearLayout,
		...
		onTouch=hanshu,
			...
			{LinearLayout,
		visibility = "visible",
		background="https://wp.td88.cyou/view.php/446080ee0edb234100f7da1532a28c04.png",
		padding="10dp",
	...
	{LinearLayout,
		visibility = "visible",
		background="https://wp.td88.cyou/view.php/c16bf848898430447f1e0d38d3a0f439.png",
		padding="10dp",
		layout_marginTop="7dp",
...
jiaobiao2.onTouch = suofang
jiaobiao3.onTouch = suofang
local isMove
...
beij:setColors({0xE61A1E22,0xE61A1E22})
--ui背景气死我了我云烛找老久这个
beij:setStroke(0,"0x00FFFFFF")--边框宽度和颜色
--beijw
...
luajava.runUiThread(function()
	if tonumber(tostring(exit:getVisibility())) == 8.0 then
		...
		jiaobiao2:setVisibility(View.VISIBLE)
		jiaobiao3:setVisibility(View.VISIBLE)
		...
		jiaobiao2:setVisibility(View.GONE)
		jiaobiao3:setVisibility(View.GONE)
...
function XG.switch(name,func1,func2,yans)
nid = name..guid()
...
function XG.button(txt,func,yans)
...
--XG.text   文字
--XG.seek  拖动条
--XG.edit  输入框（XG.getedit和XG.setedit调用
...
-----------------------------------------------------
		{----1
-----------------------------------------------------
			--XG.text("欢迎使用长安UIUI","#008EFF","20sp"),  
			XG.box2({--如果不需要请删除box2，注意括号的对应。如果需要请注意用box2套box，不然会很丑
				"套娃",
				XG.box({--box开头
					"",--box名字
					...
					XG.edit("输入演示"),--括号填的“输入演示”就是当前输入框的ID
					
					XG.button("调用输入框",
						function()
							tmp=XG.getedit("输入演示")--传入ID调用内容，返回string类型
							gg.alert(tmp)
						end),
					
					XG.button("设置输入框",
						function()
							XG.setedit("输入演示","修改了输入框内容")--传入ID和要改成的内容
						end),
			...
			XG.box({--box开头
				...
				XG.seek("拉条演示","ltys",1,10,6),--名字，变量名字，最小值，最大值，初始值
