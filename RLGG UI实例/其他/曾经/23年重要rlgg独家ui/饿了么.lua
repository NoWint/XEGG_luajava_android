...
write("/sdcard/饿了吗/配置文件/"..lujing,neirong)
end
function rdcf(lujing)
return read("/sdcard/饿了吗/配置文件/"..lujing)
end

开 = "开" 关 = "关"
function checkimg(tmp)
if panduan("/sdcard/饿了吗/图片/"..tmp) ~= true then
gg.toast("正在下载资源"..tmp.."\n请耐心等待")
download("https://escape2020-1303126286.cos.ap-shenzhen-fsi.myqcloud.com/"..tmp,"/sdcard/饿了吗/图片/"..tmp)
end
end
ckimg = {
	'bbts_checkoff',
...
if panduan("/sdcard/饿了吗/图片/"..ntxt) == false then
file.download(txt,"/sdcard/饿了吗/图片/"..ntxt)
else
	if file.length("/sdcard/饿了吗/图片/"..ntxt) <= 1 then
file.download(txt,"/sdcard/饿了吗/图片/"..ntxt)
end
end
txt = "/sdcard/饿了吗/图片/"..ntxt
end
return luajava.getBitmapDrawable(txt)
end
function getRes(x)
return 获取图片("/sdcard/饿了吗/图片/"..x)
end
...
jmbg={}
dbback=getVerticalBG({0xff01b6fd,0xff01b6fd},90)
for i = 1,菜单页数 do
	分区页数[i]= #sview[i]
	table.insert(顶边,{
		LinearLayout,
...
floatWindow = {
	...
	{FrameLayout,
	id='ggscro',
	layout_marginTop='235dp',
	layout_marginLeft='68dp',
	layout_height='60dp',
	layout_width='300dp',
	background=getCorner({0xffffffff,0xffffffff},12,0,0xff000000,0,0,25,0),
	{LinearLayout,
		layout_height='45dp',
		layout_width='match_parent',
		...
		{	LinearLayout,
			id=luajava.newId('line'),
			layout_height='35dp',
			layout_width='70dp',
			layout_gravity='center_vertical',
			background=getVerticalBG({0xff01b6fd,0xff01b6fd},90),
			gravity='center',
			onClick=结算,
			{TextView,
				text='结算',
...
luajava.runUiThread(function()
	...
	luajava.getIdView('line'):setVisibility(View.VISIBLE)
	显示 = 1
	...
	luajava.getIdView('line'):setVisibility(View.GONE)
...
checkbg4 = getVerticalBG({0x00000000,0x00000000},360,11,0xff01b6fd)
