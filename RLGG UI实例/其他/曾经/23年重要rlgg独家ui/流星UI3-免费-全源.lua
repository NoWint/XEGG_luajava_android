...
if panduan("/sdcard/原神/图片/"..tmp) ~= true then
gg.toast("正在下载资源"..ii.."/"..#ckimg.."\n请耐心等待")
luajava.download("https://escape2020-1303126286.cos.ap-shenzhen-fsi.myqcloud.com/"..tmp,"/sdcard/原神/图片/"..tmp)
else
if file.length("/sdcard/原神/图片/"..tmp) <= 1 then
gg.toast("正在下载资源"..ii.."/"..#ckimg.."\n请耐心等待")
luajava.download("https://escape2020-1303126286.cos.ap-shenzhen-fsi.myqcloud.com/"..tmp,"/sdcard/原神/图片/"..tmp)
end
end
if panduan("/sdcard/原神/图片/"..tmp) ~= true then
gg.toast("正在下载资源"..ii.."/"..#ckimg.."\n请耐心等待")
luajava.download("https://rl-1303126286.cos.ap-beijing-fsi.myqcloud.com/"..tmp,"/sdcard/原神/图片/"..tmp)
else
	if file.length("/sdcard/原神/图片/"..tmp) <= 1 then
gg.toast("正在下载资源"..ii.."/"..#ckimg.."\n请耐心等待")
luajava.download("https://rl-1303126286.cos.ap-beijing-fsi.myqcloud.com/"..tmp,"/sdcard/原神/图片/"..tmp)
...
if panduan("/sdcard/原神/图片/opo1")~=true then
	file.unzip("/sdcard/原神/图片/opo","/sdcard/原神/图片/")
end
function 获取图片(txt)
txt = string.url(txt,"de")
ntxt = string.sub(string.gsub(txt,"/","-"),-10,-1)
if string.find(tostring(txt),"http") ~= nil then
if panduan("/sdcard/原神/图片/"..ntxt) == false then
file.download(txt,"/sdcard/原神/图片/"..ntxt)
else
	if file.length("/sdcard/原神/图片/"..ntxt) <= 1 then
file.download(txt,"/sdcard/原神/图片/"..ntxt)
end
end
txt = "/sdcard/原神/图片/"..ntxt
end
return luajava.getBitmapDrawable(txt)
end
function getRes(x)
return 获取图片("/sdcard/原神/图片/"..x)
end
...
draw3 = require('draw3')
			draw3 = require('draw3')
			text1 = draw3.text('UI分享频道@https://t.me/rlgg_changan',200,200)
			--draw.setStyle('填充')
			text2 = draw3.text('UI分享频道@https://t.me/rlgg_changan',200,200)
			for i = 1, 5 do--这里改太大可能造成悬浮窗不出来

				-- 设置随机颜色
				local color = math.random(000000, 0xffffff)
				text1.setColor(color)
				local colo = math.random(000000, 0xffffff)
				text2.setColor(color)
			end
		end
		draw.setStyle('填充')

悬浮窗图标="https://mq2024.oss-cn-hangzhou.aliyuncs.com/YS_logo.png"
...
loadingBox = getLoadingBox('正在启动原神...')
loadingBox['显示']()
...
local tt = {0x3AD38, 0x0, 0x78, 0x84}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 16, value = 2.6}})
gg.sleep(12000)
local t = {"libUE4.so:bss", "Cb"}
local tt = {0x3AD38, 0x0, 0x78, 0x84}
local ttt = S_Pointer(t, tt, true)
...
local cfg_file = gg.EXT_FILES_DIR.."/"..gg.getFile():match("[^/]+$")..'(灵魂配置.).lua'
...
"请输入跳远数值:",
"是否开启防摔让灵魂高跳变得流畅",
},cfg,{"text","text","text","checkbox"})
if renwu == nil then else
gg.saveVariable(renwu, cfg_file)

if renwu[1] == true then gt() end
if renwu[2] == true then gx() end
if renwu[3] == true then ty() end
if renwu[4] == true then


local t = {"libUE4.so:bss", "Cb"}
local tt = {0x3AD38,0x0,0x48,0x8,0x184}
local ttt = S_Pointer(t, tt,true)
gg.addListItems({{address = ttt, flags = 16, value = 10,freeze = true}})
end
local t = {"libUE4.so:bss", "Cb"}
local tt = {0x3AD38,0x0,0x48,0x8,0x18C}
local ttt = S_Pointer(t, tt, true)
gg.addListItems({{address = ttt, flags = 16, value = renwu[1],freeze = true}})
local t = {"libUE4.so:bss", "Cb"}
local tt = {0x3AD38,0x0,0x48,0x8,0x1D8}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 16, value = renwu[2]}})
local t = {"libUE4.so:bss", "Cb"}
local tt = {0x3AD38,0x0,0x1018}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 16, value = renwu[3]}})
local t = {"libUE4.so:bss", "Cb"}
local tt = {0x44750,0x0,0x38,0x78,0x38}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 500}})--灵魂
end
							end,
					function()
...
--动态基址配置
function BaseAddressWrite(data) local so = gg.getRangesList(data.so) if so[1] ~= nil then local t = {} for i = 1, #data do t[#t+1] = {} t[#t].address = so[1].start + data[i].offset t[#t].value = data[i].value t[#t].flags = data[i].type end gg.setValues(t) else gg.toast("❌搜索不到") end end


--0x3AD38+0x0+0x90+0x608+0x38

    lhsl = readPointer('libUE4.so:bss', {0x3AD38,0x0,0x90,0x608,0x38}, 1)
local ttsl = readPointer('libUE4.so:bss', {0x3AD38,0x0,0x2F0,0x40,0x168,0x1C0,0x1B8}, 1)
local jznbsl = readPointer('libUE4.so:bss', {0x476588,0x20,0x270}, 1)
local jzdxsl = readPointer('libUE4.so:bss', {0x476588,0x20,0x27c}, 1)
local jzsxsl = readPointer('libUE4.so:bss', {0x476588,0x20,0x29c}, 1)
 zbxsl = readPointer('libUE4.so:bss', {0x3AD38,0x0,0x1b0,0x1c0}, 1)
 zbysl = readPointer('libUE4.so:bss', {0x3AD38,0x0,0x1b0,0x1c4}, 1)
 zbzsl = readPointer('libUE4.so:bss', {0x3AD38,0x0,0x1b0,0x1c8}, 1)
 local ds = readPointer('libUE4.so:bss', {0x3AD38,0x0,0x460,0x1Cc}, 1)--定点
...
if tt == 1032 then
gg.edits(lhsl, {{0, 16, 0, false}})
gg.edits(ds, {{0, 4, 0, false}})
if jzsx>-0.8 and jzsx<0.3 then--视角中
if jznb>-1.2 and jznb<-0.7 then--南
gg.edits(zbxsl, {{zbx-100, 16, 0, true}})
gg.edits(zbysl, {{zby+280, 16, 0, true}})
gg.edits(zbzsl, {{zbz+5, 16, 0, true}})

else
if jznb>0.7 and jznb<1.2 then--北
gg.edits(zbxsl, {{zbx+100, 16, 0, true}})
gg.edits(zbysl, {{zby-280, 16, 0, true}})
gg.edits(zbzsl, {{zbz+5, 16, 0, true}})
...
if tt == 8 or tt == 16 then
gg.edits(lhsl, {{10000, 16, 0, false}})
gg.edits(ds, {{1, 4, 0, false}})
...
Yk = gg.alert("检测循环探头路飞时打开修改器 是否返回 \n(点击空白处即可继续执行)","退出脚本","返回主页")
gg.sleep(150)
if  Yk~=1 and CB~=2 then
 gg.setVisible(false)
   ...
   Main1()
					...
					function()
local t = {"libUE4.so:bss", "Cb"}
local tt = {0x469C50,0x410,0x2F0,0x1E0,0x2CC}--@YSNNB6
local ttt = S_Pointer(t, tt, true)
...
string.toMusic('开启成功')
					...
					function()
local t = {"libUE4.so:bss", "Cb"}
local tt = {0x3AD38,0x0,0x1A0,0x0,0x7A8,0xB98}
local ttt = S_Pointer(t, tt, true)
...
while true do
so=gg.getRangesList('libUE4.so')[1].start
add=RUI(RUI(so+0xCA66820)+0x3F8)
for i=1,9999 do
  add1=RUI(0x8*i+add)+0x9C
  if readF(add1)==16.0 and readF(add1+0x4)==13 then
    setvalue(add1,16,x)
    setvalue(add1+0x4,16,y)
    add1=string.format("%X",add1)
    ad=ad..add1.."\n"
  end
end
end
