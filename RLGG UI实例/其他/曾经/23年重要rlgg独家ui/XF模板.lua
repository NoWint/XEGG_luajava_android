...
if panduan("/sdcard/长安1/图片/"..tmp) ~= true then
gg.toast("正在下载资源"..ii.."/"..#ckimg.."\n请耐心等待")
luajava.download("https://escape2020-1303126286.cos.ap-shenzhen-fsi.myqcloud.com/"..tmp,"/sdcard/长安1/图片/"..tmp)
else
if file.length("/sdcard/长安1/图片/"..tmp) <= 1 then
gg.toast("正在下载资源"..ii.."/"..#ckimg.."\n请耐心等待")
luajava.download("https://escape2020-1303126286.cos.ap-shenzhen-fsi.myqcloud.com/"..tmp,"/sdcard/长安1/图片/"..tmp)
end
end
if panduan("/sdcard/长安1/图片/"..tmp) ~= true then
gg.toast("正在下载资源"..ii.."/"..#ckimg.."\n请耐心等待")
luajava.download("https://rl-1303126286.cos.ap-beijing-fsi.myqcloud.com/"..tmp,"/sdcard/长安1/图片/"..tmp)
else
	if file.length("/sdcard/长安1/图片/"..tmp) <= 1 then
gg.toast("正在下载资源"..ii.."/"..#ckimg.."\n请耐心等待")
luajava.download("https://rl-1303126286.cos.ap-beijing-fsi.myqcloud.com/"..tmp,"/sdcard/长安1/图片/"..tmp)
...
if panduan("/sdcard/长安1/图片/opo1")~=true then
	file.unzip("/sdcard/长安1/图片/opo","/sdcard/长安1/图片/")
end
function 获取图片(txt)
txt = string.url(txt,"de")
ntxt = string.sub(string.gsub(txt,"/","-"),-10,-1)
if string.find(tostring(txt),"http") ~= nil then
if panduan("/sdcard/长安1/图片/"..ntxt) == false then
file.download(txt,"/sdcard/长安1/图片/"..ntxt)
else
	if file.length("/sdcard/长安1/图片/"..ntxt) <= 1 then
file.download(txt,"/sdcard/长安1/图片/"..ntxt)
end
end
txt = "/sdcard/长安1/图片/"..ntxt
end
return luajava.getBitmapDrawable(txt)
end
function getRes(x)
return 获取图片("/sdcard/长安1/图片/"..x)
end
...
function getShapeA(tmp0,tmp1,tmp2,tmp3)
jianbians = luajava.new(GradientDrawable)
...
function getShapeB(tmp0,tmp1,tmp2,tmp3)
jianbians = luajava.new(GradientDrawable)
jianbians:setCornerRadius(tmp0)
jianbians:setGradientType(GradientDrawable.LINEAR_GRADIENT)
jianbians:setColors(tmp1)
...
luajava.getIdView("backv"):setColorFilter(控件颜色[1])
checkbg = getShapeB(45,{控件颜色[1],控件颜色[2]},4,控件颜色[1])
...
公告内容="[changan热度奖励通知]\nchangan为了推进正版changan的发展与巩固热度，所以changan项目组实行热度奖励活动(该活动永久有效)\n1.在各大短视频平台发布关于changan的作品获得80赞以上的根据账号本人截图与作品热度情况可找相关客服领取2张永久卡\n2.在各大短视频平台使用changan进行直播，当前直播中人数到达50人以上可根据截图证明找相关客服领取2张永久卡\n鼓励大家使用changan才能促进更新的积极性!"

悬浮窗图标="https://out.zxglife.top/view.php/515f8ab328f02580e2d7b0fec15dc4c4.png"
...
sviews["第1页"]["第2栏"]={
	changan.box({ "道具赛" , ---box示例 可以删掉
					...
					changan.edit("道具ID" , "锁捡道具ID" ) , ---输入框名字，输入框提示
					changan.button("开启锁定" ,
						function()
