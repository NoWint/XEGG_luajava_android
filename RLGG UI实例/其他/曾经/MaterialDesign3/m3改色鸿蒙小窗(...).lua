...
for i=1,#MSeek do
SliderColor(luajava.getIdView(MSeek[i]),topic[1].seek[1],topic[1].seek[2],topic[1].seek[3])
end
...
firadio = {
    ...
    gravity = "center",
    orientation = "vertical",
    background = luajava.loadlayout{GradientDrawable,color=0xff000000,cornerRadius=20},
    LayoutTransition=layoutTransition
}
if type (views [1]) == "string" or type (views [1]) == "number" then
firadio [# firadio + 1] = {
...
for i=1,#MSeek do
SliderColor(luajava.getIdView(MSeek[i]),topic[nowbg].seek[1],topic[nowbg].seek[2],topic[nowbg].seek[3])
end
	YoYoImpl:with("FadeIn"):duration(600):playOn(floatWindow)
	end)
end

color = tonumber(0xff0062FF) or 0
local R, G, B = string.format('%06x', color):match('(%x%x)(%x%x)(%x%x)')
R = tonumber(R, 16)
G = tonumber(G, 16)
B = tonumber(B, 16)
print(R , G , B)
changecolor = luajava.loadlayout({
	LinearLayout,
	layout_height = "match_parent",
	layout_wight = "match_parent",
	orientation = 'vertical',
      {
			Slider,
			layout_weight = 1,
...
local color1, color2, color3 = table.unpack(input)
local color = tonumber(string.format('%02x%02x%02x', color1, color2, color3), 16)
			end
		},
      {
			Slider,
...
topic = {
	...
	0xffE4E4E4,0xFF000000--thumb(开,关)
		...
		seek={
	0xff0062FF,0x28FFFFFF,0xFF000000
	--活动轨道,禁止轨道,thumb
		}
	},
	{
	...
	0xffE4E4E4,0xFF000000
		...
		seek={
	0xFF00FFFF,0x28FFFFFF,0xFF000000
		}
	},
	{
		switch={
	...
	0xffE4E4E4,0xFF000000
		...
		seek={
	0xFFFF97FF,0x28FFFFFF,0xFF000000
		}
	},
	{
		switch={
	0xffBA4C33,0xFFFFC67C,--轨道(开,关)
	0xFFFFC67C,0xffF3DABC--thumb(开,关)
		},
		seek={
	0xffBA4C33,0xFFFFC67C,0xffBA5033
	--活动轨道,禁止轨道,thumb
		}
	},
}
...
{
  {
	...
	YC.button("按钮测试",function() gg.alert("余词牛逼") end),
	YC.button("6",function() gg.alert("6") end),
	YC.box({
		"灵动岛",
		YC.text("音乐播放器","0xffffffff","18sp",true),
