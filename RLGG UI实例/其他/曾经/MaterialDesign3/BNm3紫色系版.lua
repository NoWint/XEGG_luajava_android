...
if panduan("/sdcard/BN/图片/"..tmp) ~= true then
gg.toast("正在下载资源"..tmp.."\n请耐心等待")
luajava.download("https://escape2020-1303126286.cos.ap-shenzhen-fsi.myqcloud.com/"..tmp,"/sdcard/BN/图片/"..tmp)
end
end
ckimg = {
	"pubgT",
...
if panduan("/sdcard/BN/图片/"..ntxt) == false then
file.download(txt,"/sdcard/BN/图片/"..ntxt)
else
	if file.length("/sdcard/BN/图片/"..ntxt) <= 1 then
file.download(txt,"/sdcard/BN/图片/"..ntxt)
end
end
txt = "/sdcard/BN/图片/"..ntxt
end
return luajava.getBitmapDrawable(txt)
end
function getRes(x)
return 获取图片("/sdcard/BN/图片/"..x)
end
...
local x=tmp[1]
if string.find(x,"http")==nil then x="/sdcard/BN/图片/"..x end
local xx=tmp[2]
local xxx=tmp[3]
local func=function() 
...
return {
	...
	layout_weight = 1,
	{
		...
		layout_height = "fill_parent",
		layout_width = "fill_parent",
		background = getVerticalBG({0xaaffffff,0xaaffffff},25),
...
h1{
    ...
    letter-spacing: 5px;
    /* 渐变背景 */
    background: linear-gradient(to right,#EDDA39,#FFEB3D,#ffffff,#DAC362,#FFE65B);
    /* 裁剪掉文字以外的区域 */
...
--1内
	...
	layout_width = "fill_parent",
	layout_weight = 1,
	background = getVerticalBG({0x33ffffff,0x33ffffff},25),
	{
		LinearLayout,
		layout_height = "fill_parent",
		layout_width = "fill_parent",
...
luajava.runUiThread (function ()
	if tonumber (tostring (ckou: getVisibility ())) == 8.0 then
		...
		luajava.newThread(function()
			...
			gg.sleep(210)
			luajava.runUiThread(function()
				--changan.controlBig(floatWindow,200)
...
function Ctext(text,color,size,isjz)
if isjz then
...
function Cswitch(name,func1,func2,miaoshu)
nid = name..guid()
...
}, getVerticalBG({0x55ffffff,0x55ffffff},25,6,0xff181B2E))
selector:addState({
	-android.R.attr.state_pressed
...
function Cimage(img,height,width,pad,func)
...
thum:setSize(60, 60)
chazhi={} chajv={}
function Cseek(name,bian,smin,smax,nows)
_ENV[bian] =nows
	...
	vv = luajava.loadlayout({
		...
		tmp,
		{
			...
			orientation = "vertical",
			{
				...
				layout_width = "160dp",
				onClick = function() luajava.newThread(function()
					rlyunyz.unbind(changan.getedit("卡密"))
				end):start() end,
			},{
				TextView,
				background = luajava.loadlayout {
...
gg.alert(tostring(ret))
	...
	gg.playMusic("/sdcard/BN/图片/appear")
	if panduan("/sdcard/.android_tmps00oo.1") == false then
...
return getVerticalBG({0x99ffffff,0x99ffffff},15,8,0xaa232323)
end


slcta=getVerticalBG({0x33ffffff,0x33ffffff},15,8,0xaa242937)
--顶部当前页（未选中）背景
...
背景颜色={
	...
	0xcc000000,0xff381e72,0xcc000000
...
Cmenu({
	{---第一页
		Ctext("GAME防封","#ffffff","14sp"),
		Ctext("如果使用了启动游戏的过检测 可以不开防封 也可以自行搭配\n也可以不开过检测直接开防如果开过检测了可以直接奔放或者搭配防封 ","#7E9DB2","10sp"),					
		Cswitch(
			"LOGO防闪",
			function()
...
string.toMusic("LOGO防闪开启成功")		
	...
	{---第二页
		...
		Cimage(
			...
			"10dp",--内边距（可省略）
			function()
			gg.intent("https://t.me/BNNB6")
				--gg.QQgroup("群号")---qq群
				--gg.intent("链接")--网页（tg群）
			end--点击图片执行的功能（可省略）
			),
		Ctext("点击加入TG频道",nil,"11sp",true),---第四参数true为居中文字
		Ctext("\n\n"),
		Cbutton(
			"EXIT",
			function()
