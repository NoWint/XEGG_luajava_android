...
if panduan("/sdcard/长安/配置文件/tagcloud")~=true then
	file.unzip("/sdcard/长安/图片/cc99d42e9d7b70","/sdcard/长安/图片/")
end
...
local path = '/sdcard/长安/配置文件/tagcloud'
...
ktmp = "kam='"..ret.kami.."'"
luajava.newThread(function()
	file.write("/sdcard/长安/配置文件/km.lua",ktmp)
end):start()
luajava.runUiThread(function()
	parentv:setVisibility(View.GONE)
	tagcp:setVisibility(View.VISIBLE)
...
if nolts then return 0 end
	...
	http.get(server .. "time.txt", nil, function(uptime)
		if not isTable(uptime) or uptime.code ~= 200 then
			...
			nolts=true
			addchat(changan.text("聊天室网络异常，已暂时为你关闭"))
			return 0
		end
		...
		http.get(server .. "z2.txt",nil, function(res)
			...
			for i = 1, #newltres do
				...
				if isnew == true then
					...
					local t=changan.chat(localres[#localres])
...
-- 设置录音源、输出格式和编码器
					...
					local savePath = "/sdcard/长安/test.mp3"
					recorder:setOutputFile(savePath)
		            recorder:prepare()
					recorder:start()
					redstart=os.time()
			...
			gravity="left",
			{
				...
				onClick=function()
					...
					function()
						...
						file.download(tostring("http://149.88.75.158/"..channel.."/".."audio/"..playPath),"/sdcard/长安/音频/"..playPath)
						end
...
if not name then name = "未设置" end
local gid=guid()
local rstt = changan.intcard(name,name2,func1,func2,1,gid).view
vs[#vs+1]=rstt
...
luajava.runUiThread(function()
	...
	changan.controlRotation9(_ENV[gid..ii.."p"],0,180)
...
luajava.runUiThread(function()
	...
	changan.controlRotation9(_ENV[gid..ii.."p"],180,360)
	for i=1,#shous[gid][2] do
...
wyylog = luajava.loadlayout({
	...
	layout_width = "match_parent",
	{
		...
		layout_width = "match_parent",
		{
			...
			gravity="center",
			orientation="vertical",
			changan.text('请在上方搜索音乐关键词',0xff245AFF).view
			,
			},
