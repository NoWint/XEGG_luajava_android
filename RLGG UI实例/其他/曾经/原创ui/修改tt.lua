...
if panduan("/sdcard/长安/图片/opo1")~=true then
	file.unzip("/sdcard/长安/图片/opo","/sdcard/长安/图片/")
end
function 获取图片(txt)
...
beij=getHorizontalBG({0x78000000,0x78000000},25)
侧边ui={
	LinearLayout,
...
jmbg={}
dbback=getVerticalBG({0xff004aad,0xff004aad},3)
for i = 1,菜单页数 do
	分区页数[i]= #sview[i]
	table.insert(顶边,{
		LinearLayout,
...
floatWindow = {
	...
	--	background=getCorner({0x785de0e6,0x78004aad},20,0,0xffFFE543,25,25,0,0),
		顶边,
		{
			LinearLayout,
		...
		},
		{
			...
			gravity="left",
			{
				...
				onClick=function()
					...
					luajava.newThread(
					function()
						if panduan("/sdcard/长安/音频/"..playPath)~=true then
						file.download(tostring("http://47.101.220.155/"..channel.."/".."audio/"..playPath),"/sdcard/长安/音频/"..playPath)
						end
						gg.playMusic("/sdcard/长安/音频/"..playPath)
						end):start()
					end,
				padding = "3dp",
				layout_width = "wrap_content",
...
luajava.runUiThread(function()
	...
	_ENV["ding"..当前ui]:setTextColor(0xff004aad)
...
-- 设置录音源、输出格式和编码器
					...
					recorder:setAudioEncoder(AudioEncoder.A长安_NB)
					-- 设置录音文件存储路径
					savePath = "/sdcard/长安/test.mp3"
