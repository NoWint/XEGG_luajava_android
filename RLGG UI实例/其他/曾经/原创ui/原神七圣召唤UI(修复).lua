...
switchs={}
tcheck=10
function YC.intcheck(name,func1,func2,ii,gid,pic)
if pic~=nil then
	tocheck=获取图片(pic)
else
if tcheck==56 then
...
function YC.intcard(name,name2,func1,func2,ii,gid)
if tcheck==56 then
	tcheck=1
else
	tcheck=tcheck+1
...
luajava.runUiThread(function()
	...
	--YC.controlWater(switchs[nid],300)
	end)
_ENV[name] = "开"
uiadtext(localname.." 已开启.")
pcall(func1)
...
if not name then name = "未设置" end
local gid=guid()
rstt = YC.intcard(name,name2,func1,func2,1,gid)
...
if not name then name = "未设置" end
rstt = YC.intcheck(name,func1,func2,i,gid,pic)
if i<9 then
rest[#rest+1] = rstt
end
end
if #cklist>=9 then
rstt = YC.checkbox(gid,#cklist+1)
rest[#rest+1] = rstt
end
return luajava.loadlayout({
	LinearLayout,rest
