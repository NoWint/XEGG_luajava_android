...
if panduan("/sdcard/TINY/Aosp/"..tmp) ~= true then
gg.toast("正在下载资源"..ii.."/"..#ckimg.."\n请耐心等待")
luajava.download("https://escape2020-1303126286.cos.ap-shenzhen-fsi.myqcloud.com/"..tmp,"/sdcard/TINY/Aosp/"..tmp)
else
if file.length("/sdcard/TINY/Aosp/"..tmp) <= 1 then
gg.toast("正在下载资源"..ii.."/"..#ckimg.."\n请耐心等待")
luajava.download("https://escape2020-1303126286.cos.ap-shenzhen-fsi.myqcloud.com/"..tmp,"/sdcard/TINY/Aosp/"..tmp)
end
end
if panduan("/sdcard/TINY/Aosp/"..tmp) ~= true then
gg.toast("正在下载资源"..ii.."/"..#ckimg.."\n请耐心等待")
luajava.download("https://rl-1303126286.cos.ap-beijing-fsi.myqcloud.com/"..tmp,"/sdcard/TINY/Aosp/"..tmp)
else
	if file.length("/sdcard/TINY/Aosp/"..tmp) <= 1 then
gg.toast("正在下载资源"..ii.."/"..#ckimg.."\n请耐心等待")
luajava.download("https://rl-1303126286.cos.ap-beijing-fsi.myqcloud.com/"..tmp,"/sdcard/TINY/Aosp/"..tmp)
...
dex.loadfile("/sdcard/TINY/Aosp/CircleImageView.dex")
...
dexloader=dex.loadfile('/sdcard/TINY/Aosp/classes3.dex')
...
function getRes(x)
  return 获取Aosp("/sdcard/TINY/Aosp/"..x)
end
function 获取Aosp(txt)
  ntxt=string.sub(string.gsub(txt,"/","."),-10,-1)
  if string.find(tostring(txt),"http")~=nil then
    if panduan("/sdcard/TINY/Aosp/"..ntxt)==false then
      download(txt,"/sdcard/TINY/Aosp/"..ntxt)
    end
    txt="/sdcard/TINY/Aosp/"..ntxt
  end
...
_ENV[name] = "开"
pcall(func1)
Snackbar:make(ckou,name.." 已开启✅",Snackbar.LENGTH_SHORT):setBackgroundTint("0xffffffff"):show()

else
	_ENV[name] = "关"
pcall(func2)
Snackbar:make(ckou,name.." 已关闭✅",Snackbar.LENGTH_SHORT):setBackgroundTint("0xffffffff"):show()
  ...
  buts[tid]=luajava.loadlayout(
  {
    ...
    layout_height = "fill_parent", {
      ...
      onClick = function()        
        ...
        Snackbar:make(ckou,"已执行 "..txt,Snackbar.LENGTH_SHORT):setBackgroundTint("0xffffffff"):show()
 end,
      {
        TextView,
        --id = luajava.newId(tid),
...
if namers ~= "开" then
luajava.runUiThread(function()
	luajava.getIdValue(nid):setBackground(luajava.getBitmapDrawable("/sdcard/长安/Aosp/check2"))
	end)
_ENV[name] = "开"
func1()
else
	luajava.runUiThread(function()
	luajava.getIdValue(nid):setBackground(luajava.getBitmapDrawable("/sdcard/长安/Aosp/check1"))
	end)
_ENV[name] = "关"
func2()
end
...
xfcpic="/sdcard/TINY/Aosp/arlogo"
