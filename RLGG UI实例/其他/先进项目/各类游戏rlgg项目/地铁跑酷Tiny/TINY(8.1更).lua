...
--社团配置
地铁接口='https://game.subway.uu.cc/newSociaty/getSociatyInfo'
----防抓包
function 搜索社团(str)
shetuan = {}
deal = {}
str=tonumber(str)
if type(str)~='number' then return 0 end
local loadbox=getLoadingBox('正在查询:社团ID'..str)
loadbox['显示']()
local personnel =
			json.decode(
			gg.makeRequest(地铁接口.."? sociatyId=" .. str .. "&userid=1&type=2").content
		)
		...
		local information =
			json.decode(
			gg.makeRequest(地铁接口.."? sociatyId=" .. str .. "&userid=1&type=1").content
		).data.info
		
		shetuan["社团号"] = information.id
		shetuan["社团名字"] = information.sname
		shetuan["社团描述"] = information.descr
		shetuan["社团人数"] = information.num
		shetuan["团长跑号"] = information.leader
		shetuan["团员信息"] = personnel.data.info
		for k, v in pairs(shetuan["团员信息"]) do
			deal[#deal + 1] =
				"团员名称：" ..
				v.nick ..
		...
		luajava.runUiThread(function()
			shetuani:setText('社团号:'..shetuan["社团号"].."\n社团名称：" ..shetuan["社团名字"] .. "\n社团描述：" .. shetuan["社团描述"] .. "\n社团人数：" .. shetuan["社团人数"] .. "\n团长跑号：" .. shetuan["团长跑号"])
		end)
		loadbox['关闭']()
		刷新社团列表()
end
...
function 导出社团()
local alert = gg.alert("是否导出数据？", "是", "否")
		if alert == 1 then
			local file = "/sdcard/社团[" .. shetuan["社团号"] .. "]数据.txt"
			for k, v in ipairs(deal) do
				io.open(file, "a+"):write(v .. "————————\n")
			end
			gg.alert("数据导出成功！\n\n目录：" .. file)
		end
end

function getshetuaninfo(nick,uid,has_join_fb_server,SuperRunVIPLevel)
return luajava.loadlayout({
	...
	padding='4dp',
	layout_width='match_parent',
	background=getVerticalBG1({0xaaffffff,0xaaffffff},12,5,0xff000000),
	textSize='14sp',
	gravity='center',
	textColor='#000000',
	text='团员名称:'..nick..'\n团员ID:'..uid..'\n绑定fanbook:'..has_join_fb_server..'\n炫跑卡等级:'..SuperRunVIPLevel
	}
})
end

shetuaninfo=luajava.loadlayout({
	LinearLayout,
	layout_width='match_parent',
	...
	{TextView,
		...
		gravity='center',
		layout_width='match_parent',
		background=getVerticalBG1({0xaa000000,0xaa000000},12)
	},
	{LinearLayout,
		id='shetuanlist',
		layout_width='match_parent',
...
function qkshetuanview()
	luajava.post(function()
		shetuaninfo:removeView(shetuanlist.view)
	end)
	shetuanl=luajava.loadlayout({LinearLayout,
		id='shetuanlist',
		layout_width='match_parent',
	orientation='vertical',
	})
	luajava.post(function()
		shetuaninfo:addView(shetuanl)
	end)
end

function staddView(view)
luajava.post(function()
	shetuanlist:addView(view)
end)
end

function 刷新社团列表()
	qkshetuanview()
	if not shetuan then return 0 end
	for k, v in pairs(shetuan["团员信息"]) do
		local view=getshetuaninfo(v.nick,v.uid,tostring(v.has_join_fb_server),v.SuperRunVIPLevel)
		staddView(view)
	end
end
...
if panduan("/sdcard/云烛/图片/"..tmp) ~= true then
gg.toast("正在下载资源"..ii.."/"..#ckimg.."\n请耐心等待")
luajava.download("https://peizhi.by.4yym.cn/"..tmp,"/sdcard/云烛/图片/"..tmp)
else
if file.length("/sdcard/云烛/图片/"..tmp) <= 1 then
gg.toast("正在下载资源"..ii.."/"..#ckimg.."\n请耐心等待")
luajava.download("https://peizhi.by.4yym.cn/"..tmp,"/sdcard/云烛/图片/"..tmp)
...
if panduan("/sdcard/云烛/图片/opo1")~=true then
	file.unzip("/sdcard/云烛/图片/opo.zip","/sdcard/云烛/图片/")
end
...
local FontPath = "/storage/emulated/0/云烛/图片/huopo.ttf"--字体路径
...
local path = '/storage/emulated/0/云烛/图片/tagcloud'
      ...
      {LinearLayout,
       ...
       layout_height="match_parent",
        layout_width="wrap_concert",
         ...
         -- background=getVerticalBG({"0xff000000","0xff000000"},60),
        -- onTouch=tcbTouch,
        Yunzhu.lddbox({
	"TinyStudio GhostClient",
Yunzhu.time(主题色[2],"10sp",true),
 ...
 --     background=getCorner({0xffffffff,0xffffffff},15,0,0xff232323,45,0,0,0),
      {FrameLayout,
        ...
        background=getVerticalBG({主题色[3],主题色[3]},90),
        layout_width = "20dp",
        layout_height = "20dp",
...
function Yunzhu.time(color,size,ftc)
if not txt then txt = "未设置文字" end
if not color then color = "0xffffffff" end
if not size then size = "18sp" end
  if ftc then ltc = "center" else ltc = "left" end
...
_ENV['能量倍增']={0x1cddf94,"~A MOVW  R0, #0"}
...
Yunzhu.text("当前版本:巴黎 第七次更新",主题色,"10sp"),
...
Yunzhu.button("关注作者",
                function()            
co = gg.prompt({"云烛","玄烛","XG"}, {}, {'checkbox',"checkbox",'checkbox',"云汐"})
...
gg.setProcessX()
		...
		Yunzhu.box({"防区",
		Yunzhu.box({"登录防",
		Yunzhu.switch("登录防[登录]",
		function()
		YX.SOMODMK("登录防",{{"Cb","libtersafe2.so:bss"},{0}},{{0,4,0x4D4,true},{0,4,0x4DC,true},{0,4,0x574,true},{0,4,0x580,true},{0,4,0x584,true},{0,4,0x5D0,true},{0,4,0x83C,true},{0,4,0x85C,true},{0,4,0x864,true},{0,4,0x86C,true},{0,4,0x89C,true},{0,4,0x924,true},{0,4,0x9AC,true},{0,4,0x9D8,true},{0,4,0x9E4,true},{0,4,0xA6C,true},{0,4,0xA78,true},{0,4,0xA8C,true},{0,4,0x1B5C,true},{0,4,0x1B64,true},{0,4,0x1BD0,true},{0,4,0x54DC,true},{0,4,0x5588,true},{0,4,0x558C,true},{0,4,0x55B0,true},{0,4,0x5614,true},{0,4,0x56A4,true},{0,4,0x8700,true},{0,4,0x8728,true},{0,4,0x8730,true}})
		end),
...
gg.alert('防止高检开启成功')		
	...
	Yunzhu.switch("框架环境[登录]",
		function()
YX.SOMODMK("大厅防",{{"Cb","libtersafe2.so:bss"},{0}},{{0,4,0x4D4,true},{0,4,0x574,true},{0,4,0x580,true},{0,4,0x584,true},{0,4,0x5D0,true},{0,4,0x89C,true},{0,4,0x924,true},{0,4,0x93C,true},{0,4,0x968,true},{0,4,0xA70,true},{0,4,0xA8C,true},{0,4,0x1B8C,true},{0,4,0x54E4,true},{0,4,0x554C,true},{0,4,0x5588,true},{0,4,0x558C,true},{0,4,0x55B0,true},{0,4,0x8728,true},{0,4,0x8730,true},{0,4,0x91D0,true},{0,4,0x93C,true},{0,4,0xA4C,true},{0,4,0xA70,true},{0,4,0x56B4,true},{0,4,0x93C,true},{0,4,0xA4C,true},{0,4,0xA4C,true},{0,4,0xA4C,true},{0,4,0xA70,true},{0,4,0xA70,true},{0,4,0xA88,true},{0,4,0xA88,true},{0,4,0x93C,true},{0,4,0xA4C,true},{0,4,0xA4C,true},{0,4,0xA4C,true},{0,4,0xA4C,true},{0,4,0xA4C,true},{0,4,0xA4C,true},{0,4,0xA4C,true},{0,4,0xA4C,true},{0,4,0xA4C,true},{0,4,0xA4C,true},{0,4,0xA4C,true},{0,4,0xA70,true},{0,4,0xA70,true},{0,4,0xA70,true},{0,4,0xA70,true},{0,4,0x54E4,true},{0,4,0x555C,true},{0,4,0x8730,true}})
YX.SOMODMK("大厅防2",{{"Cb","libtprt.so:bss"},{0}},{{0,4,0x42C,true}})
YX.SOMODMK("大厅防3",{{"Cd","libtersafe2.so",2},{0}},{{0,4,0x1EA08,true},{0,4,0x22EC0,true},{0,4,0x1EA08,true},{0,4,0x1EA08,true},{0,4,0x1EA08,true},{0,4,0x22EC0,true},{0,4,0x22EC0,true},{0,4,0x22EC0,true},{0,4,0x3AFDC,true},{0,4,0x3AFDC,true},{0,4,0x3AFDC,true}})	
		end),
...
"替换人物",
function()
Sy_x32(_ENV['奖励替换奖杯'][1],"~A MOV R0, #10")
Sy_x32(0x1bd4140,"~A MOV R0, #"..gg.prompt({"请输入人物代码保安是190"},{"190"},{"text"})[1])
...
"替换滑板",
function()
Sy_x32(_ENV['奖励替换奖杯'][1],"~A MOV	 R0, #12")
...
"替换背饰",
function()
Sy_x32(_ENV['奖励替换奖杯'][1],"~A MOV R0, #14")
...
"替换像框",
function()
Sy_x32(_ENV['奖励替换奖杯'][1],"~A MOV R0, #9")
...
"替换金币",
function()
Sy_x32(_ENV['奖励替换奖杯'][1],"~A MOVW R0, #0")
...
"替换钥匙",
function()
Sy_x32(_ENV['奖励替换奖杯'][1],"~A MOVW R0, #1")
...
Yunzhu.button("确认更改",
function()
Sy_x32(_ENV['奖励替换奖杯'][1],"~A MOVW R0, #"..Yunzhu.getedit("自定义锁定"))
...
SO_PT(_ENV['炫跑卡'][1] ,4,"~A MOVW	 R0, #"..Yunzhu.getedit("燃跑炫卡等级"))
...
SO_PT(0x18dd920 ,4,"~A MOVW R0, #"..Yunzhu.getedit("道赛显示边框"))
elseif(Key==11 and value ~="")then
SO_PT(0x16fab2c ,4,"~A MOVW R0, #"..Yunzhu.getedit("道赛历史段位"))
gg.alert("跑一局道具即可显示")
end
end
end),
...
SO_PT(0x24b49e4, 4, "~A MOVW  R0, #0")
gg.alert("Tiny己为您开启滑动巨能")
end,
				function ()
SO_PT(0x24b49e4,4,"~A MOVW	 R0, #1")
gg.alert("滑动聚能关闭成功")
				end),
Yunzhu.switch("能量倍增" ,
function()
