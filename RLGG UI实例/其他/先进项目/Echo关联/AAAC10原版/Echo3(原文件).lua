...
changan.controlRotation = function(control, time,t)
	luajava.runUiThread(function()
		import "android.view.animation.Animation"
		import "android.animation.ObjectAnimator"
...
function changan.text(text,color,size,isjz)
	...
	buts={}
	heir=getRes("heir")
function changan.button(name, func, color)
  local nid = name..string.randomUUID(true)
...
function changan.box(views)
  ...
  if type(views[1])=="string" or type(views[1])=="number" then
    ...
    gg.alert("changan.box 的 table 内第一个元素必须是 string") os.exit()
  end
  radios={
    LinearLayout,
    layout_marginLeft="5dp",
...
changan.menu({
  {---第一页          
    changan.text(项目名字,'0xff161616',"15sp",true),
...
luajava.showAlert(dialogBuilder)						
      ...
      changan.seek("拉条演示","ltxx",1,50,1),--“ltxx”为此拉条变量名字，使用时每个拉条不可重复
      changan.button(
      "调用拉条",
      function()
        gg.alert(ltxx)
