...
launchui = luajava.loadlayout({
	...
	layout_width = "match_parent",
	{
		...
		gravity="center",
		{
			...
			gravity = "center",
			layout_gravity="center_horizontal",
						background=getHorizontalBG({0x25ffffff,0x25ffffff},15,5,0xfff2f3f5),
...
if not color then color = "#333333" end
...
function tosearch(besearch)
	...
	elseif #result==0 then
		...
		adtext("你是否在找:","#333333")
		for i=1,#result do
			adtext("第"..result[i][2].."页第"..result[i][3].."板块的“"..result[i][1].."”","#333333")
		end
		切换(result[1][2])
		if string.find(besearch,"打开")~=nil or string.find(besearch,"开启")~=nil or string.find(besearch,"启动")~=nil or string.find(besearch,"执行")~=nil then
			if #result==1 then
				...
				adtext("搜索到"..#result.."个结果，请手动执行","#333333")
			end
		end
...
slcta = getShape(
	15,
	{
	0xffF9F5F3,0xffF9F5F3,0xffF9F5F3
},
	0,0xfff2f3f5)
...
--if not yans then yans = "#333333" end
...
return {luajava.loadlayout(
	...
	Typeface=font,
		...
		gravity = "left",
		background=getTextBG({
--	0xE6E9F000,0xff121212,0xffE6E9F0
0xffDCDFE6,0xffDCDFE6,0xffDCDFE6
})
	}),"标题:"..txt,function() end,"textview"}
end
function Yunzhu.time(color,size,ftc)
if not txt then txt = "未设置文字" end
if not color then color = "0xff333333" end
