...
launchui = luajava.loadlayout({
	...
	layout_width = "match_parent",
	{
		...
		gravity="center",
		{
			...
			gravity = "center",
			layout_gravity="center_horizontal",
						background=getHorizontalBG({0x25000000,0x25000000},15,5,0xff111111),
...
if not color then color = "#919293" end
...
function tosearch(besearch)
	...
	elseif #result==0 then
		...
		adtext("你是否在找:","#919293")
		for i=1,#result do
			adtext("第"..result[i][2].."页第"..result[i][3].."板块的“"..result[i][1].."”","#919293")
		end
		切换(result[1][2])
		if string.find(besearch,"打开")~=nil or string.find(besearch,"开启")~=nil or string.find(besearch,"启动")~=nil or string.find(besearch,"执行")~=nil then
			if #result==1 then
				...
				adtext("搜索到"..#result.."个结果，请手动执行","#919293")
			end
		end
...
luajava.runUiThread (function ()
	...
	_ENV["jmt"..i]:setTextColor(0xff919293)
...
slcta = getShape(
	15,
	{
	0xff141218,0xff141218,0xff141218
},
	0,0xff141218)
...
--if not yans then yans = "#919293" end
...
if not gtvb4 then gtvb4 = 0 gtvb5 = 0x19191900 end
local jianbians = luajava.new(GradientDrawable)
jianbians:setCornerRadius(gtvb3)
jianbians:setGradientType(GradientDrawable.LINEAR_GRADIENT)
jianbians:setColors(gtvb1)
...
local jianbians = luajava.new(GradientDrawable)
jianbians:setCornerRadius(15,15,0,0)
jianbians:setGradientType(GradientDrawable.LINEAR_GRADIENT)
...
return {luajava.loadlayout(
	...
	Typeface=font,
		...
		gravity = "center",
		background=getTextBG({
--	0x19191900,0xff121212,0xff191919
0x75131517,0x554F378B,0x75131517
}),
	}),"标题:"..txt,function() end,"textview"}
end
function Yunzhu.time(color,size,ftc)
if not txt then txt = "未设置文字" end
if not color then color = "0xff919293" end
...
function Yunzhu.button(txt,func,txtc)
  ...
  if not txtc then txtc="#919293" end
...
bglist={
"/storage/emulated/0/云烛/图片/yunzhuBGblue",
getShape(30,{0x78000000,0x78000000},0,0x01B8B8B8),
getShape(30,{0xe800FFC1,0x8800AFFF},0,0x01B8B8B8),
	getShape(30,{0xff131517,0xff131517},0,0xff121212),
...
---侧边标题和图标
	{
		"Aimbot", "https://cdn-community.codemao.cn/47/community/d2ViXzEwMDFfMTAwMTQwOTNfMTAwMTQwOTNfMTcwNjUyNTg1MTY4NV80ZmRmNjBjMw.png"

	},
	{
		"Visuals", "https://cdn-community.codemao.cn/47/community/d2ViXzEwMDFfMTAwMTQwOTNfMTAwMTQwOTNfMTcwNjUyNTg1MTY4NV80ZmRmNjBjMw.png"

	},
	{
		"Skins", "https://cdn-community.codemao.cn/47/community/d2ViXzEwMDFfMTAwMTQwOTNfMTAwMTQwOTNfMTcwNjUyNTg1MTY4NV80ZmRmNjBjMw.png"
	...
	{
		"Other", "https://cdn-community.codemao.cn/47/community/d2ViXzEwMDFfMTAwMTQwOTNfMTAwMTQwOTNfMTcwNjUyNTg1MTY4NV80ZmRmNjBjMw.png"
...
Yunzhu.seek("Kill X","ltlz",1,100,19),--名字，变量名字，最小，最大，初始
	Yunzhu.seek("Kill Y","ltlz",1,100,87),--名字，变量名字，最小，最大，初始
	Yunzhu.seek("Esp Size","ltlz",1,100,37),--名字，变量名字，最小，最大，初始
...
swits["第1页"]["第2板块"] = {
	Yunzhu.text2("TRIGGER"),
	Yunzhu.seek("ui大小","uikd",1,3000,410),--名字，变量名字，最小，最大，初始
	Yunzhu.seek("雷达Y","ltlz",1,100,87),--名字，变量名字，最小，最大，初始
	Yunzhu.seek("雷达大小","ltlz",1,100,37),--名字，变量名字，最小，最大，初始
}-- 此板块结束
UI宽度=uikd.."dp"--620dp全屏 410dp半屏
