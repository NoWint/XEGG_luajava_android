...
function Yunzhu.menu(sview)
  ...
  floatWindow=luajava.loadlayout{
    ...
    {
      MaterialCardView,
      background={GradientDrawable,color=0xFF111111,cornerRadius=20},
      layout_width="match_parent",
  ...
  Downprogress:setIndicatorColor({0xfff5f5f5})
  Downprogress:setTrackColor(0x16f5f5f5)
  --luajava.getIdView("progressIndicator"):setIndicatorColor({0xff1E8FFF, 0xff3555FF, 0xff682BFF})
  luajava.getIdView("progressIndicator"):setIndicatorColor({
    0xFF6A1B9A,  -- 深紫色
    0xFF8E24AA,  -- 中等紫色
    0xFFAB47BC,  -- 浅紫色
    0xFFBA68C8,  -- 更浅的紫色
    ...
    if 灵动 then
      luajava.startThread(function()
        ...
        luajava.runOnUiThread(function()
          ...
          YoYoImpl:with("FadeIn"):duration(100):playOn(luajava.getIdView("Smalldisplay"))
        end)
      end)
     else
      luajava.startThread(function()
        ...
        luajava.runOnUiThread(function()
          ...
          YoYoImpl:with("FadeIn"):duration(100):playOn(luajava.getIdView("music"))
        end)
      end)
    end
  end
...
return jianbians
  ...
  function seeColor()
    luajava.startThread(function()
      luajava.runUiThread(function()
        ...
        luajava.getIdView("tab2"):setTabTextColors(0xfff4f4f4,"0xff"..RGBcolor)
  ...
  function 修改控件颜色(color)
    luajava.runOnUiThread(function()
      tab:setTabTextColors(0xfff4f4f4,"0xff"..RGBcolor)
...
---菜单
  menuView= luajava.loadlayout {
    ...
    layout_height="match_parent",
    {
      ...
      orientation="vertical",
      {
        ...
        layout_width="100dp",
          layout_height="25dp",
          layout_gravity="center",
          background=getCorner({0xFF000000,0xFF000000},55,0,0xff232323,40,40,40,40),
        },
        {
          FrameLayout,
    ...
    background={GradientDrawable,color=0xFF191919,cornerRadius=00},
...
--    layout_marginRight="20dp",
    ...
    layout_height='wrap_content',
    elevation="8dp",
    background={GradientDrawable,color=0x00161616,cornerRadius=90},
    LayoutTransition=layoutTransition,
    ...
    },
    {
      ...
      visibility="gone",
      layout_width="fill_parent",
      background = getHorizontalBG({0xaa000000,0xaa000000},15,1,0xffffffff),
      onClick=function() end,
      onTouch=function(view, event)
        local action = event:getAction()
        if action == MotionEvent.ACTION_DOWN then
...
"守护进程标题",
		...
		Yunzhu.text("   设定 Settings  ",0xffffffff,"19sp"),
		Yunzhu.text("   Powered by YunZhu",0xfff4f4f4,"12sp"),	
		Yunzhu.check({
					{
						"例子xxxxxx",--名称
						function()--开
		...
		Yunzhu.quyu({
"守护进程信息",		
		Yunzhu.text("   Context: Service",0xfff4f4f4,"12sp"),
		Yunzhu.text("   使用通知： : 否",0xfff4f4f4,"12sp"),
		Yunzhu.text("   运行守护： : 否",0xfff4f4f4,"12sp"),
		Yunzhu.text("   在虚拟空间中使用root：：否",0xfff4f4f4,"12sp"),		
		...
		Yunzhu.button2(
					...
					Yunzhu.seek("拉条演示","ltxxa",1,50,1),--“ltxx”为此拉条变量名字，使用时每个拉条不可重复
					}),
  ...
  if type(views[1])=="string" or type(views[1])=="number" then
    firadio[# firadio + 1]={
      ...
      gravity="center_vertical",
      {
       ...
       background = getHorizontalBG({0x00ff00cf,0x005500ff},15,1,0x00ffffff),
        {
          TextView,
          gravity="top",
          text=views[1],
...
switchs[gid..ii] = luajava.loadlayout{
	...
	padding = "1dp",
	{
		...
		gravity = "center_horizontal",
		orientation="vertical",
		background=getVerticalBG({0x80161616,0x80161616,0x80161616},50,3,0x80161616),
		{
			ImageView,
			id = luajava.newId(gid..ii.."p"),
			src = tocheck,
...
switchs[gid..ii] = {
	...
	
	{
		...
		layout_height = "wrap_content",
		gravity = "left",
		background=getVerticalBG({0xaa161616,0xaa161616},30),
		orientation="vertical",
		padding="5dp",
		--background=getVerticalBG({0xffFFFDF2,0xddffffff,0xffFFFDF2},15,8,0xffFFDA71),
		{
...
xfcpic="/storage/emulated/0/云烛/图片/N2M4Ng.png"
...
Yunzhu.menu({
  {---第一页  
  Yunzhu.quyu({        "YunzhuNB",
    Yunzhu.text(项目名字,'0xfff4f4f4',"25sp",true),
    Yunzhu.text("精准覆盖技术 颠覆性解决方案\nQ群1016641691",'0xfff4f4f4',"10sp",true),
    }),
    Yunzhu.quyu({
"",
Yunzhu.quyu({
"守护进程标题",
		...
		Yunzhu.text("   守护进程",0xffffffff,"19sp"),
		Yunzhu.text("   Powered by YunZhu",0xfff4f4f4,"12sp"),	
		}),
		Yunzhu.quyu({
"守护进程信息",		
...
Yunzhu.quyu({
"标题",
		Yunzhu.text("   获取VIP",0xffffffff,"19sp"),
		Yunzhu.text("   向开发者捐赠以表示支持。",0xfff4f4f4,"12sp"),
		}),
		Yunzhu.quyu({
		"",
		Yunzhu.text("   即刻购买曜界至臻VIP 使用更多独家功能",0xfff4f4f4,"12sp"),		
		Yunzhu.buttonlink(
					"了解更多",
					function()
						local tmp=Yunzhu.getedit("输入框演示")
...
"",
		Yunzhu.quyu({
"守护进程信息",
		Yunzhu.text("   最新版本 27.0[27000] => YZ kernel",0xfff4f4f4,"12sp"),
		Yunzhu.text("   当前版本 27.0[27000] => YZ kernel",0xfff4f4f4,"12sp"),
		}),		
		Yunzhu.quyu({
"",
Yunzhu.text("   注意事项",0xffffffff,"19sp"),
}),
		Yunzhu.quyu({
		"",
		Yunzhu.text("   仅从官方 GitHub 页面下载 曜界至臻",0xfff4f4f4,"12sp"),
		Yunzhu.text("   未知来源的文件可能具有恶意行为！",0xfff4f4f4,"12sp"),		
					}),										
					}),
...
Yunzhu.quyuM({
"",
Yunzhu.text("   通过链接电脑启动(使用 adb)",0xfff4f4f4,"15sp"),
--Yunzhu.text("   \n",0xfff4f4f4,"12sp"),
Yunzhu.text("   对于没有root的设备需要借助adb来启动Shizuku。这个过程每次设备重新启动时都需要重新进行。",0xfff4f4f4,"9sp"),
Yunzhu.buttonM(
					"↺ 查看指令",
					function()
						local tmp=Yunzhu.getedit("输入框演示")
...
"",
Yunzhu.text("   启动(针对已root设备)",0xfff4f4f4,"15sp"),
--Yunzhu.text("   \n",0xfff4f4f4,"12sp"),
Yunzhu.text("   另外，Shizuku可以在开机时自动启动。如果没有，请检查您的系统或是第三方工具是否限制了Shizuku。",0xfff4f4f4,"9sp"),
Yunzhu.buttonM(
					"▶ 启动",
					function()
						local tmp=Yunzhu.getedit("输入框演示")
