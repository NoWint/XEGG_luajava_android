...
if 卡密 == '' then gg.alert("你UI卡密呢？\n快去找云烛买卡密！") qq.join("3359239164") os.exit() end
local info = {
...
local kamiyz = rlyunyz.postApi('kmlogon',{kami=卡密})
if kamiyz == "卡密不存在" then gg.alert("卡密不存在\n快去找云烛买卡密！") qq.join("3359239164") os.exit() end
...
local path = '/storage/emulated/0/云烛/配置文件/tagcloud'
...
function YunzhuMainmenu(sview)
	...
	topbar={
		...
		layout_width="fill_parent",
		layout_height="35dp",
		background=getCorner({0xffffffff,0xffffffff},15,0,0xff6750a4,15,15,0,0),
elevation='2dp',
		{LinearLayout,
			layout_width="35dp",
			layout_height="match_parent",
	...
	{onPageSelected=function(view)
			...
			Typeface=font,
			_ENV["jmt"..view+1]:setBackground(YZdhla)
	...
	ImageView,
		...
		background=getHorizontalBG({0x00f2f3f5,0x00f2f3f5},15,2,0xffffffff),
		layout_width = "135dp",
		layout_height = "40dp",
		onTouch = hanshu,
		onClick = 隐藏,
...
--	beij=newbg(背景颜色,15)
beij=getHorizontalBG({0xfff2f3f5,0xfff2f3f5},15,2,0xff6750a4)
...
floatWindow = {
    ...
    Yunzhu.box({
	"切换页面",
	 Yunzhu.button(YZstab[1],function() luajava.newThread(function() 切换(1)  end):start() end ),
	 Yunzhu.button(YZstab[2],function() luajava.newThread(function() 切换(2)  end):start()end ),
	 Yunzhu.button(YZstab[3],function() luajava.newThread(function() 切换(3)  end):start()end ),
	 Yunzhu.button(YZstab[4],function() luajava.newThread(function() 切换(4)  end):start() end ),
	 Yunzhu.button(YZstab[5],function() luajava.newThread(function() 切换(5)  end):start() end ),
	...
	local function invoke()
		...
		mainLayoutParams.height = YZg
		mainLayoutParams.width = YZk
...
boxes = {} boxpic = {}
function Yunzhu.lddboxcebian(views)
local tid = "box"..guid ()
local ttid = tid.."6"
local t1id = guid ()
kgid = views[1].."关"
...
---setThumbTintList('0xff000000'),
	...
	function Yunzhu.buttonred(txt,func,txtc)
		if not txt then txt = "未设置" end
		if not txtc then txtc="#ff0000" end
		local tid="Cbutton"..guid()
		buts[tid]=luajava.loadlayout(
		{
			LinearLayout,
		...
		layout_height = "wrap_content", {
			...
			layout_marginRight='3dp',
			elevation='2dp',
			background = getHorizontalBG({0x00000000,0x00111111},15,2,0xffff0000),
			padding="10dp",
				onClick = function()
					Yunzhu.controlWater(buts[tid],300)
					luajava.newThread(func):start() end,
	...
	function Yunzhu.buttonlink(txt,func,txtc)
		...
		layout_height = "wrap_content", {
			...
			layout_marginRight='3dp',
			elevation='2dp',
			background = getHorizontalBG({0xffff00cf,0xff5500ff},15,1,0xffffffff),
			padding="10dp",
				onClick = function()
					Yunzhu.controlWater(buts[tid],300)
					luajava.newThread(func):start() end,
		...
		local tmp1={TextView,
			...
			onClick=function()
				...
				if _ENV[bian]>smin then
					...
					if bian=='YZg' or bian=='YZk' then daxiao() end
				end
			end,
		}
		local tmp2={TextView,
...
elseif Action == MotionEvent.ACTION_MOVE then
isMove = true
YZg = tonumber(hx) + (event:getRawY() - RawY)
if YZg >= 250 and YZg <= 11100 then
mainLayoutParams.height = YZg
end
YZk = tonumber(hy) + (event:getRawX() - RawX)
if YZk >= 250 and YZk <= 24500 then
mainLayoutParams.width = YZk
end
if YZk<=250 and YZg<=250 then YZk=250 YZg=250
--隐藏()
return 0 end
window:updateViewLayout(floatWindow, mainLayoutParams)
end
...
--底部导航栏（未选中）背景
YZdhlb=getVerticalBG({0x99ffffff,0x99ffffff},15,0,0xaa232323)



YZdhla=getVerticalBG({0xff6750A4,0xff6750A4},15,0,0xaa242937)
--底部导航栏（选中）背景
...
云烛月租 YZ10 静谧土星UI
====================
本UI控件已全部升级material design 3.0规范
购买UI卡密找3359239164
反馈UI的问题或者有待提高的地方也可以找我 如果确实有必要我会继续更新
====================
卡密价格
...
Yunzhu.text文字
Yunzhu.button按钮
Yunzhu.button2紫色空心按钮
Yunzhu.buttonred红色空心按钮
Yunzhu.buttonlink彩色高亮按钮
Yunzhu.buttonM质感规范按钮
Yunzhu.box动画盒子
Yunzhu.edit输入框
Yunzhu.seek正常拉条
Yunzhu.seek2加减拉条
...
项目名称即为你的公告内容和部分标题以及灵动岛内显示内容
YZstab为页面名称配置 最好只保留五个
上部灵动岛点击后可以搜索音乐 下方灵动岛小横条可以切换页面 或者你可以直接滑动切换页面
区域框不是box 而且yunzhu.quyu和yunzhu.quyuM不一样 自己查看区别
还有更多内容你可以自己探索
...
"守护进程标题",
		...
		Yunzhu.text("   守护进程",switch颜色,"19sp"),
		Yunzhu.text("   Powered by YunZhu",0xff333333,"12sp"),	
		}),
		Yunzhu.quyu({
"守护进程信息",		
		Yunzhu.text("   Context: Service",0xff333333,"12sp"),
		Yunzhu.text("   使用通知： : 否",0xff333333,"12sp"),
		Yunzhu.text("   运行守护： : 否",0xff333333,"12sp"),
		Yunzhu.text("   在虚拟空间中使用root：：否",0xff333333,"12sp"),		
		}),
		Yunzhu.quyu({
		"",
		Yunzhu.button2(
...
Yunzhu.quyu({
"标题",
		Yunzhu.text("   获取VIP",switch颜色,"19sp"),
		Yunzhu.text("   向开发者捐赠以表示支持。",0xff333333,"12sp"),
		}),
		Yunzhu.quyu({
		"",
		Yunzhu.text("   即刻购买静谧土星VIP 使用更多独家功能",0xff333333,"12sp"),		
		Yunzhu.buttonlink(
					"了解更多",
					function()
						local tmp=Yunzhu.getedit("输入框演示")
...
"",
		Yunzhu.quyu({
"守护进程信息",
		Yunzhu.text("   最新版本 27.0[27000] => YZ kernel",0xff333333,"12sp"),
		Yunzhu.text("   当前版本 27.0[27000] => YZ kernel",0xff333333,"12sp"),
		}),		
		Yunzhu.quyu({
"",
Yunzhu.text("   注意事项",switch颜色,"19sp"),
}),
		Yunzhu.quyu({
		"",
		Yunzhu.text("   仅从官方 GitHub 页面下载 静谧土星",0xff333333,"12sp"),
		Yunzhu.text("   未知来源的文件可能具有恶意行为！",0xff333333,"12sp"),		
					}),										
					}),
...
---以上是magisk风格面板实例 下面是Shizuku风格面板实例
...
Yunzhu.quyuM({
"",
Yunzhu.text("   通过链接电脑启动(使用 adb)",0xff000000,"15sp"),
--Yunzhu.text("   \n",0xff000000,"12sp"),
Yunzhu.text("   对于没有root的设备需要借助adb来启动Shizuku。这个过程每次设备重新启动时都需要重新进行。",0xff000000,"9sp"),
Yunzhu.buttonM(
					"↺ 查看指令",
					function()
						local tmp=Yunzhu.getedit("输入框演示")
...
"",
Yunzhu.text("   启动(针对已root设备)",0xff000000,"15sp"),
--Yunzhu.text("   \n",0xff000000,"12sp"),
Yunzhu.text("   另外，Shizuku可以在开机时自动启动。如果没有，请检查您的系统或是第三方工具是否限制了Shizuku。",0xff000000,"9sp"),
Yunzhu.buttonM(
					"▶ 启动",
					function()
						local tmp=Yunzhu.getedit("输入框演示")
	...
	{---第三页
		...
		Yunzhu.seek2('窗口宽度','YZg',250,dwidth,YZg,10),
		Yunzhu.seek2('窗口高度','YZk',250,dheight,YZk,10),
		Yunzhu.switch(
			"音量键隐藏UI",--改名会导致不能自动开启
			function()
				音量键=true
