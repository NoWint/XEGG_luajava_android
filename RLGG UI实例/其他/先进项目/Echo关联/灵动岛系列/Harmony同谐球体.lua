...
if panduan("/sdcard/云烛/配置文件/tagcloud")~=true then
	file.unzip("/sdcard/云烛/图片/cc99d42e9d7b70","/sdcard/云烛/图片/")
end
...
local path = '/sdcard/云烛/配置文件/tagcloud'
local dexloader = dex.loadfile(path)
local TagCloudView = dexloader:loadClass('com.moxun.tagcloudlib.view.TagCloudView')
local MyTagsAdapter = dexloader:loadClass('cn.rlyun.a3dtagcloudandroid.MyTagsAdapter')
...
stit=luajava.loadlayout{ShimmerTextView,
	...
	ckou={FrameLayout,
		layout_height='match_parent',
		layout_width='match_parent',
		background=newbg({0xff111111,0xff111111},0,0xffffffff,25,25,25,25),
...
topbar = luajava.loadlayout({
	...
	layout_height="35dp",
	background=getCorner({0xff111111,0xff111111},15,0,0xff232323,20,20,0,0),
			...
			onTouch = hanshu,
	{
			ImageView,
			gravity="center",
			background=getVerticalBG({0xff191919,0xff191919},90),
			src=getRes('ca_hm'),
			layout_width = "20dp",
			padding='2dp',
			layout_height = "20dp",
...
floatWindow = {
	...
	elevation='2dp',
		...
		{LinearLayout,
			...
			{LinearLayout,
				...
				layout_width='14dp',
				layout_marginLeft='-2dp',
				background=newbg({0x33ffffff,0x33ffffff},2,0x88ffffff,0,10,10,0),
				gravity='center',
				text='←',
				textSize='10sp',
				textColor='#343434',
				...
				layout_height='match_parent',
				layout_width='14dp',
				background=newbg({0x33ffffff,0x33ffffff},2,0x88ffffff,10,0,0,10),
				gravity='center',
				text='→',
				textSize='10sp',
				textColor='#d7d7d7',
	...
	{LinearLayout,
		...
		elevation='3dp',
				...
				padding='8dp',
					{LinearLayout,
						layout_weight=1,
							...
							background=newbg({0xffffffff,0xffffffff},0,0xff232323,10,10,10,10),
							padding='3dp',
							elevation='2dp',
							{ScrollView,
								id='ggscro',
				...
				layout_marginTop='3dp',
						...
						layout_marginLeft='3dp',
						layout_marginRight='3dp',
				background=newbg({0xffffffff,0xffffffff},0,0xff2d2d2d,10,10,10,10),
						src=getRes('bm_music'),
						padding='4dp',
	...
	Yunzhu.text("Powered By 云烛","0xffffffff","10sp",true),
...
ktmp = "kam='"..ret.kami.."'"
luajava.newThread(function()
	file.write("/sdcard/云烛/配置文件/km.lua",ktmp)
end):start()
luajava.runUiThread(function()
	parentv:setVisibility(View.GONE)
	tagcp:setVisibility(View.VISIBLE)
...
-- 设置录音源、输出格式和编码器
					...
					local savePath = "/sdcard/云烛/test.mp3"
					recorder:setOutputFile(savePath)
		            recorder:prepare()
					recorder:start()
					redstart=os.time()
			...
			gravity="left",
			{
				...
				onClick=function()
					...
					function()
						...
						file.download(tostring("http://149.88.75.158/"..channel.."/".."audio/"..playPath),"/sdcard/云烛/音频/"..playPath)
						end
...
boxpic[tid]:setColorFilter(0xff191919)
...
--SwitchColor(_ENV[nid],控件颜色,"0x90000000")
...
_ENV [t1id] = luajava.loadlayout ({
    ...
    gravity = "center",
    {
      ...
      layout_marginRight = "4dp",
	  layout_marginLeft = "4dp",
      background = luajava.loadlayout{GradientDrawable,color=控件颜色,cornerRadius=20},
      firadio
      }
      })
return _ENV [t1id]
