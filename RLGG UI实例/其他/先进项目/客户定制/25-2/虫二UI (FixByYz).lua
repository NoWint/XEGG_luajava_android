...
if not io.open ( "/storage/emulated/0/长安.zip" ) then

	local f = io.open ( "/storage/emulated/0/长安.zip" , 'a' )

	f : write ( gg.makeRequest ( 'https://xg-cloudreve.cloud-k1.riyx.cn/f/2VpSo/%E5%A4%8F%E5%A4%A9%E9%85%8D%E7%BD%AE.zip' ).content )---下载zip资源

end

file.mkdir("/storage/emulated/0/长安")----创文件夹
file.mkdir("/storage/emulated/0/长安/图片")
file.mkdir("/storage/emulated/0/长安/图片")

unzip("/storage/emulated/0/长安.zip","/storage/emulated/0/长安/图片/")---解压
local typeface=import("android.graphics.Typeface")
local FontPath="/storage/emulated/0/长安/图片/字体.ttf"--字体路径
...
slcta = getCorner({0xffa0d3ee,0xffa0d3ee},12,0,0xffa0d3ee,0,4,4,0)
...
cebian = {
	...
	layout_marginButton = "5dp",
	background=getVerticalBG({0xffffffff,0xffffffff},18,3,0xffa0d3ee),
}
for i = 1,#stab do
cebian[#cebian+1] = {
LinearLayout,
	...
	layout_margin = "3dp",
	gravity = "center",
	onClick = function() changan.controlWater2(floatWindow,500)	切换(i) end,
	{
		ImageView,
		background=获取图片(stab[i][2]),
		layout_height='27dp',
		layout_width='27dp',
...
ckou = {
	...
	padding='3dp',
	{
		...
		padding = "2dp",
		layout_height='match_parent',
		background=getVerticalBG({0xffa0d3ee,0xffa0d3ee},18,3,0xffa0d3ee),
		{
			LinearLayout,
			gravity = "center",
			{
...
floatWindow = {
    ...
    {LinearLayout,
      ...
      {LinearLayout,
        ...
        {LinearLayout,
	changan.lddLsland2({"",
	--changan.text("  "..项目名称,"0xffffffff","13sp",true),
	{FrameLayout,
    layout_width = "match_parent",
    layout_height = "140dp",
	...
	src=getRes("盗用资源死妈哦"),
		...
		changan.music2("拉条演示","Baisu",1,100,1),},{ImageView,
		   layout_hight="20dp",
			layout_width='20dp',
			 layout_Right="10dp",
			layout_gravity="right|center",
	src=getRes("524085136"),
	onClick=function() 
			vibra:vibrate(15)
...
luajava.showAlert(dialogBuilder)
		   ...
		   layout_hight="20dp",
			...
			layout_gravity="left|bottom",
			  layout_marginLeft="20dp",
			  	onClick=function() 
			  	vibra:vibrate(15)
			  	 Snackbar:make(ckou,"收藏成功✅",Snackbar.LENGTH_SHORT):setBackgroundTint("0xFFA0C9FD"):show()
	...
	onClick=function() 
	vibra:vibrate(15)
	 Snackbar:make(ckou,"取消成功✅",Snackbar.LENGTH_SHORT):setBackgroundTint("0xFFA0C9FD"):show()
...
mediaPlayer:start()
			vibra:vibrate(15)
	ssss1:setVisibility(View.VISIBLE)
	...
	mediaPlayer:pause()
	  ...
	  id="QieHuan",
		layout_hight="20dp",
		layout_width='20dp',
		src = getRes("1884812492"),
		layout_marginRight="20dp",
		layout_gravity="right|bottom",
			onClick=function() 
			vibra:vibrate(15)
			 Snackbar:make(ckou,"循环播放成功✅",Snackbar.LENGTH_SHORT):setBackgroundTint("0xFFA0C9FD"):show()
			mediaPlayer:setLooping(true)
		sssss1:setVisibility(View.VISIBLE)
	  ...
	  id="sssss1",
		layout_hight="20dp",
		layout_width='20dp',
		src = getRes("1360640248"),
		layout_marginRight="20dp",
		layout_gravity="right|bottom",
		onClick=function() 
		mediaPlayer:seekTo(Baisu)
			vibra:vibrate(15)
		sssss2:setVisibility(View.VISIBLE)
	  sssss1:setVisibility(View.GONE)
	        	end,
	  },
	  {ImageView,
	  id="sssss2",
		layout_hight="20dp",
		layout_width='20dp',
		src = getRes("836253849"),
		layout_marginRight="20dp",
		layout_gravity="right|bottom",
		onClick=function() 
			vibra:vibrate(15)
			 Snackbar:make(ckou,"关闭循环播放成功✅",Snackbar.LENGTH_SHORT):setBackgroundTint("0xFFA0C9FD"):show()
			mediaPlayer:setLooping(false)
		QieHuan:setVisibility(View.VISIBLE)
	  sssss2:setVisibility(View.GONE)
	        	end,
	  },
	  },
	}),
	...
	layout_width="14dp",
	gravity="center",
	background=getCorner({0xffa0d3ee,0xffa0d3ee},12,0,0xffa0d3ee,0,15,15,0),
	{TextView,
				text="»",
				textSize="18sp",
			}
	...
	changan.lddLsland({
	"                              ",
	changan.text("  "..项目名称,"0xffffff","13sp",true),
	--changan.text("  静谧内部绕过","0xffffffff","10sp"),
	
	changan.seek("音量设置","lt",1,100,1),--名字，变量名字，最小，最大，初始
	
    changan.box({
	"切换页面",
	 changan.button(stab[1],function() luajava.newThread(function() 切换(1)  end):start() end ),
	 changan.button(stab[2],function() luajava.newThread(function() 切换(2)  end):start()end ),
	 changan.button(stab[3],function() luajava.newThread(function() 切换(3)  end):start()end ),
	 changan.button(stab[4],function() luajava.newThread(function() 切换(4)  end):start() end ),
	 changan.button(stab[5],function() luajava.newThread(function() 切换(5)  end):start() end ),
	changan.text("  ","0xffffffff","100sp"),
	}),
	...
	项目名称,
	--changan.text("  "..项目名称,"0xffffffff","13sp",true),
	changan.text("  Powered By 长安","0xff6750a4","10sp",true),
	
	changan.seek("调节音量","lt",1,10,1),--名字，变量名字，最小，最大，初始
...
for i=1,#MSwitch do
				SwitchColor(luajava.getIdView(MSwitch[i]),0xffa0d3ee,0xffffffff,0xeeeeeeee,0x88888888)
			end
...
beij = getVerticalBG({0xffa0d3ee,0xffa0d3ee},20,3,0xffa0d3ee)
...
boxes = {} boxpic = {}
function changan.lddLsland(views)
local tid = "LDD"..guid ()
local ttid = tid.."6"
local t1id = guid ()
kgid = views[1].."关"
...
--SwitchColor(_ENV[nid],灵动岛,"0xff252525")
...
_ENV [t1id] = luajava.loadlayout ({
    ...
    gravity = "center",
    {
      ...
      layout_marginRight = "4dp",
	  layout_marginLeft = "4dp",
      background = luajava.loadlayout{GradientDrawable,color=灵动岛,cornerRadius=30},
      firadio
      }
      })
return _ENV [t1id]
...
--	buts={}
	...
	function changan.button(txt,func,txtc)
		...
		if not txtc then txtc="#333333" end
	...
	buts={}
	heir=getRes("heir")
	function changan.button2(txt,func,txtc)
		if not txt then txt = "未设置" end
		if not txtc then txtc="#6750a4" end
		local tid="Cbutton"..guid()
		buts[tid]=luajava.loadlayout(
		{
			LinearLayout,
		...
		layout_height = "wrap_content", {
			...
			layout_marginRight='3dp',
			elevation='2dp',
			background = getHorizontalBG({0x00000000,0x00111111},15,2,0xff6750a4),
			padding="10dp",
				onClick = function()
					changan.controlWater(buts[tid],300)
					luajava.newThread(func):start()
	...
	buts={}
	heir=getRes("heir")
	function changan.buttonM(txt,func,txtc)
		if not txt then txt = "未设置" end
	...
	chazhi={} chajv={}
function changan.music2(name,bian,smin,smax,nows)
_ENV[bian] =nows
...
rest = luajava.loadlayout({
	...
	thumbHeight='23dp',
	trackHeight='15dp',
	trackStopIndicatorSize='7dp',
	tickVisible=true,--true
	__onFinish=function(v)
...
function 开关3(name,func1,func2,nid)
  ...
  if type(func1) == "function" then
	...
	function getButtonBG()
		...
		selector:addState({
			-android.R.attr.state_pressed
		}, getVerticalBG({0x88ffffff,0x88ffffff},17))
		return selector
	end
...
function 二级分页(tab)
	...
	local tmp={
		...
		{LinearLayout,
			padding='2dp',
			background=getVerticalBG({0xffa0d3ee,0xffa0d3ee},15),
			{TextView,
			Typeface=font,
				text=tab['第1页名字'],
				textSize='11sp',
	...
	function changan.radio (cklist)
		if not radoff then
			radon=getVerticalBG({0xffa0d3ee,0xffa0d3ee},360,20,0xffa0d3ee)
			radoff=getVerticalBG({0x00ffffff,0x00ffffff},360,8,0xffa0d3ee)
		...
		rest = luajava.loadlayout({
			...
			layout_hight = "match_parent",
			{
				...
				},
				{
					...
					tickVisible=false,
					__onFinish=function(v)
						v:setTrackActiveTintList(ColorStateList({{},},{0xffa0d3ee}))
						v:setTrackInactiveTintList(ColorStateList({{},},{0xffa0d3ee}))
						v:setThumbTintList(ColorStateList({{},},{0xffa0d3ee}))
						luajava.setInterface(v, 'addOnChangeListener',
						function(SeekBar, var2, var3)
							_ENV[bian] = var2
						end)
...
function changan.intcheck(name,func1,func2,size)
	nid = name..guid()
	lname=name
	...
	local rest = {
		...
		padding = "1dp",
		{
			...
			gravity = "center_vertical",
			{
				...
				buttonTintList=ColorStateList({{android.R.attr.state_checked},{-android.R.attr.state_checked}},{0xffa0d3ee,0xffa0d3ee},{0xffa0d3ee,0xffa0d3ee}),
				onClick=function()
					luajava.newThread(function()
						pcall(func)
					end):start() end
...
switchs[gid..ii] = {
	...
	
	{
		...
		layout_height = "wrap_content",
		gravity = "left",
		background=getVerticalBG({0xaaffffff,0xaaffffff},30),
		orientation="vertical",
		padding="5dp",
		--background=getVerticalBG({0xffFFFDF2,0xddffffff,0xffFFFDF2},15,8,0xffFFDA71),
		{
...
vibra:vibrate(9)
luajava.runUiThread(function()
	luajava.getIdValue(nid.."t"):setTextColor(0xff0091FE)
	luajava.getIdValue(nid.."t2"):setTextColor(0xff0091FE)
	luajava.getIdValue(nid.."p"):setColorFilter(0xff0091FE)
...
--菜单名字，添加即可加页数，需要与结尾配置表页数对应
	{ "主页", "https://www.cccimg.com/down.php/a421bc2dabb37814c121a68fa4b1b0ed.png"},
	{ "防封", "https://www.cccimg.com/down.php/a421bc2dabb37814c121a68fa4b1b0ed.png"},
	{ "刷号", "https://www.cccimg.com/down.php/a421bc2dabb37814c121a68fa4b1b0ed.png"},
	{ "音乐", "https://www.cccimg.com/down.php/a421bc2dabb37814c121a68fa4b1b0ed.png"},
	{ "设置", "https://www.cccimg.com/down.php/a421bc2dabb37814c121a68fa4b1b0ed.png"},
}


菜单背景="https://www.cccimg.com/down.php/a421bc2dabb37814c121a68fa4b1b0ed.png"

左上角图标='https://pan.jl8.top/view.php/a5cce3328d4b5171967802a0488b084d.jpeg'

小悬浮窗图标 = "https://pan.jl8.top/view.php/a5cce3328d4b5171967802a0488b084d.jpeg"
--------窗链接或路径
		...
		changan.groupbox({
					{
						...
						--"https://postimage.me/images/2024/08/07/709da244ba3f4bb81f8040f9a2b2e09a.png"
					}, {
						"例子",
						function()
			...
			二级分页({
				...
				['第1页']={
					...
					changan.seek('例子','lz1',1,10,5),--名字，变量名，最小，最大，初始
					changan.button('调用拉条例子',
						function()
							gg.alert(lz1)--上边定义拉条变量名lz1直接调用
						end),
					changan.text("下方是check例子（单列）","#000000","20sp"),
					changan.check({
							{
								"例子",
								function()
			...
			二级分页({
				...
				['第1页']={
					...
					changan.text("下方是check例子","#000000","20sp"),
					changan.card({
					{
						"防封",
						function()
...
gg.playMusic(muchen)
			...
			changan.switch('音量键隐藏',--默认开，改名就是默认关
				function()
					音量键=true
				end,
				function()
