...
if panduan("/sdcard/云烛/图片/opo1")~=true then
	file.unzip("/sdcard/云烛/图片/opo","/sdcard/云烛/图片/")
end
function 获取图片(txt)
...
beij=getHorizontalBG({0xfff5f5f5,0xfff5f5f5},25)
侧边ui={
	LinearLayout,
...
jmbg={}
dbback=getVerticalBG({0xff004eff,0xff004eff},3)
for i = 1,菜单页数 do
	分区页数[i]= #sview[i]
	table.insert(顶边,{
		LinearLayout,
	...
	for j = 1,分区页数[i] do
		jmbg[i][j]={
			getCorner({0x00004eff,0x00004eff},10,0,0xff000000,20,20,20,20),
...
local menus={
	...
	background=getCorner({0xaaffffff,0xaaffffff},20,0,0xffffffff,0,0,20,0),
	{
		ScrollView,
		fillViewport = "true",
--padding = "10dp",
...
floatWindow = {
	...
	{--顶部栏
		...
		layout_width="fill_parent",
		 background=getCorner({0xffffffff,0xffffffff},20,0,0xffff0090,25,25,0,0),
		   Elevation="3dp",
		顶边,
...
luajava.runUiThread(function()
	for k,v in pairs(floattable) do
		if v==1 then
			Yunzhu.controlSmall(_ENV[k],400)
		end
	end
		...
		},
		{
			...
			gravity="left",
			{
				...
				onClick=function()
					...
					luajava.newThread(
					function()
						if panduan("/sdcard/云烛/音频/"..playPath)~=true then
						file.download(tostring("http://47.101.220.155/"..channel.."/".."audio/"..playPath),"/sdcard/云烛/音频/"..playPath)
						end
						gg.playMusic("/sdcard/云烛/音频/"..playPath)
						end):start()
					end,
				padding = "3dp",
				layout_width = "wrap_content",
...
luajava.runUiThread(function()
	...
	_ENV["ding"..当前ui]:setTextColor(0xff004eff)
	_ENV["layout"..当前ui]:setVisibility(View.VISIBLE)
	_ENV['dingbar'..当前ui]:setBackground(dbback)
	Yunzhu.controlBigY(_ENV['dingbar'..当前ui],600)
...
Yunzhu.controlBigY = function(control,time)
luajava.runUiThread(function()
	import "android.animation.ObjectAnimator"
	ObjectAnimator():ofFloat(control,"scaleX", {
		0, 0.4, 0.7, 1
...
-- 设置录音源、输出格式和编码器
					...
					recorder:setAudioEncoder(AudioEncoder.A云烛_NB)
					-- 设置录音文件存储路径
					savePath = "/sdcard/云烛/test.mp3"
...
悬浮窗图标 = "https://rl-1303126286.cos.ap-beijing-fsi.myqcloud.com/Yunzhu"
...
FIRSTPAGE = {--创建分页，结尾调用
	...
	[1]={--第一分区
		['分区名字']="公告",
		['功能配置']={
			Yunzhu.text("   公告AAAAAAAAA","#000000","15sp"),	
					Yunzhu.button('选择进程',gg.setProcessX),
				Yunzhu.switch(
					"防封菜单",
					function()
