...
function LSQ_Chain(so, offset, format, value, type, Function)--模块设置, 偏移量, 功能参数, 17024, 类型, 功能
...
function pdcf(lujing) rec = "/storage/emulated/0/BY/配置文件/"..lujing fille,err = io.open(rec) if fille == nil then return false else return true end end
sleep = gg.sleep
--if gg.isHTTPdump()==true and panduan("/storage/emulated/0/BY/配置文件/vpn.txt") == false then gg.alert("\n抓包？") os.exit() end
...
write("/storage/emulated/0/BY/配置文件/"..lujing,neirong)
end
function rdcf(lujing)
return read("/storage/emulated/0/BY/配置文件/"..lujing)
end
function checkimg(tmp)

if panduan("/sdcard/BY/图片/"..tmp) ~= true then
gg.toast("正在下载资源"..tmp.."\n请耐心等待")
download("https://wf980627-512739054-1317447113.cos.ap-nanjing.myqcloud.com/"..tmp,"/sdcard/BY/图片/"..tmp)
end
end
...
tmp1 = 0
if audiokg == "开" then
if panduan("/sdcard/BY/音频/"..ress) == true then
pmusic("/sdcard/BY/音频/"..ress)
tmp1 = 1
else
	download("https://wf980627-512739054-1317447113.cos.ap-nanjing.myqcloud.com/"..ress ,"/sdcard/BY/音频/"..ress)
pmusic("/sdcard/BY/音频/"..ress)
tmp1 = 1
end
end
...
slctb = luajava.loadlayout {
	GradientDrawable,
	color = "#00272327",----侧边栏背景颜色
	cornerRadius = 12
}
slcta = luajava.loadlayout {
	GradientDrawable,
	color = "#FFC1C1",-----侧边栏选项颜色
	cornerRadius = 16
}
...
ckou = luajava.loadlayout({
	...
	layout1,layout2,layout3,layout4,layout5,
	{
		...
		orientation = "vertical",
		{
			...
			background = {
				GradientDrawable,
				color = "#00000000,#00ff0000",-----悬浮窗背景颜色
				cornerRadius = 16
			},
...
ckou = luajava.loadlayout(	{LinearLayout,
		...
		{LinearLayout,
			layout_height='match_parent',
			layout_width='match_parent',
			background=getHorizontalBG({0xaaffFFC1C1,0xaaFFC1C1},30),
			{LinearLayout,
				layout_height='match_parent',
				layout_width='match_parent',
				layout_weight=1,
			...
			{
			TextView,
			text = string.fromHtml(getTimeStamp(os.time()).." <font color=#FFC1C1>".."启动完毕".."</font>"),
			textSize = "12sp",
			--textColor = color,
			gravity="left",
			layout_width = "wrap_content",
...
function 隐藏()
luajava.runUiThread(function()
--control:setBackground(luajava.getBitmapDrawable("/sdcard/BY/图片/imgui"))
	if tonumber(tostring(cebian:getVisibility())) == 8.0 then
	ggscrom:setVisibility(View.VISIBLE)
...
--control:setBackground(luajava.getBitmapDrawable("/sdcard/BY/图片/imgui"))
	ggscrom:setVisibility(View.GONE)
...
firadio[#firadio+1] = {
	...
	background = getSelector3(),
	{
		...
		background = "/sdcard/BY/图片/hsanjiao",
		layout_width = "10dp",
		layout_height = "10dp",
		layout_marginTop = "5dp",
	},
...
jianbians:setStroke(0,"0xff456ff00")--边框宽度和颜色
...
jianbians:setGradientType(GradientDrawable.LINEAR_GRADIENT)
jianbians:setColors({0x6600FF76,0x9900A5FF})
jianbians:setOrientation(GradientDrawable.Orientation.LEFT_RIGHT)
jianbians:setStroke(4,"0x663fff000")--边框宽度和颜色
return jianbians
end
...
jianbians:setStroke(0,"0x778fff66")--边框宽度和颜色
return jianbians
end
...
radios[#radios+1] = {
	...
	}),
	{
		...
		layout_width = "20dp",
		layout_height = "20dp",
		background = "/sdcard/BY/图片/"..radio[i][2]
	},
	{
		TextView,
		text = radio[i][1],
...
rstt = luajava.loadlayout({
	...
	onClick = function() luajava.newThread(function() func() end):start() end,
	{
		...
		layout_marginLeft = "8dp",
		layout_marginRight = "0dp",
		background = "/sdcard/BY/图片/check1",
	}, {
		TextView,
		gravity = "top",
		text = name,
...
namers = _ENV[name]
if namers ~= "开" then
luajava.getIdValue(nid):setBackground(luajava.getBitmapDrawable("/sdcard/BY/图片/checkon2"))
_ENV[name] = "开"
pcall(func1)
else
luajava.getIdValue(nid):setBackground(luajava.getBitmapDrawable("/sdcard/BY/图片/checkof"))
_ENV[name] = "关"
pcall(func2)
end
...
rstt = luajava.loadlayout({
	...
	layout_marginRight = "10dp",
	gravity = "center_vertical",
	background = "/sdcard/BY/图片/checkof",
	onClick = function() luajava.newThread(function() func() end):start() end,
	{
		TextView,
		gravity = "top",
...
if namers ~= "开" then
luajava.runUiThread(function()
	luajava.getIdValue(nid):setBackground(luajava.getBitmapDrawable("/sdcard/BY/图片/check2"))
	end)
_ENV[name] = "开"
func1()
else
	luajava.runUiThread(function()
	luajava.getIdValue(nid):setBackground(luajava.getBitmapDrawable("/sdcard/BY/图片/check1"))
	end)
_ENV[name] = "关"
func2()
end
...
ttview:setBackground(luajava.getBitmapDrawable("/sdcard/BY/图片/hsj"))
else
	tview:setVisibility(View.GONE)
ttview:setBackground(luajava.getBitmapDrawable("/sdcard/BY/图片/sj"))
end
end
...
local func2 = cklist[i][3]
if not name then name = "未设置" end
local rstt = intswitch2(name,func1,func2)
rest[#rest+1] = rstt
end
return luajava.loadlayout({
	LinearLayout,rest
...
swits={}
function intswitch2(name,func1,func2)
nid = name..guid()
swits[nid]={
	--getVerticalBG({0xff00ACFF,0xff4472FF},18),
	getVerticalBG({0xffFF9DBC,0xffFF9DBC},18),
	false
}
local func = boxfunc(name,func1,func2,nid)
if not name then name = "未设置" end
...
控件颜色={0xff80C3F9,0xff80C3F9}
--empty=getVerticalBG({0xeeeeeeee,0xeeeeeeee},18,3,0xaa1a1b1d)
empty=getVerticalBG({0xeeeeeeee,0xeeeeeeee},18,3,0xff80C3F9)
function boxfunc(name,func1,func2,nid)
local sname = nid
...
--拖动条：
--seek("文本","bianliang",0,100,6)
--参数依次为 控件名字，变量名字，最小值，最大值，初始值
--这里传入的是"bianliang"
--那么只需要gg.alert(bianliang)即可调用
...
text("地铁跑酷幻想城\nInfo免费版 开挂要扣税不开挂要扣税绕过要扣税没绕过也要扣税不和我税要扣税和我税了要扣税税了我也要扣税小蓝酿要和我税","#FFC1C1","20sp"),
particle("BY","By Yunzhu",{"#FFC1C1","#ffffff","#FFC1C1"}),
},		
...
os.remove("/storage/emulated/0/Android/data/com.tencent.ig/files/cabundle.pem")
os.remove("/storage/emulated/0/Android/data/com.tencent.ig/files/cacheFile.txt")
os.remove("/storage/emulated/0/Android/data/com.tencent.ig/files/loginidentifier.txt")
...
os.remove("/storage/emulated/0/Android/data/com.tencent.ig/files/UE4Game/ShadowTrackerExtra/ShadowTrackerExtra/Saved/Paks/1375135419_225_2.4.1.17139_20230202163013_1505504061_cures.ifs.res")
...
os.remove("/storage/emulated/0/Android/data/com.pubg.krmobile/files/cabundle.pem")
os.remove("/storage/emulated/0/Android/data/com.pubg.krmobile/files/cacheFile.txt")
os.remove("/storage/emulated/0/Android/data/com.pubg.krmobile/files/loginidentifier.txt")
...
os.remove("/storage/emulated/0/Android/data/com.pubg.krmobile/files/UE4Game/ShadowTrackerExtra/ShadowTrackerExtra/Saved/Paks/1375135419_225_2.4.1.17139_20230202163013_1505504061_cures.ifs.res")
...
os.remove("/storage/emulated/0/Android/data/com.rekoo.pubgm/files/cabundle.pem")
os.remove("/storage/emulated/0/Android/data/com.rekoo.pubgm/files/cacheFile.txt")
os.remove("/storage/emulated/0/Android/data/com.rekoo.pubgm/files/loginidentifier.txt")
...
os.remove("/storage/emulated/0/Android/data/com.rekoo.pubgm/files/UE4Game/ShadowTrackerExtra/ShadowTrackerExtra/Saved/Paks/1375135419_225_2.4.1.17139_20230202163013_1505504061_cures.ifs.res")
...
os.remove("/storage/emulated/0/Android/data/com.vng.pubgmobile/files/cabundle.pem")
os.remove("/storage/emulated/0/Android/data/com.vng.pubgmobile/files/cacheFile.txt")
os.remove("/storage/emulated/0/Android/data/com.vng.pubgmobile/files/loginidentifier.txt")
...
os.remove("/storage/emulated/0/Android/data/com.vng.pubgmobile/files/UE4Game/ShadowTrackerExtra/ShadowTrackerExtra/Saved/Paks/1375135419_225_2.4.1.17139_20230202163013_1505504061_cures.ifs.res")
...
text("↓正常版局内不闪↓","#FFC1C1","15sp"),
switch("PUBG大厅过检测",
				function()
--gg.lookKillGG()-----杀GG
local t = {"libanogs.so:bss", "Cb"}
local tt = {0x62A0, 0x510, 0x198, 0x450, 0x42C}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 130024}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x15018, 0x3D0, 0x690, 0x4E0, 0x66C}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 130024}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x62A0, 0x50, 0x198, 0x450, 0x3F8}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 130024}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x15018, 0x3D0, 0x690, 0x4E0, 0x668}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 130024}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x62A0, 0x50, 0x198, 0x450, 0x3E8}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 130024}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x15018, 0x3D0, 0x690, 0x4E0, 0x688}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 130024}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x15018, 0x3D0, 0x690, 0x3A0, 0x698}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 130024}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x15018, 0x3D0, 0x690, 0x580, 0x63C}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 130024}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x15018, 0x3D0, 0x690, 0x580, 0x638}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 130024}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x618, 0xE8, 0xD0, 0x4E0, 0x660}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 130024}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x15018, 0x3D0, 0x690, 0x580, 0x658}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 130024}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x4B8, 0x1E8, 0xD0, 0x4E0, 0x620}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 130024}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x4B8, 0x1E8, 0xD0, 0x4E0, 0x644}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 130024}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x4B8, 0x1E8, 0xD0, 0x4E0, 0x684}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 130024}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x220, 0x4D0, 0x40, 0x268, 0x544}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 130024}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x220, 0x4D0, 0x40, 0x260, 0x564}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 130024}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x4B8, 0x1E8, 0xD0, 0x4E0, 0x5F8}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 130024}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x4B8, 0x1E8, 0xD0, 0x4E0, 0x698}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 130024}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x220, 0x4D0, 0x40, 0x270, 0x584}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 130024}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x458, 0x140, 0xD0, 0x268, 0x544}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 130024}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x62A0, 0x510, 0x198, 0x450, 0x40C}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 130024}})

local t = {"libanogs.so", "Cd"}
local tt = {0x5EE78, 0x470, 0x198, 0x450, 0x430}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 130024}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x458, 0x140, 0xD0, 0x260, 0x524}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 130024}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x62A0, 0x510, 0x198, 0x450, 0x3EC}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 130024}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x560, 0x170, 0xD0, 0x260, 0x524}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 130024}})

local t = {"libanogs.so", "Cd"}
local tt = {0x5EE78, 0x470, 0x198, 0x450, 0x3E4}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 130024}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x62A0, 0x510, 0x198, 0x450, 0x24C}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 130024}})

local t = {"libanogs.so:bss", "Cb"}
local tt = {0x62A0, 0x510, 0x198, 0x450, 0x41C}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 130024}})
string.toMusic("修改成功")
gg.toast("修改成功")
...
local tt = {0x66D2E0, 0xB8}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 0}})

local t = {"libUE4.so:bss", "Cb"}
local tt = {0x66D2E8, 0x84}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 0}})

local t = {"libUE4.so:bss", "Cb"}
local tt = {0x66DC08, 0x38}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 0}})

local t = {"libUE4.so:bss", "Cb"}
local tt = {0x66DC10, 0x4}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 0}})
string.toMusic("修改成功")
gg.toast("修改成功")				
...
local tt = {0x66D2E0, 0xB8}
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 4, value = 10000000}})
...
gg.toast("修改成功")
 ...
 switch("静态广角[基址]",
function()
local r = gg.prompt({'自改广角[推荐1.5]'}, {'2.0'}, {'text'})
so=gg.getRangesList('libUE4.so')[1].start
py=0x32b1608
...
switch("动态广角[基址]",
function()
OP=gg.prompt({'自定义超广角 原值=90 可修改范围 90-300'},{[1]='120'},{[1]='number'})--动态广角
...
local ttt = S_Pointer(t, tt, true)
gg.setValues({{address = ttt, flags = 16, value = 120}})--锁帧
string.toMusic("修改成功")
gg.toast("修改成功")
end),
			}, {
...
local UE4 = gg.getRangesList("libUE4.so")
    ...
    local base_pointer = readPointer('libUE4.so', {0x391E0,0x0,0x340,0x30,0x388,0x118}, 1)
    local pointers = {
