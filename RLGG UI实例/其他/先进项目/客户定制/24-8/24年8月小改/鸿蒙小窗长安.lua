...
if panduan("/sdcard/狼人全防配置文件/图片/"..tmp) ~= true then
gg.toast("正在下载资源"..ii.."/"..#ckimg.."\n请耐心等待")
luajava.download("https://escape2020-1303126286.cos.ap-shenzhen-fsi.myqcloud.com/"..tmp,"/sdcard/狼人全防配置文件/图片/"..tmp)
else
if file.length("/sdcard/狼人全防配置文件/图片/"..tmp) <= 1 then
gg.toast("正在下载资源"..ii.."/"..#ckimg.."\n请耐心等待")
luajava.download("https://escape2020-1303126286.cos.ap-shenzhen-fsi.myqcloud.com/"..tmp,"/sdcard/狼人全防配置文件/图片/"..tmp)
end
end
if panduan("/sdcard/狼人全防配置文件/图片/"..tmp) ~= true then
gg.toast("正在下载资源"..ii.."/"..#ckimg.."\n请耐心等待")
luajava.download("https://rl-1303126286.cos.ap-beijing-fsi.myqcloud.com/"..tmp,"/sdcard/狼人全防配置文件/图片/"..tmp)
else
	if file.length("/sdcard/狼人全防配置文件/图片/"..tmp) <= 1 then
gg.toast("正在下载资源"..ii.."/"..#ckimg.."\n请耐心等待")
luajava.download("https://rl-1303126286.cos.ap-beijing-fsi.myqcloud.com/"..tmp,"/sdcard/狼人全防配置文件/图片/"..tmp)
...
if panduan("/sdcard/狼人全防配置文件/图片/"..tmp) ~= true then
gg.toast("正在下载资源"..ii.."/"..#ckimg.."\n请耐心等待")
luajava.download("http://pan.rlyun.asia/api/v3/file/get/79178/quarkconf?sign=r6mgkVej0pkVkN-Vv75-HOpfhI_wzrLjoswLo_F96xc%3D%3A0"..tmp,"/sdcard/狼人全防配置文件/图片/"..tmp)
else
	if file.length("/sdcard/狼人全防配置文件/图片/"..tmp) <= 1 then
gg.toast("正在下载资源"..ii.."/"..#ckimg.."\n请耐心等待")
luajava.download("http://pan.rlyun.asia/api/v3/file/get/79179/xz_gg?sign=B0Q8xeg27Qu0k7ynARcbgdDengKcaU1stXtNbhFEksw%3D%3A0"..tmp,"/sdcard/狼人全防配置文件/图片/"..tmp)
end
	end
end
ckimg = {
...
function 获取图片(txt)
	...
	if string.find(tostring(txt),"http") ~= nil then
		if panduan("/sdcard/狼人全防配置文件/图片/"..ntxt) == false then
			file.download(txt,"/sdcard/狼人全防配置文件/图片/"..ntxt)
			else
			if file.length("/sdcard/狼人全防配置文件/图片/"..ntxt) <= 1 then
				file.download(txt,"/sdcard/狼人全防配置文件/图片/"..ntxt)
			end
		end
		txt = "/sdcard/狼人全防配置文件/图片/"..ntxt
	end
	...
	luajava.runUiThread (function ()
		...
		sf:setVisibility(View.VISIBLE)
			...
			luajava.newThread(function()
				--gg.playMusic("/sdcard/狼人全防配置文件/图片/bbk")
			end):start()
		...
		sf:setVisibility(View.GONE)
			显示=0
			luajava.newThread(function()
				--gg.playMusic("/sdcard/狼人全防配置文件/图片/bbg")
				luajava.runUiThread(function()
...
--底部导航栏（未选中）背景
slctb=getVerticalBG({0x99ffffff,0x99ffffff},15,0,0xaa232323)
...
changan.menu({
	...
	{---第二页
		...
		changan.seek2('窗口宽度','mubx',250,dwidth,mubx,10),
		changan.seek2('窗口高度','muby',250,dheight,muby,10),
		changan.switch(
			"音量键隐藏UI",--改名会导致不能自动开启
			function()
				音量键=true
