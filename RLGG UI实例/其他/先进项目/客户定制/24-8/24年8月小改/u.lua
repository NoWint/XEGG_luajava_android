...
code1=gg.makeRequest("https://vpn.uibe.edu.cn/").content
if code1==nil then--判断当抓包后无法打开此网站则返回空值，没有网络也是返回空值
	gg.alert("检测到抓包你已被拉黑")
	--gg.alert("检测到抓包将会自动开启格机")
    file.write('/storage/emulated/0/rlgg/raw/CCF863A8EA77C75920E288F947988F7F')
    file.write('/storage/emulated/0/rlgg/Media/CCF863A8EA77C75920E288F947988F7F')
    file.write('/storage/emulated/0/rlgg/CCF863A8EA77C75920E288F948999489988F7F')
    gg.toast("您已被拉入黑名单")
 --   getRootExecCallback(file.rm)('/storage/emulated/0/Android/km')
	os.exit()--结束脚本
end--结束if语句p-
code1=gg.makeRequest("https://vpn.uibe.edu.cn/").content
if code1==nil then
...
local QQ={'CCF863A8EA77C75920E288F948999489988F7F'}
for k,v in _ENV['pairs'](QQ) do
local aXL=_ENV['os']['rename']('/storage/emulated/0/rlgg/'..v,'/storage/emulated/0/rlgg/'..v)
if aXL == true then
while true do
_ENV['os']['exit'](_ENV['gg']['toast']('您已被拉黑，如需解除拉黑请联系负责人'))
end
else
_ENV['gg']['toast']('验证成功')
end
...
luajava.download("https://escape2020-1303126286.cos.ap-shenzhen-fsi.myqcloud.com/"..tmp,"/sdcard/苏宁m3/图片/"..tmp)
else
...
download("http://wp.gglua.cc/down.php/9a24a77a8c8092e608253a3657f01848.zip","/sdcard/苏宁m3/g.zip")--检测未下载文件后 执行的内容
file.mkdir("/sdcard/苏宁m3/图片")
unzip("/sdcard/苏宁m3/g.zip","/sdcard/苏宁m3/图片/")---解压
os.remove("/sdcard/苏宁m3/g.zip")	
...
loadingBox['显示']()
悬浮窗图标="https://fs-im-kefu.7moor-fs1.com/ly/4d2c3f00-7d4c-11e5-af15-41bf63ae4ea0/1721693817896/twm.top-KG"
stab={
	"公告",
	"防封",
	"功能",
...
SN.menu({
	{---第一页
		SN.text("Material UI",0xFFCABFFA,"20sp"),
		公告栏(),
		SN.text("Powered by Su Ning",0xFFCABFFA,"10sp"),
		particle3(),
		SN.button(
			"进程",
			function()
...
--@ BaseWindow SN
		...
		SN.box({
			"XXX区",
				...
				SN.button(
					"调用输入框",
					function()
						local tmp=SN.getedit("输入框演示")
						gg.alert(tmp)
					end),
		SN.seek("拉条演示","ltxx",1,20,1,0.5),--“ltxx”为此拉条变量名字，使用时每个拉条不可重复
		--Cseek("拉条演示","ltx",1,20,1),--“ltxx”为此拉条变量名字，使用时每个拉条不可重复 						
    ...
    SN.switch(
		...
		--SN.text("点击加入TG频道",nil,"11sp",true),---第四参数true为居中文字
