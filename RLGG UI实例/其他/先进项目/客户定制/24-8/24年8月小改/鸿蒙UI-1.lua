...
local layout = {
	...
	slcta[i]=getVerticalBG({0xff6750A4,0xff6750A4},10,0,0xaa242937)
		slctb[i]=getVerticalBG({0x99ffffff,0x99ffffff},10,0,0xaa232323)
...
local ui = require('ui')
	...
	luajava.setInterface(ViewPager, 'addOnPageChangeListener', 
		...
		for i=1,#stab do
			_ENV["jm"..i .."t"]:setTextColor(0xff1C1C1C)
...
_ENV["jm"..view+1 .."t"]:setBackground(slcta[view+1])	
	...
	ckou={FrameLayout,
		...
		background=newbg({0xffF0F2F4,0xffF0F2F4},0,0xffffffff,0,0,35,35),
		{
		LinearLayout,
		layout_height="5dp",
		layout_width="match_parent",
		...
		layout_height="match_parent",
		layout_width="match_parent",
		background=newbg({0xffF0F2F4,0xffF0F2F4},0,0xffffffff,0,0,0,0),
		}
		},
		{
		LinearLayout,
...
local Action = event:getAction()
if Action == MotionEvent.ACTION_DOWN then
luajava.getIdValue("ckbg"):setBackground(newbg({0xffF0F2F4,0xffF0F2F4},5,控件颜色,0,0,35,35))
isMove = false
RawX = event:getRawX()
RawY = event:getRawY()
...
elseif Action == MotionEvent.ACTION_UP then
luajava.getIdValue("ckbg"):setBackground(newbg({0xffF0F2F4,0xffF0F2F4},0,0xffffffff,0,0,35,35))
elseif Action == MotionEvent.ACTION_MOVE then
isMove = true
...
if not txtc then txtc="#212121" end
...
luajava.getIdValue(_ENV [name]):setHintTextColor(0xAA000000)
return {["view"] = rest,
	["name"] = name,
	["type"] = "输入框",
}
		...
		changan.card({
					{
						...
						"功能介绍xxx",
						function()
	Snackbar:make(floatWindow,"开启成功",Snackbar.LENGTH_SHORT)
:show()
						end,
						function()
							gg.alert("关闭")
...
changan.menu({
	{---第一页
		changan.text("鸿蒙 UI",控件颜色,"15sp"),
		changan.text("By：云汐",0xff34E58D,"15sp"),
		changan.text("深圳×32",0xffD174D3,"15sp"),
		changan.text("QQ群:2194632244",0xff95C9E9,"15sp"),
		changan.text("这世界破破烂烂,但总有人缝缝补补",0xff335032,"15sp"),
		changan.button(
			"进程",
			function()
				gg.setProcessX()
...
changan.button("打开窗口",
		...
		打开窗口("窗口示例")
					...
					"Snackbar[ 加载开 ]" ,
					function ()
Snackbar:make(topbar,"开启成功",Snackbar.LENGTH_SHORT):show()
					end
				} , {
					"防闪[ 加载开 ]" ,
					function ()
	...
	changan.seek("拉条例子","ltlz",1,10,1),
	changan.edit("输入框"),
		changan.switch(
			"Snackbar",
			function()
				Snackbar:make(ckou,"开启成功",Snackbar.LENGTH_SHORT)
:show()
			end,
			function()
