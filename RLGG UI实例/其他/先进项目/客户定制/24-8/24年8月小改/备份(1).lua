...
local FontPath = "/sdcard/云烛/卷轴/long.ttf"--字体路径
...
function changan.seek2(name,bian,smin,smax,nows,size)
_ENV[bian] =nows
if _ENV[bian] == nil then _ENV[bian] = 1.0 end
...
function changan.seek3(name,bian,smin,smax,nows,size)
_ENV[bian] =nows
if _ENV[bian] == nil then _ENV[bian] = 1.0 end
if size == nil then size = 1 end
if not name then name = "未设置" end
...
创建窗口("小组测试",--窗口名字
	{
changan.seek2("拉条演示","ltxx",1,20,1,0.5),--“ltxx”为此拉条变量名字，使用时每个拉条不可重复
changan.switch(
			"功能",
			function()
...
changan.menu({
	{---第一页
		...
		changan.seek2("拉条演示","ltxxd",1,20,1,0.5),--“ltxx”为此拉条变量名字，使用时每个拉条不可重复
		changan.button(
			"进程",
			function()
				gg.setProcessX()
