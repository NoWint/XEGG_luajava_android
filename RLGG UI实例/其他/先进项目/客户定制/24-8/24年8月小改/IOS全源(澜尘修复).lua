...
if panduan("/sdcard/JR项目/图片/"..tmp) ~= true then
gg.toast("正在加载资源"..tmp)
download("https://escape2020-1303126286.cos.ap-shenzhen-fsi.myqcloud.com/"..tmp,"/sdcard/JR项目/图片/"..tmp)
end
end
function 获取图片(txt)
ntxt=string.sub(string.gsub(txt,"/","."),-10,-1)
if string.find(tostring(txt),"http")~=nil then
    if panduan("/sdcard/JR项目/图片/"..ntxt)==false then
    download(txt,"/sdcard/JR项目/图片/"..ntxt)
    end
    txt="/sdcard/JR项目/图片/"..ntxt
end
...
JR.controlFlip = function(control,time)
...
ttview:setBackground(luajava.getBitmapDrawable("/sdcard/JR项目/图片/.com.xflog"))
else
tview:setVisibility(View.GONE)
ttview:setBackground(luajava.getBitmapDrawable("/sdcard/JR项目/图片/xflog"))
end
end
function JR.box2(views,boxi)
local tid = "box"..guid()
local ttid = tid.."678"
firadio = {
	LinearLayout,
...
firadio[#firadio+1] = {
	...
	background = getSelector3(),
	{
		...
		background = "/sdcard/JR项目/图片/.com.xflog",
		layout_width = "20dp",
		layout_height = "20dp",
	...
	gg.alert("JR.box第一个参数必须是string") os.exit()
end
...
chazhi={} chajv={}
function JR.seek(name,bian,smin,smax,nows)
_ENV[bian] =nows
thum=getShape3()
thum:setSize(80, 80)
...
function JR.switch(name,func1,func2,yans)
nid = name..guid()
...
function JR.button(txt,func,yans)
if not yans then yans="#00A4FF" end
...
function JR.text(txt,color,size)
...
xfcpic = "http://zzb.vsc.icu/view.php/70b735139755e79c740b24684ae8dae9.png"
--↑悬浮窗链接或路径↑
...
--JR.text   文字
--JR.seek  拖动条
--JR.edit  输入框（JR.getedit和JR.setedit调用
...
JR.menu(
	{
		{----1
			--JR.text("欢迎使用JRUI","#008EFF","20sp"),
			JR.box2({--如果不需要请删除box2，注意括号的对应。如果需要请注意用box2套box，不然会很丑
				"套娃",
				JR.box({--box开头
					"",--box名字
					...
					JR.edit("输入演示"),--括号填的“输入演示”就是当前输入框的ID
					
					JR.button("调用输入框",
						function()
							tmp=JR.getedit("输入演示")--传入ID调用内容，返回string类型
							gg.alert(tmp)
						end),
					
					JR.button("设置输入框",
						function()
							JR.setedit("输入演示","修改了输入框内容")--传入ID和要改成的内容
						end),
			...
			JR.box({--box开头
				...
				JR.seek("拉条演示","ltys",1,10,6),--名字，变量名字，最小值，最大值，初始值
