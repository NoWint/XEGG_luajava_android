...
function pdcf(lujing) rec = "/storage/emulated/0/NR/配置文件/"..lujing fille,err = io.open(rec) if fille == nil then return false else return true end end
sleep = gg.sleep
if gg.isHTTPdump()==true and panduan("/storage/emulated/0/LNR/配置文件/vpn.txt") == false then gg.alert("\n抓包？") os.exit() end
...
write("/storage/emulated/0/NR/配置文件/"..lujing,neirong)
end
function rdcf(lujing)
return read("/storage/emulated/0/NR/配置文件/"..lujing)
end
function checkimg(tmp)

if panduan("/sdcard/NR/图片/"..tmp) ~= true then
gg.toast("正在下载资源"..tmp.."\n请耐心等待")
download("https://wf980627-512739054-1317447113.cos.ap-nanjing.myqcloud.com/"..tmp,"/sdcard/NR/图片/"..tmp)
end
end
ckimg = {
    "baizw",
...
function read(fileName) f = assert(io.open(fileName, 'r')) content = f:read("*all") f:close() return content end
function wtcf(lujing,neirong)
write("/sdcard/NR/配置文件/"..lujing,neirong)
end
function rdcf(lujing)
return read("/sdcard/NR/配置文件/"..lujing)
end
...
tmp1 = 0
if audiokg == "开" then
if panduan("/sdcard/NR/音频/"..ress) == true then
pmusic("/sdcard/NR/音频/"..ress)
tmp1 = 1
else
	download("https://wf980627-512739054-1317447113.cos.ap-nanjing.myqcloud.com/"..ress ,"/sdcard/NR/音频/"..ress)
pmusic("/sdcard/NR/音频/"..ress)
tmp1 = 1
end
end
...
wanfeng.menu = function(stext,sview)
if isswitch then
return false
end
isswitch = true
...
function 隐藏()
luajava.runUiThread(function()
control:setBackground(luajava.getBitmapDrawable("/sdcard/NR/图片/imgui"))
...
chazhi = {} chajv = {}
function wanfeng.seek(name,bian,smin,smax,nows)
...
paramt = {}
titletable={}
function wanfeng.newtitle(name)
titletable[name]=1
...
function wanfeng.rmvtitle(name)
titletable[name]=0
local function invoke2()
...
layout_marginTop = "5dp",
	...
	gg.alert("wanfeng.box第一个参数必须是string") os.exit()
end
radios = {
	LinearLayout,
	layout_marginLeft = "0dp",
...
function wanfeng.switch(name,func1,func2)
...
radios[#radios+1] = {
	...
	}),
	{
		...
		layout_width = "20dp",
		layout_height = "20dp",
		background = "/sdcard/NR/图片/"..radio[i][2]
	},
	{
		TextView,
		text = radio[i][1],
...
function wanfeng.button(txt,func)
if not txt then txt = "NR" end
...
function wanfeng.text(txt,color,size)
...
namers = _ENV[name]
if namers ~= "开" then
luajava.getIdValue(nid):setBackground(luajava.getBitmapDrawable("/sdcard/NR/图片/checkon2"))
_ENV[name] = "开"
pcall(func1)
else
luajava.getIdValue(nid):setBackground(luajava.getBitmapDrawable("/sdcard/NR/图片/checkof"))
_ENV[name] = "关"
pcall(func2)
end
...
function wanfeng.check2(cklist)
rest = {
	LinearLayout,
	layout_width = 'match_parent',
...
rstt = luajava.loadlayout({
	...
	layout_marginRight = "10dp",
	gravity = "center_vertical",
	background = "/sdcard/NR/图片/checkof",
	onClick = function() luajava.newThread(function() func() end):start() end,
	{
		TextView,
		gravity = "top",
...
param1 = {}
floattable={}
function wanfeng.newfloat(name,func1,func2)
floattable[name]=1
...
function wanfeng.rmvfloat(name)
floattable[name]=0
local function invoke2()
...
function wanfeng.getedit(name)
--gg.alert(edit)
edit = tostring(luajava.getIdValue(_ENV[name]):getText())
return edit
end
function wanfeng.setedit(name,txt)
txt = tostring(txt)
luajava.runUiThread(function()
...
if namers ~= "开" then
luajava.runUiThread(function()
	luajava.getIdValue(nid):setBackground(luajava.getBitmapDrawable("/sdcard/NR/图片/check2"))
	end)
_ENV[name] = "开"
func1()
else
	luajava.runUiThread(function()
	luajava.getIdValue(nid):setBackground(luajava.getBitmapDrawable("/sdcard/NR/图片/check1"))
	end)
_ENV[name] = "关"
func2()
end
...
ttview:setBackground(luajava.getBitmapDrawable("/sdcard/NR/图片/hsj"))
else
	tview:setVisibility(View.GONE)
ttview:setBackground(luajava.getBitmapDrawable("/sdcard/NR/图片/sj"))
end
end
...
if panduan("/sdcard/NR/音频/"..gqmz..".mp3")==true then
gg.playMusic("/sdcard/NR/音频/"..gqmz..".mp3")
else
gg.toast("下载中..."..gqmz)
...
if musurl=="" or musurl=="null" then gg.alert("http://music.163.com/api/search/get?s="..name.."&type=1&offset=0&total=true&limit=") else
 download(musurl,"/sdcard/NR/音频/"..gqmz..".mp3")
 gg.playMusic("/sdcard/NR/音频/"..gqmz..".mp3")
gg.sleep(1000)
end
end
...
{
	{
		...
		--wanfeng.switch 开关
		--wanfeng.check2 图片开关
		--wanfeng.newfloat 生成悬浮按钮
		--wanfeng.rmvfloat 删除悬浮按扭
    --
	--gg.toast("开启成功")
	--toMusic('开启成功')
    --gg.toast("开启成功")
		wanfeng.text("封帝公告区域","#FF2A3F","20sp"),
		wanfeng.button("选择进程",
		function()
			gg.setProcessX()
		end,
