...
BxCb =io.open('/sdcard/Yunzhu/图片/地铁跑酷','r') 
 if BxCb == nil then 
download("https://vip.kyun.top/down.php/9e74f67f1d702300a1d637bb1fc17e95.heif", "/sdcard/Yunzhu/图片/地铁跑酷") 
 
else 

--不用添加
end 
BxCb =io.open('/sdcard/Yunzhu/图片/lixin','r') 
 if BxCb == nil then 
download("https://vip.kyun.top/down.php/6d1b97f7db0639b18223b440a9c6d924.", "/sdcard/Yunzhu/图片/lixin") 
...
BxCb =io.open('/sdcard/Yunzhu/图片/图片','r') 
 if BxCb == nil then 
download("https://vip.kyun.top/view.php/85471933490f1cb518781e16c1b45119.jpg", "/sdcard/Yunzhu/图片/图片") 
 
else 


BxCb =io.open('/sdcard/Yunzhu/图片/ret','r') 
 if BxCb == nil then 
download("https://vip.kyun.top/view.php/066cd029c96c7eb6971128f77e1554b9.png", "/sdcard/Yunzhu/图片/ret") 
...
BxCb =io.open('/sdcard/Yunzhu/图片/大背景','r') 
 if BxCb == nil then 
download("https://vip.kyun.top/view.php/da953ec40d27c41e128364f6647e628a.jpg", "/sdcard/Yunzhu/图片/大背景") 
...
return 获取图片2("/sdcard/Yunzhu/图片/"..x)
end
...
span::before{
    ...
    width: 100px;
    height: 3px;
    background: linear-gradient(90deg,#9393FF,transparent);
}
...
audiotype = {
	AudioManager.STREAM_ALYunzhu, --手机闹铃的声音
...
selector:addState({
	-android.R.attr.state_pressed
}, getVerticalBG({0x00ffffff,0x00ffffff},20,2,预制颜色))
return selector
end
...
topbar = luajava.loadlayout({
	...
	--	background=topSelect(),
		...
		layout_width="60dp",
		id='tops',
		background=getVerticalBG({预制颜色,预制颜色},20)
...
floatWindow = {
	...
	elevation='2dp',
		...
		background=newbg({0xff2E2E2E,0xff2E2E2E},0,0xffff0000,10,10,10,10),
		topbar,
		...
		layout_marginLeft='3dp',
				layout_marginRight='3dp',
		background=getVerticalBG({0xaa161616,0xaa161616},10,0,0xffffffff),
		{LinearLayout,
			layout_weight=1,
				layout_height='34dp',
				layout_marginTop='3dp',
	...
	{LinearLayout,
		...
		layout_marginTop='5dp',
				...
				layout_marginLeft='3dp',
				layout_marginRight='3dp',
		background=newbg({0x55181818,0x55181818},4,0xff181818,10,10,10,10),
				stit
			},
	tiaose(),
		ckou,
...
luajava.getIdView("backv"):setColorFilter(预制颜色)
luajava.getIdView("suov"):setColorFilter(预制颜色)
luajava.getIdView("tosearch"):setHintTextColor(预制颜色)
...
luajava.runUiThread(function()
if type(ret)~="table" then log_init() return 0 end
	luajava.getIdValue(nid.."t"):setTextColor(预制颜色)
	luajava.getIdValue(nid.."p"):setColorFilter(预制颜色)
...
boxpic[tid]:setColorFilter(预制颜色)
colorvs[tid]={true,"box"}
else
	tview : setVisibility (View.GONE)
Yunzhu.controlWater (_ENV [tid.."7"] , 200)
...
if not radoff then
radon=getVerticalBG({0xffffffff,0xffffffff},360,20,预制颜色)
...
luajava.runUiThread(function()
if type(ret)~="table" then log_init() return 0 end
--_ENV["jm"..当前ui.."t"]:setTextColor(预制颜色)
luajava.getIdView("backv"):setColorFilter(预制颜色)
luajava.getIdView("suov"):setColorFilter(预制颜色)
tops:setBackground(getVerticalBG({预制颜色,预制颜色},30))
checkbg = getShape2(45,{预制颜色,预制颜色},4,预制颜色)
radon=getVerticalBG({0xffffffff,0xffffffff},360,20,预制颜色)
luajava.getIdView("tosearch"):setHintTextColor(预制颜色)
--uiadtext('已刷新预制颜色',预制颜色)
musici:setColorFilter(预制颜色)
for k,v in pairs(colorvs) do
	if v[1]==true then
		if v[2]=="switch" then
			luajava.getIdValue(k):setBackground(checkbg)
		elseif v[2]=="box" then
			boxpic[k]:setColorFilter(预制颜色)
		elseif v[2]=="txt" then
			luajava.getIdValue(k):setTextColor(预制颜色)
		elseif v[2]=="img" then
			luajava.getIdValue(k):setColorFilter(预制颜色)
		elseif v[2]=="rad" then
			luajava.getIdValue(k):setBackground(radon)
		elseif v[2]=="seek" then
			luajava.getIdView(k):setProgressDrawable( getVerticalBG({预制颜色,预制颜色},45))
		end
	end
end
end)
...
rest = luajava.loadlayout({
	...
	layout_hight = "fill_parent",
	{
		...
		},
		{
			...
			--paddingStart="0dp",
			--paddingEnd="0dp",
			progressDrawable={ getVerticalBG({预制颜色,预制颜色},45)},
			thumb=thum,
			progressHeight="10dp",
			onSeekBarChange = {
...
function bofang(gqmz, gdmn)
	...
	uiadtext("正在播放 </font><font color='#0062ff'>"..gqmz,"#ffffff")
...
wyylog = luajava.loadlayout({
	...
	layout_width = "match_parent",
	{
		...
		layout_width = "match_parent",
		{
			...
			gravity="center",
			orientation="vertical",
			Yunzhu.text('请在上方搜索音乐关键词',0xff245AFF)
			},
...
while true do
	...
	if tostring(gg.getTargetPackage()) == appstart then
		loadBox['关闭']()
		uiadtext(name.." </font><font color='#0062ff'>启动成功.","#ffffff")
		gg.alert("启动成功")
	else
		jci=jci+1
		if jci==14 then
			loadBox['关闭']()
			uiadtext(name.." </font><font color='#0062ff'>启动失败.","#ffffff")
			gg.alert("自动获取进程失败\n请手动选择游戏进程")
			gg.setProcessX()
			break
		end
...
tmp = luajava.loadlayout(
	{
		...
		},
		{
			LinearLayout,
			onClick = function()
			luajava.newThread(function() if type(ret)~="table" then log_init() return 0 end uiadtext(name.." </font><font color='#0062ff'>正在启动.","#ffffff")  func() end):start() end,
...
Yunzhu.controlFlip = function(control,time)---翻转
luajava.runUiThread(function()
if type(ret)~="table" then log_init() return 0 end
	import "android.view.animation.Animation"
	import "android.animation.ObjectAnimator"
...
function shenglongyp()
	...
	Yunzhu.controlFlip(shenglong,1900)
end
...
luajava.newThread(function()
	...
	vv = luajava.loadlayout({
		...
		orientation = "vertical",
		visibility = "gone",
		background = "/sdcard/Yunzhu/图片/大背景",
		layout_width = "match_parent",
		layout_height = "match_parent",
		gravity = "center",
		onTouch=hanshu,
		...
		tmp,
		{
			...
			
			{
				...
				layout_height = "60dp",
				{
					...
					elevation = "10dp",
					gravity = "center",
					background = getHorizontalBG({预制颜色,0xffffffff},25,2,0xff000000),
					layout_height = "50dp",
					layout_width = "160dp",
					layout_marginBottom = "20dp",
				}}, {
				TextView,
				background = getVerticalBG({预制颜色,预制颜色},15),
				text = "登录",
				textColor = "#ffffff",
				textSize = "16sp",
				gravity = "center",
...
ktmp = "kam='"..ret.kami.."'"
luajava.newThread(function()
	write("/sdcard/Yunzhu/配置文件/km.lua",ktmp)
end):start()
切换(1)
...
卡网="https://www.aidoumi.net/links/99AC5932"

悬浮窗图标="/sdcard/Yunzhu/图片/图片"
