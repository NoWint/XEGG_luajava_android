...
function getVerticalBG(gtvb1,gtvb3,gtvb4,gtvb5)
  if not gtvb4 then gtvb4=0 gtvb5=0xffE6E0E9 end
...
mubx=dp2px(UI高度)
  ...
  floatWindow=luajava.loadlayout{
    ...
    {
      MaterialCardView,
      background = getHorizontalBG({0xff141218,0xff141218},40,3,0xff49454F),
      layout_width="match_parent",
  ...
  Downprogress:setIndicatorColor({0xffffffff})
...
<body>
    <div class="chat-container">
        ...
        <div id="chat-box" class="chat-box">
            <div class="message bot-message">
                <div class="message-content">
                    您好！我是Kimi AI助手，可以随时帮助您解决问题。如果您需要清空聊天记录，只需输入"清空聊天记录"即可。
                </div>
        ...
        <div class="input-container">
            <input type="text" id="user-input" placeholder="输入你的消息..." class="input-text">
    ...
    <script>
        // API接口逻辑
        const API_KEY = 'sk-87JV5EmfDM8K5Xul8wacPEiFkcCDaMmmb57Qfvxp6orksTRK';
        ...
        async function getAIResponse(prompt) {
            ...
            try {
                const response = await fetch('https://api.moonshot.cn/v1/chat/completions', {
                    ...
                    body: JSON.stringify({
                        ...
                        messages: [
                            {
                                role: "system",
                                content: "你是Kimi（Moonshot AI）开发的人工智能助手，擅长通过搜索来回答问题。"
                            },
                        ...
                        if (line.startsWith('data: ')) {
                            try {
                                ...
                                if (data.choices[0].delta.content) {
                                    ...
                                    messageContent.innerHTML = assistantMessage.replace(/\n/g, '<br>');
                ...
                const aiMessage = document.createElement('div');
                aiMessage.className = 'message bot-message';
                aiMessage.innerHTML = `<div class="message-content">⚠️ 请求失败: ${error.message}</div>`;
                chatBox.appendChild(aiMessage);
                chatBox.scrollTop = chatBox.scrollHeight;
            }
        }
        ...
        function sendMessage() {
            ...
            if (message === "清空聊天记录") {
                ...
                initialMessage.innerHTML = '<div class="message-content">聊天记录已清空！有什么可以帮您？</div>';
                const copyBtn = document.createElement('button');
                copyBtn.className = 'copy-btn';
                copyBtn.textContent = '复制';
                initialMessage.appendChild(copyBtn);
                
                // 为复制按钮添加事件监听
                copyBtn.addEventListener('click', function() {
                    navigator.clipboard.writeText(initialMessage.querySelector('.message-content').textContent).then(() => {
                        showToast('已复制到剪贴板');
                    }).catch(err => {
                        showToast('复制失败', true);
                    });
...
return webView
  ...
  function seeColor()
    luajava.startThread(function()
      luajava.runUiThread(function()
        ...
        luajava.getIdView("tab2"):setTabTextColors(0xffE6E0E9,"0xff"..RGBcolor)
  ...
  function 修改控件颜色(color)
    luajava.runOnUiThread(function()
      tab:setTabTextColors(0xffE6E0E9,"0xff"..RGBcolor)
...
thread=luajava.getThread(function()
  ...
  SearchProgress:setIndicatorColor({0xffffffff,0xFFF44336, 0xFFFF9800, 0xFFFFEB3B, 0xFF4CAF50, 0xFF00BCD4, 0xFF2196F3, 0xFF673AB7, 0xFFE91E63})
--  SearchProgress:setTrackColor(0x16161616)  
  ...
  menuView= luajava.loadlayout {
    FrameLayout,
    background={GradientDrawable,color=0xFF211F26,cornerRadius=20},
    layout_width="match_parent",
    layout_height="match_parent",
    {
      LinearLayout,
        ...
        layout_weight=1,
          {
            ...
            elevation="3dp",
              background = getHorizontalBG({0xff141218,0xff141218},40,3,0xffCAC4D0),
           -- layout_height = "77dp",
            LabelVisibilityMode = 0,  --1 是否显示标签	
            background = getVerticalBG({0xff141218, 0xff141218}, 10),
            ...
            --[[
            for i=1,#stab do
            local item = menu:add(0, i, i, stab[i] ) -- 添加菜单项
    ...
    background={GradientDrawable,color=0xFF141218,cornerRadius=20},
    elevation="5dp",
...
"守护进程标题",
		...
		Yunzhu.texta("   设定 Settings  ",0xffffffff,"19sp"),
		Yunzhu.texta("   Powered by Yunzhu",0xfff4f4f4,"12sp"),	
		}),
		Yunzhu.quyu({
"守护进程信息",		
		Yunzhu.texta("   Context: Service",0xfff4f4f4,"12sp"),
		Yunzhu.texta("   使用通知： : 否",0xfff4f4f4,"12sp"),
		Yunzhu.texta("   运行守护： : 否",0xfff4f4f4,"12sp"),
		Yunzhu.texta("   在虚拟空间中使用root：：否",0xfff4f4f4,"12sp"),		
		--Yunzhu.text("   -----------------------------------------",0xfff4f4f4,"12sp"),
		}),
		Yunzhu.quyu({
		"",
		Yunzhu.button2(
					...
					Yunzhu.seeka("拉条演示","ltxxa",1,50,1),--“ltxx”为此拉条变量名字，使用时每个拉条不可重复
					}),
					
		--以上是一个面板集合的实例，用这种布局方式会好看很多
  ...
  {LinearLayout,
        ...
        gravity  = "right|center",
                layout_marginBottom="300dp",
        padding = "2dp",
        onTouch=hanshu,
        {LinearLayout,
        Yunzhu.lddboxBT({
	"  布局编辑 ",
	--Yunzhu.text("  "..项目名称,"0xff333333","13sp",true),
	--Yunzhu.text("  静谧内部绕过","0xffffffff","10sp"),
	
	Yunzhu.seek("视图高度（layout_height）","lt",1,100,1),--名字，变量名字，最小，最大，初始
	Yunzhu.seek("视图宽度（layout_width）","lt",1,100,1),--名字，变量名字，最小，最大，初始
...
local UIEditor = {
    ...
    _CONTROLS = {
        layouts = {
            -- 基础布局
            {type="LinearLayout",    name="线性布局",   icon="⬅", 
             properties={
                 orientation={type="choice", options={"vertical","horizontal"}, default="vertical"},
                 gravity={type="multi", options={"left","center","right","top","bottom"}, default="left|top"},
                 weightSum={type="number", default=1, min=0, max=10}
             }},
            {type="FrameLayout",     name="帧布局",     icon="🖼", 
             properties={
                 foregroundGravity={type="choice", options={"left","center","right"}, default="center"},
                 measureAllChildren={type="boolean", default=false}
             }},
            {type="RelativeLayout",  name="相对布局",   icon="🔗",
             properties={
                 gravity={type="multi", options={"left","center","right","top","bottom"}, default="left|top"},
                 ignoreGravity={type="boolean", default=false}
             }},
            {type="ScrollView",      name="滚动布局",   icon="📜",
             properties={
                 fillViewport={type="boolean", default=false},
                 scrollbarStyle={type="choice", options={"insideOverlay","insideInset","outsideOverlay","outsideInset"}, default="insideOverlay"}
             }},
            {type="GridLayout",      name="网格布局",   icon="🔳",
             properties={
                 rowCount={type="number", default=2, min=1, max=10},
                 columnCount={type="number", default=2, min=1, max=10},
                 alignmentMode={type="choice", options={"alignBounds","alignMargins"}, default="alignBounds"}
             }},
            
            -- 高级布局
            {type="DrawerLayout",    name="抽屉布局",   icon="🗄️",
             properties={
                 drawerLockMode={type="choice", options={"unlocked","lockedClosed","lockedOpen"}, default="unlocked"},
                 drawerShadow={type="color", default="#80000000"}
             }},
            {type="CoordinatorLayout", name="协调布局", icon="🔄",
             properties={
                 statusBarBackground={type="color", default="#00000000"},
                 keylines={type="text", default="300dp,500dp"}
             }}
        },
        
        widgets = {
            -- 基础控件
            {type="TextView",        name="文本视图",   icon="📄",
             properties={
                 text={type="text", default="文本内容"},
                 textSize={type="number", default=14, min=8, max=72},
                 textColor={type="color", default="#FFFFFFFF"},
                 gravity={type="multi", options={"left","center","right","top","bottom"}, default="left|top"}
             }},
            {type="Button",          name="按钮",       icon="🆗",
             properties={
                 text={type="text", default="按钮文本"},
                 textColor={type="color", default="#FFFFFFFF"},
                 onClick={type="function", default="function() gg.toast('点击事件') end"},
                 background={type="color", default="#FF2196F3"}
             }},
            {type="EditText",        name="输入框",     icon="📝",
             properties={
                 hint={type="text", default="输入提示"},
                 inputType={type="choice", options={"text","number","phone","password"}, default="text"},
                 textSize={type="number", default=14, min=8, max=72}
             }},
            ...
            {type="CheckBox",        name="复选框",     icon="☑️",
             properties={
                 ...
                 text={type="text", default="选项文本"}
             }},
            {type="RadioButton",     name="单选按钮",   icon="🔘",
             properties={
                 checked={type="boolean", default=false},
                 text={type="text", default="单选选项"}
             }},
            {type="Switch",          name="开关",       icon="🔧",
             properties={
                 checked={type="boolean", default=false},
                 textOn={type="text", default="开"},
                 textOff={type="text", default="关"}
             }},
            ...
            {type="ProgressBar",     name="进度条",     icon="📊",
             properties={
                 ...
                 style={type="choice", options={"horizontal","circular"}, default="horizontal"}
             }},
            {type="SeekBar",         name="拖动条",     icon="🎚️",
             properties={
                 progress={type="number", default=50, min=0, max=100},
                 thumb={type="color", default="#FFFFFFFF"}
             }},
            
            -- 高级组件
            {type="RecyclerView",    name="回收视图",   icon="♻️",
             properties={
                 layoutManager={type="choice", options={"linear","grid","staggered"}, default="linear"},
                 itemCount={type="number", default=10, min=1, max=100}
             }},
            {type="ViewPager",       name="视图翻页",   icon="📖",
             properties={
                 offscreenPageLimit={type="number", default=1, min=1, max=5},
                 pageMargin={type="number", default=0, min=0, max=100}
             }},
            {type="WebView",         name="网页视图",   icon="🌐",
             properties={
            ...
            -- 导航组件
            {type="BottomNavigationView", name="底部导航", icon="⬇️",
             properties={
                 menu={type="items", default={"首页","分类","我的"}},
                 itemIconTint={type="color", default="#FFFFFFFF"},
                 itemTextColor={type="color", default="#FFFFFFFF"}
             }},
            {type="Toolbar",         name="工具栏",     icon="🛠️",
             properties={
                 title={type="text", default="应用标题"},
                 titleTextColor={type="color", default="#FFFFFFFF"},
                 background={type="color", default="#FF2196F3"}
             }}
        },
        
        dialogs = {
            {type="AlertDialog",     name="提示对话框", icon="💬",
             properties={
                 title={type="text", default="提示标题"},
                 message={type="text", default="对话框内容"},
                 positiveButton={type="text", default="确定"},
                 negativeButton={type="text", default="取消"}
             }},
            {type="BottomSheetDialog", name="底部弹窗", icon="📥",
             properties={
                 peekHeight={type="number", default=200, min=100, max=1000},
                 halfExpandedRatio={type="number", default=0.5, min=0.1, max=1}
             }}
        }
    },
    
    _ATTRIBUTES = {
        size = {"layout_width", "layout_height", "minWidth", "minHeight"},
        margin = {"layout_margin", "layout_marginLeft", "layout_marginTop", "layout_marginRight", "layout_marginBottom"},
        padding = {"padding", "paddingLeft", "paddingTop", "paddingRight", "paddingBottom"},
        background = {"background", "foreground", "backgroundTint"},
        text = {"text", "hint", "textSize", "textColor", "textStyle"},
        gravity = {"gravity", "layout_gravity"},
        weight = {"layout_weight", "weightSum"},
        advanced = {"elevation", "translationZ", "rotation"}
    }
}
...
function UIEditor:showMainMenu()
    while true do
        ...
        local choice = gg.choice(menuItems, nil, "云烛RLGG UI编辑器 v"..self._VERSION)
        ...
        elseif choice == 6 then
            self:generateAndSaveCode()
        elseif choice == 7 then
            self:loadLayout()
        elseif choice == 8 then
            self:showEditorSettings()
...
function UIEditor:createNewLayout()
    ...
    if choice then
        ...
        self:recordHistory("创建 "..layoutDef.name)
        gg.toast(layoutDef.name.." 创建成功", false)
    end
end
...
function UIEditor:addComponent()
    ...
    local widgetCategories = {
        "📄 基础控件", "✅ 选择控件", "📈 进度控件",
        "🔄 高级组件", "🗺️ 导航组件", "💬 对话框"
    }
    
    local choice = gg.choice(widgetCategories, nil, "选择组件类别")
    local widgetList = {}
    
    if choice == 1 then
        widgetList = self._CONTROLS.widgets[1].type == "TextView" and self._CONTROLS.widgets or {}
    elseif choice == 2 then
        -- 选择控件逻辑
    end
    
    -- 组件选择逻辑（因篇幅限制简化处理）
    local selectedWidget = self._CONTROLS.widgets[1]
    local newComponent = {
        type = selectedWidget.type,
        properties = self:getDefaultProperties(selectedWidget.properties),
        metadata = {
    ...
    gg.toast(selectedWidget.name.." 添加成功", false)
end

--[[ 属性编辑系统 ]]--
function UIEditor:editComponentProperties()
    if not self.currentLayout then
        gg.alert("当前没有可编辑的布局")
        return
    end
    ...
    local components = {}
    for i, comp in ipairs(self.currentLayout.children) do
        table.insert(components, string.format("%s #%d", comp.type, i))
    end
    table.insert(components, "🔙 返回")

    while true do
        local choice = gg.choice(components, nil, "选择要编辑的组件")
        if not choice or components[choice] == "🔙 返回" then break end
        
        local selected = self.currentLayout.children[choice]
        local widgetDef
        for _, w in ipairs(self._CONTROLS.widgets) do
            if w.type == selected.type then
                widgetDef = w
                break
            end
        end
        ...
        if widgetDef then
            self:showAttributeEditor(selected, widgetDef.properties)
        end
    end
end

function UIEditor:showAttributeEditor(component, propertiesDef)
    while true do
        local attrList = {"🔙 返回"}
        for attrName, _ in pairs(propertiesDef) do
            table.insert(attrList, 1, attrName)
        end

        local choice = gg.choice(attrList, nil, "编辑属性 - "..component.type)
        if not choice or attrList[choice] == "🔙 返回" then break end
        
        local attrName = attrList[choice]
        local propDef = propertiesDef[attrName]
        local currentValue = component.properties[attrName]

        -- 根据属性类型处理输入
        if propDef.type == "choice" then
            local newChoice = gg.choice(propDef.options, nil, "选择 "..attrName)
            if newChoice then
                component.properties[attrName] = propDef.options[newChoice]
            end
        elseif propDef.type == "color" then
            local color = gg.colorPicker(currentValue)
            if color then
                component.properties[attrName] = color
            end
        elseif propDef.type == "function" then
            local code = gg.prompt({"输入Lua函数代码"}, {currentValue or ""}, {"text"})
            if code and code[1] ~= "" then
                component.properties[attrName] = code[1]
            end
        else
            local inputType = propDef.type == "number" and "number" or "text"
            local result = gg.prompt({"设置 "..attrName}, {tostring(currentValue)}, {inputType})
            if result then
                component.properties[attrName] = propDef.type == "number" and tonumber(result[1]) or result[1]
            end
        end
    end
end
...
function UIEditor:generateCode()
    if not self.currentLayout then return nil end
    ...
    local function processComponent(comp, indent)
        local indentStr = string.rep("    ", indent)
        table.insert(code, indentStr..comp.type..",")
        
        for prop, value in pairs(comp.properties) do
            local propLine
            if prop == "onClick" then
                propLine = string.format('%sonClick = %s,', indentStr.."    ", value)
            elseif type(value) == "string" and value:match("^#") then
                propLine = string.format('%s%s = "%s",', indentStr.."    ", prop, value)
            else
                propLine = string.format('%s%s = %s,', indentStr.."    ", prop, tostring(value))
            end
            table.insert(code, propLine)
        end

        if comp.children then
            for _, child in ipairs(comp.children) do
                table.insert(code, indentStr.."    {")
                processComponent(child, indent + 2)
                table.insert(code, indentStr.."    },")
            end
        end
    end

    processComponent(self.currentLayout, 1)
...
function UIEditor:previewUI()
    ...
    callAsyncTask(function()
        local success, view = pcall(tempFunc)
        if success and view then
            gg.showAlert(view)
        else
            gg.alert("预览失败：\n"..tostring(view))
        end
...
--[[ 文件操作 ]]--
function UIEditor:generateAndSaveCode()
    local fileName = gg.prompt({"输入保存文件名"}, {self.config.defaultFile}, {"text"})[1]
    if not fileName then return end
    
    local fullPath = self.config.outputPath..fileName
    local code = self:generateCode()
    
    if file.write(fullPath, code) then
        gg.toast("保存成功："..fullPath, false)
        return true
    else
...
"})",
    ...
    table.insert(code, "    }, nil, context)")
    table.insert(code, "floatingWindowManager:run()")
...
Yunzhu.button(
	...
	Yunzhu.seek("控件高度（layout_height）","lt",1,100,1),--名字，变量名字，最小，最大，初始
	Yunzhu.seek("控件宽度（layout_width）","lt",1,100,1),--名字，变量名字，最小，最大，初始
...
Yunzhu.button(
	...
	Yunzhu.seek("容器高度（layout_height）","lt",1,100,1),--名字，变量名字，最小，最大，初始
	Yunzhu.seek("容器宽度（layout_width）","lt",1,100,1),--名字，变量名字，最小，最大，初始
...
radios = {
	...
	orientation = "vertical",
	padding = "0dp",
	      background = getHorizontalBG({0x00ff00cf,0x005500ff},15,1,0x16ffffff),
		elevation='2dp',
	layout_width = 'fill_parent',
}
for i = 2,#views do
...
buts={}
	heir=getRes("heir")
	function Yunzhu.button2(txt,func,txtc)
		if not txt then txt = "未设置" end
		if not txtc then txtc="#07C160" end
		...
		layout_height = "wrap_content", {
			...
			layout_marginRight='3dp',
			elevation='2dp',
			background = getHorizontalBG({0x00000000,0x00111111},15,2,0xff07C160),
			padding="10dp",
				onClick = function()
	...
	butsq={}
	heir=getRes("heir")
	function Yunzhu.buttonM(txt,func,txtc)
		if not txt then txt = "未设置" end
		if not txtc then txtc="#ffffff" end
		local tid="Cbutton"..guid()
		butsq[tid]=luajava.loadlayout(
		{
			LinearLayout,
		layout_width = 'wrap_content',
		layout_height = "wrap_content", 
      ...
      textSize = "13sp";
				onClick = function()
					Yunzhu.controlWater(butsq[tid],300)
					luajava.newThread(func):start() end,
	...
	buts={}
	heir=getRes("heir")
	function Yunzhu.buttonred(txt,func,txtc)
		if not txt then txt = "未设置" end
		if not txtc then txtc="#0E492A" end
		local tid="Cbutton"..guid()
		buts[tid]=luajava.loadlayout(
		{
			LinearLayout,
		...
		layout_height = "wrap_content", {
			...
			layout_marginRight='3dp',
			elevation='2dp',
			background = getHorizontalBG({0x00000000,0x00111111},15,2,0x5007C160),
			padding="10dp",
				onClick = function()
					Yunzhu.controlWater(buts[tid],300)
					luajava.newThread(func):start() end,
	...
	buts={}
	heir=getRes("heir")
	function Yunzhu.buttonlink(txt,func,txtc)
		if not txt then txt = "未设置" end
		if not txtc then txtc="#ffffff" end
		local tid="Cbutton"..guid()
		buts[tid]=luajava.loadlayout(
		...
		layout_height = "wrap_content", {
			...
			layout_marginRight='3dp',
			elevation='2dp',
			background = getHorizontalBG({0xff07C160,0x500E492A},15,1,0xffffffff),
			padding="10dp",
				onClick = function()
					Yunzhu.controlWater(buts[tid],300)
					luajava.newThread(func):start() end,
...
function Yunzhu.quyuBasic(views)
local tid = "box"..guid()
local ttid = tid.."678"
firadio = {
	LinearLayout,
...
radios = {
	...
	orientation = "vertical",
	padding = "0dp",
	background = getHorizontalBG({views[1],views[1]},40,3,0xff49454F),
	layout_width = 'fill_parent',
}
for i = 2,#views do
radios[#radios+1] = views[i]
...
function Yunzhu.texta(text,color,size,isjz)
...
-- 封装可实时更新的文本框控件
function Yunzhu.dynamicText(initText, color, size, isCenter)
    -- 默认参数处理
    if not initText then initText = "" end
    if not color then color = "#161616" end
    if not size then size = "16sp" end
    ...
    -- 创建TextView
    local textView = luajava.loadlayout(layoutParams)
    
    -- 添加更新方法
    textView.updateText = function(newText)
        textView.text = newText
    end
    
    -- 添加颜色更新方法
    textView.updateColor = function(newColor)
        textView.textColor = newColor
    end
    
    -- 添加大小更新方法
    textView.updateSize = function(newSize)
...
--[[
local myText = Yunzhu.dynamicText("初始文本", "#FF0000", "20sp", true)

-- 添加到布局中（假设有一个父布局叫mainLayout）
...
timers.setInterval(function()
    counter = counter + 1
    myText.updateText("更新次数: "..counter)
    
    -- 随机改变颜色
    local r = math.random(0, 255)
    local g = math.random(0, 255)
    local b = math.random(0, 255)
    myText.updateColor(string.format("#%02X%02X%02X", r, g, b))
end, 1000)
...
pcall(func2)
	...
	function getButtonBG()
		...
		selector:addState({
			-android.R.attr.state_pressed
		}, getHorizontalBG({0xff141218,0xff141218},40,3,0xff49454F))
		return selector
	end
	boxes = {} boxpic = {}
...
firadio = {
    ...
    gravity = "center",
    orientation = "vertical",
   background=getHorizontalBG({0x78141218,0x78141218},40,3,0xff49454F),
    LayoutTransition=layoutTransition,
...
function Yunzhu.boxl(views)
 ...
 --   src=getRes("hei_right"),
  ...
  firadio={
    LinearLayout,
    layout_width='fill_parent',
    background=luajava.loadlayout{GradientDrawable,color=0x00000000,cornerRadius=20},
    elevation="3dp",
    layout_height="wrap_content",
    layout_margin="4dp",
    orientation="vertical",
  ...
  if type(views[1])=="string" or type(views[1])=="number" then
    ...
    gg.alert("Yunzhu.boxl 的 table 内第一个元素必须是 string") os.exit()
  end
  radios={
    LinearLayout,
    layout_marginLeft="5dp",
...
function Yunzhu.seeka(name,variable,min,max,now)
  local nid=name..string.randomUUID(true)
...
Yunzhu.menu({
  {---第一页          
  Yunzhu.text("主页",'0xffE6E0E9',"18sp",true),
    --Yunzhu.text(项目名字,'0xffffffff',"15sp",true),
    Yunzhu.quyuBasic({
	"0xffD0BCFF",
--	Yunzhu.text("  ⊕\n",'0xff381E72',"38sp",true),
	Yunzhu.text("云烛 UI工具箱",'0xff381E72',"18sp",true),
	Yunzhu.text("这是一个基于RLGG material design style的Luajava UI工具箱。提供大量免费/付费独家UI",'0xff381E72',"13sp",true),
	}),
	Yunzhu.quyuBasic({
	"0xff4F378B",
--	Yunzhu.text("  ⊕\n",'0xffffffff',"38sp",true),
		Yunzhu.text("这是什么？",'0xffEADDFF',"18sp",true),
	Yunzhu.text("云烛UI工具箱 涵盖UI编辑器 UI下载 聊天室 免费音乐播放等为一体 ",'0xffEADDFF',"13sp",true),	
	}),

	--Yunzhu.dynamicText("初始文本", "#FF0000", "20sp", true),
    Yunzhu.switch(
			"设定",
			function()
				自定义主题()
