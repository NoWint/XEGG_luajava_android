...
public class Main {
	...
	public final void MenuMain(final Context context) {
		...
		menu.addTextPage(setting, 0, "设置");
		menu.addCheckbox(setting, 1, "隐藏图标", new Menu.callbackCheck() {
				public void onClickTap(int page, int feat, int ischeck) {
					menu.setIsVisible(ischeck);
				}
			});
		menu.addSlider(setting, 0, "菜单大小", 65, 100, 100, new Menu.callbackSlider() {
			public void onChange(int page, int feat, int value) {
				menu.setScale((float) value / 100);
			}
		});
		menu.addSlider(setting, 0, "菜单透明", 10, 100, 100, new Menu.callbackSlider() {
				public void onChange(int page, int feat, int value) {
		...
		menu.addCheckbox(setting, 1, "显示电量", new Menu.callbackCheck() {
        ...
        menu.addTextPage(setting,0,"此模板为Without三改美化");
        
		menu.addLinkButton(setting,"加入交流群","http://qm.qq.com/cgi-bin/qm/qr?_wv=1027&k=gSLtikmDEUuNoh7AT5vGTNvtGG91-EqZ&authKey=COtv%2BDn3yuwEsLWdbfFED5ks6R%2BlDfe8g9f9aUR4UBSkl9e%2BUBWbD2YYWHCn9La1&noverify=0&group_code=730967224");
		
		menu.addTextPage(setting,0,"二改by皓月当空");
		menu.addTextPage(setting,0,"三改by孤寂");
