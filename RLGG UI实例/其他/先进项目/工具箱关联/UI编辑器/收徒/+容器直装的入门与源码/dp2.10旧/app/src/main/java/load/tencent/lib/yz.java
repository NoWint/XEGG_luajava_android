...
public class yz {
	...
	static String WY_APPID = "56788"; // 项目id
	static String WY_APPKEY = "8jkx8R1Ph2jq2tD1"; //项目key
	static String WY_RC4KEY = "mumAMlfHxLgcv";//RC4密钥
		...
		new Thread(new Runnable() {
				...
				public void run() {
                    ...
                    try {
                        ...
                        JSONObject jsonObject = new JSONObject(content);
                        String code=jsonObject.getString("vsBzMruiTDNLH");//是否登录成功
                        String Message=jsonObject.getString("gCfsiamSKLPFA");
                        String check=jsonObject.optString("fKUQHWAgzxErN");//校验密钥
                        Long timee=jsonObject.optLong("VjMyuvnXbxkEz");//服务器时间戳
                        if (check.equals(encodeMD5(timee.toString() + WY_APPKEY + uuid))) {
                            Looper.prepare();							
							...
							Toast.makeText(context, "数据过期", Toast.LENGTH_LONG).show();
							Looper.loop();
                        } else if (code.equals("415")) {//登录成功
                            JSONObject json = new JSONObject(Message);						
                            Long vip=json.optLong("vXmfNKiaJcUQH");
