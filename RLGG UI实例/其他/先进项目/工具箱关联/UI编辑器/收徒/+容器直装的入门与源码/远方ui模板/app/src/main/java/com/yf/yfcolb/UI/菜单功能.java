...
public class 菜单功能 extends LinearLayout {
    ...
    private void init(Context mContext) {
        ...
        final Button 菜单一 = Packaging.Button("功能菜单", "CD1.png", 配置.buttonParams);
        菜单栏.addView(菜单一);

        final Button 菜单二 = Packaging.Button("菜单二", "CD2.png", 配置.buttonParams);
        菜单栏.addView(菜单二);

        final Button 菜单三 = Packaging.Button("菜单三", "CD3.png", 配置.buttonParams);
        菜单栏.addView(菜单三);

        final Button 菜单四 = Packaging.Button("菜单四", "CD4.png", 配置.buttonParams);
        菜单栏.addView(菜单四);

        final Button 菜单五 = Packaging.Button("菜单设置", "CD5.png", 配置.buttonParams);
        菜单栏.addView(菜单五);
