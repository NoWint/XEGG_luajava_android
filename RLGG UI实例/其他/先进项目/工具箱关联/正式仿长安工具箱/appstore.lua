...
--[[创建App Store风格卡片控件（带默认参数），
function createAppStoreCard(params)
    -- 合并默认参数与用户参数
    local config = {
        imagePath = "/sdcard/default.jpg",  -- 默认图片路径
        title = "应用名称\n应用描述",         -- 默认文字内容
        buttonText = "获取",               -- 默认按钮文字
        onClick = function() end,          -- 默认空点击事件
        width = 0.8,                       -- 默认宽度比例
        elevation = 4                      -- 默认阴影高度
    }
    for k,v in pairs(params or {}) do config[k] = v end

    -- 卡片容器（模拟MaterialCardView）
    local card = createView(LAYOUT_LINEAR)
    card:setSize(config.width*SCR_W, -2)    -- 设置卡片宽度
    card:setGravity(17)                     -- CENTER
    card:setBackgroundColor(0xFFFFFFFF)     -- 白色背景
    card:setCornerRadius(dp2px(12))         -- 12dp圆角
    card:setElevation(dp2px(config.elevation)) -- 动态阴影
    card:setPadding(dp2px(16))              -- 统一内边距

    -- 内部垂直布局
    local layout = createView(LAYOUT_LINEAR)
    layout:setOrientation(1)                -- VERTICAL
    layout:setGravity(17)                   -- CENTER
    card:addView(layout)

    -- 图片区域（带比例约束）
    local img = createView(IMAGEVIEW)
    img:setSize(-1, 0.6*config.width*SCR_W) -- 动态高度计算
    img:setScaleType(1)                     -- CENTER_CROP
    img:setImagePath(config.imagePath)
    layout:addView(img)

    -- 文字区域（带自动换行）
    local text = createView(TEXTVIEW)
    text:setSize(-1, -2)                    -- 自适应高度
    text:setText(config.title)
    text:setTextSize(18)
    text:setTextColor(0xFF333333)
    text:setLineSpacing(dp2px(6), 1)        -- 6dp行间距
    text:setGravity(17)                     -- CENTER
    text:setMarginTop(dp2px(12))            -- 上边距
    layout:addView(text)

    -- 操作按钮（带点击反馈）
    local btn = createView(BUTTON)
    btn:setText(config.buttonText)
    btn:setTextSize(16)
    btn:setTextColor(0xFFFFFFFF)
    btn:setBackgroundColor(0xFF007AFF)      -- iOS风格蓝色
    btn:setCornerRadius(dp2px(20))          -- 圆角按钮
    btn:setPadding(dp2px(16), dp2px(8))     -- 按钮内边距
    btn:setMarginTop(dp2px(16))             -- 按钮上边距
...
-- 在父容器中添加卡片
--
local mainLayout = createView(LAYOUT_LINEAR)
mainLayout:setSize(-1, -1)
mainLayout:setOrientation(1)

-- 创建卡片实例1
local card1 = createAppStoreCard{
    imagePath = "/sdcard/app1.jpg",
    title = "图像编辑器\n专业级照片处理工具",
    buttonText = "免费下载",
    onClick = function()
        print("开始下载图像编辑器")
...
-- 创建卡片实例2（带自定义宽度）
local card2 = createAppStoreCard{
    imagePath = "/sdcard/app2.jpg",
    title = "文档扫描仪\n智能OCR文字识别",
    width = 0.9,
    elevation = 8,
    onClick = function()
...
--mainLayout:addView(createView(SPACE)) -- 添加间距
...
function appStoreCard(config)
    ...
    return {
        ...
        { -- 图片区域
            ImageView,
            layout_width = "match_parent",
            layout_height = "180dp", -- 固定高度
            scaleType = "centerCrop",
            src = config.imagePath or "/storage/emulated/0/云烛/图片/abc.png",
		...
		v:setVideoPath(config.imagePath)  --ca_bgsp
	--	background={GradientDrawable,cornerRadius=90},
	--	clipToOutline=true,
		v.start(v)
--	    v:setZOrderOnTop(true) --top播放
        ...
        { -- 文字与按钮容器
            ...
            { -- 操作按钮
                ...
                textSize = "14sp",
                textColor = config.BTC,
                background = {
                    GradientDrawable,
                    color = config.buttonColor, -- iOS蓝
                    cornerRadius = "5dp"
                },
                padding = "12dp",
...
local mian_layout = {
	...
	layout_height = "match_parent",
	orientation = "vertical",
	background = getShape00(0,0xfff5f5f5,0xfff5f5f5,0xfff5f5f5),
...
local drawer_titlebar_layout = luajava.loadlayout{
		...
		layout_height="170dp",
	{
		...
		gravity="right",
		__onFinish = function(v)
		v:setVideoPath("/sdcard/云烛/配置文件/yunzhumax")  --ca_bgsp
		v.start(v)
--	    v:setZOrderOnTop(true) --top播放
        v:getHolder():setFormat(PixelFormat.TRANSLUCENT) --避免黑屏闪烁
		v:setOnCompletionListener({
...
local expandableList_layout = {
	...
	background=getCorner({0xFFFFFFFF,0xFFFFFFFF},0,0,0xFFFFFFFF,50,50,0,0),
	{
		...
		elevation = "2dp",
		layout_margin = "15dp",
		background=getCorner({0xFFFFFFFF,0xFFFFFFFF},0,0,0xFFFFFFFF,10,10,10,10),
		Card("YunzhuGalaxy",'ca_hm',
			function()
			drawerView:closeDrawer(Gravity.START)
			end
		),
...
views["YunzhuGalaxy"]={
 ...
 appStoreCard{
        imagePath ="/sdcard/云烛/配置文件/yunzhumax",
        title = "歡迎使用 Yunzhu Galaxy",
        buttonText = "Download",
        miaoshu="在這裡，發現和創造整個世界。",
        backg=0xffffffff,
        tc=0xff131313,
        BTC=0xffffffff,
	...
	onClick = function() 
			luajava.newThread(function() 切换("资源下载中心")  end):start() end,
        
    },
 appStoreCard{
        imagePath ="/sdcard/云烛/配置文件/material",
        title = "全新 質感2.0 UI",
        buttonText = "Explore",
        miaoshu="動態島設計 高效 節能 創造力",
        backg=0xffffffff,
        tc=0xff131313,
        BTC=0xfffffffd,
        buttonColor=0xff131313,
