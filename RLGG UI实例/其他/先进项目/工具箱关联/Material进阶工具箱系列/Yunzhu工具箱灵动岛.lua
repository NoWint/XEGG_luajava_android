...
<meta charset="UTF-8">
  ...
  <style>
    ...
    .dynamic-island.active {
      width: 160px; /* Adjusted to maintain the same height as the circle's diameter */
      height: 30px;
      border-radius: 15px;
    ...
    .dynamic-island.expanded {
      width: 300px;
      height: 130px; /* Adjusted to fit the music player */
      border-radius: 15px;
    }
    ...
    .dynamic-island.active .state-text {
      opacity: 1;
      transform: translateY(0);
    ...
    .dynamic-island.expanded .music-player {
      display: block;
      opacity: 1;
    }
...
<script>
  let activeIndex = 0; // To keep track of the current state
  const states = ['default', 'active', 'expanded'];
  const dynamicIsland = document.querySelector('.dynamic-island');
  ...
  dynamicIsland.addEventListener('click', function() {
    ...
    dynamicIsland.classList.remove(...states);
    // Add next state
    activeIndex = (activeIndex + 1) % states.length;
    dynamicIsland.classList.add(states[activeIndex]);

    if (activeIndex === 2) {
      // When expanded, show the music player
      musicPlayer.style.display = 'block';
      musicPlayer.style.opacity = '1';
      progressBar.style.width = '50%'; // Set initial progress
    } else {
      // When not expanded, hide the music player
  ...
  if 缩放 then
    ...
    if Action == MotionEvent.ACTION_DOWN then
      ...
      if x==0 or x==-2 then x=1700 y=1300 end
        ckou:setBackground(getVerticalBG({0xffF8F9FF,0xffF8F9FF},15,5,主题色))     
     elseif Action == MotionEvent.ACTION_MOVE then
      isMove = true
      mubx=tonumber(x) + (event:getRawY() - RawY)
      if mubx>=600 and mubx<=11000 then
     ...
     elseif event:getAction() == MotionEvent.ACTION_UP then
      ckou:setBackground(getVerticalBG({0xffF8F9FF,0xffF8F9FF},15,0,0xaa232323))
    end
    window:updateViewLayout(floatWindow, mainLayoutParams)
  end
end
...
Yunzhu.menu = function(sview)
  ...
  function 切换(x)
    ...
    luajava.runUiThread(function()
      for i = 1, #stab do
       ...
       -- _ENV["jm"..i]:setBackground(getVerticalBG({0xffF8F9FF,0xffF8F9FF},15))--未选中的颜色
        _ENV["layout"..i]:setVisibility(View.GONE)
      end
      _ENV["jmt"..当前ui]:setTextColor(主题色)--选中的字体颜色
  ...
  for i = 1,#stab do
   ...
   --   background=getVerticalBG({0xffF8F9FF,0xffF8F9FF},15),
      onClick = function()
        切换(i)
      end,
      {
  ...
  topbar = luajava.loadlayout({
    ...
    layout_width="match_parent",
    layout_height="35dp", 
 background=getCorner({0xffF8F9FF,0xffF8F9FF},15,0,0xff232323,15,15,0,0),
 elevation = "3dp",
    
    {FrameLayout,
      layout_width="wrap_content",
      layout_height="match_parent",
      background=getCorner({0xffF8F9FF,0xffF8F9FF},15,0,0xff232323,45,0,0,0),
    ...
    {FrameLayout,
      layout_width="wrap_content",
     background=getCorner({0xffF8F9FF,0xffF8F9FF},15,0,0xff232323,0,45,0,0),
      layout_height="match_parent",
...
ckou = {
    ...
    layout_height = "match_parent",
    orientation = "vertical",
    background = getVerticalBG({"0xfff7f8f9","0xfff7f8f9"},20),
  ...
  floatWindow = {
	...
	Yunzhu.lddbox({
	"  By Yunzhu  ",
	Yunzhu.text("Powered By 云烛",主题色,"10sp",true),
	
	Yunzhu.seekA("00:00","lt",1,100,1),--名字，变量名字，最小，最大，初始
	Yunzhu.switchA(
    '启动AppleIOS灵动岛',
    function()
...
:setPositiveButton('我已知晓')
    ...
    Yunzhu.boxA({
	"展开灵动岛",
	Yunzhu.switchA("开关",function() gg.alert("开") end,function() gg.alert("关") end),
	Yunzhu.switchA("开关",function() gg.alert("开") end,function() gg.alert("关") end),
	Yunzhu.switchA("开关",function() gg.alert("开") end,function() gg.alert("关") end),
	Yunzhu.switchA("开关",function() gg.alert("开") end,function() gg.alert("关") end),
	}),
...
---setThumbTintList('0xff000000'),
  ...
  SwitchColor(_ENV[nid],主题色,0xffE1E2E8)
  return rest
end
switches = {}
...
function Yunzhu.text(txt,color,size,ftc)
  ...
  if not color then color = "0xff000000" end
    ...
    layout_height = "45dp",
{
      ...
      elevation='2dp',
        ...
        "0dp","0dp","0dp","0dp"
			...
			SliderColor(_ENV[nid],六位主题色,六位主题色,六位主题色)
return rest
end
buts={}
...
function getButtonBG()
  ...
  selector:addState({
    -android.R.attr.state_pressed
  }, getVerticalBG({0xffF8F9FF,0xffF8F9FF},17))--0xffF8F9FF,0xffF8F9FF
  return selector
end
...
local rest = luajava.loadlayout({
		...
		},
	{
		...
		layout_height = "34dp",
		gravity = "center",
		background = getHorizontalBG({0xffF8F9FF,0xffF8F9FF},5,3,0xffF8F9FF),
		elevation='2dp',
		{
		TextView,
		text="关",		
...
boxpic[tid]:setColorFilter(0xff000000)
end
end
boxes = {} boxpic = {}
...
firadio = {
    ...
    gravity = "center",
    orientation = "vertical",
 background = luajava.loadlayout{GradientDrawable,color=0xffF8F9FF,cornerRadius=20},
   elevation='2dp',
   layout_marginRight = "4dp",
   layout_marginLeft = "4dp",
    LayoutTransition=layoutTransition
...
_ENV [t1id] = luajava.loadlayout ({
    ...
    gravity = "center",
    {
      ...
      layout_marginRight = "4dp",
	  layout_marginLeft = "4dp",
      background = luajava.loadlayout{GradientDrawable,color=主题色[3],cornerRadius=40},
      firadio
      }
      })
return _ENV [t1id]
...
function Yunzhu.switchA(name,func1,func2,miaoshu)
...
chazhi={} chajv={}
function Yunzhu.seekA(name,bian,smin,smax,nows)
_ENV[bian] =nows
smin=tonumber(smin) smax=tonumber(smax)
chajv[bian]=smax-smin
chazhi[bian]=1-smin
...
boxpic[tid] = luajava.loadlayout {
	ImageView ,
	src = "https://cdn-community.codemao.cn/47/community/d2ViXzUwMDFfMTAwMTQwOTNfMTQ2NzVfMTcyMTM2NTg1NTk5Nl9mMDE3NmJlYg.png",
	layout_width = "10dp" ,
	layout_height = "10dp" ,
...
_ENV [t1id] = luajava.loadlayout ({
    ...
    gravity = "center",
    {
      ...
      layout_marginRight = "4dp",
	  layout_marginLeft = "4dp",
      background = luajava.loadlayout{GradientDrawable,color="0xff000000",cornerRadius=40},
      firadio
      }
      })
return _ENV [t1id]
end
function Yunzhu.alertDialog(title,text,control)
...
function Yunzhu.ztBlue()
主题色="0xff36618E"
六位主题色="36618e"
end
function Yunzhu.ztPink()
主题色="0xff333333"
六位主题色="333333"
end
...
{
  {
  Yunzhu.text("Material UI",主题色,"18sp"),
  
  Yunzhu.text("Powered by Yunzhu",主题色,"10sp"),
  Yunzhu.box({
			"XXX区",
    ...
    Yunzhu.button("按钮测试",function() Snackbar:make(ckou,"加载成功 已完成操作✅",Snackbar.LENGTH_SHORT):setBackgroundTint("0xff161616"):show()end),
    	Yunzhu.check({
				{
					"套娃",
					function()
