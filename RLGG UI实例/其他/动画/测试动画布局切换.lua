...
function YunzhuTransition:start()
    ...
    local anim = ViewAnimationUtils.createCircularReveal(
        self.endView,
        centerX,
        centerY,
        self.startRadius,
...
-- 定义 xfcpic（25dp 圆角正方形，红色背景）
local xfcpic = luajava.loadlayout({
    RelativeLayout,
    layout_width = "25dp",
    layout_height = "25dp",
    background = "#FF0000",  -- 红色
    cornerRadius = "12.5dp",  -- 圆角半径 = 宽度/2
    id = "xfcpic",
})

-- 定义 floatwindow（500dp 圆角正方形，蓝色背景）
local floatwindow = luajava.loadlayout({
    RelativeLayout,
    layout_width = "500dp",
    layout_height = "500dp",
    background = "#0000FF",  -- 蓝色
    cornerRadius = "50dp",   -- 圆角半径
    id = "floatwindow",
    visibility = gg.VIEW_GONE,  -- 初始隐藏
})

-- 添加到 RLGG 的 Window（假设 _ENV 是 RLGG 的全局变量）
_ENV:addContentView(xfcpic)
_ENV:addContentView(floatwindow)

-- 加载动画库
local CircularReveal = require("YunzhuTransition")

-- 定义两个 View（假设已经通过 luajava.loadlayout 或其他方式创建）
local xfcpic = luajava.getIdView("xfcpic")       -- 第一个 View
local floatwindow = luajava.getIdView("floatwindow") -- 第二个 View

-- 初始状态：xfcpic 显示，floatwindow 隐藏
xfcpic:setVisibility(gg.VIEW_VISIBLE)
...
button.onClick = function()
    if floatwindow:getVisibility() == gg.VIEW_GONE then
        ...
        hideFloatWindow() -- 隐藏 floatwindow，显示 xfcpic
    end
end