...
if not UI宽度 then UI宽度="500dp" end
if not UI高度 then UI高度="290dp" end
if not 项目名字 then 项目名字="云烛UI" end
local changan = {}
local 音量键=true
local 摇一摇=true
...
dex.loadfile("/storage/emulated/0/云烛/配置文件/GifView.dex")
...
if file.length("/sdcard/云烛/图片/shark/"..tmp[1],false)<200 then
gg.toast("正在下载资源"..ii.."/"..#ckimg.."\n请耐心等待")
file.download(tmp[2],"/sdcard/云烛/图片/shark/"..tmp[1])
end
end
ckimg = {
...
local FontPath = "/sdcard/云烛/配置文件/google.otf"--字体路径
...
local tmp=luajava.loadlayout(
	{
		TextView,
		Typeface=font,
		text = string.fromHtml(" <font color=#000000>"..getTimeStamp(os.time()).."</font> <font color=#545454>"..txt.."</font>"),
		textSize = size,
		gravity="left",
...
menuView= luajava.loadlayout {
    ...
    layout_height="match_parent",
    {
        ...
        id="topImage",
        layout_width="210dp",
        layout_height="105dp", -- 适当调整高度
        scaleType="centerCrop",
        src=getRes("DarkGura"), -- 这里是你的顶部图片资源
    },
    {
	TextView,
	Typeface=font,
    ...
    background=getCorner({0xAE000000,0xAE000000},15,nil,nil,0,0,15,15), 
    },      
        {
        LinearLayout,
        layout_height="match_parent",
					...
					{onCheckedChanged=function(view,isc)
						if isc then
							...
							if func~=nil then
								...
								uiadtext( name.." </font><font color='#0062ff'>已开启")
							end
						else
...
xfcpic="/sdcard/云烛/图片/shark/gura"
