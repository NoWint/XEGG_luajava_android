...
dex.loadfile("/sdcard/云烛/图片/sensor.dex")
...
function panduan(rec) fille,err = io.open(rec) if fille == nil then return false else return true end end
function pdcf(lujing) rec = "/sdcard/云烛/配置文件/"..lujing fille,err = io.open(rec) if fille == nil then return false else return true end end
sleep = gg.sleep
...
ckimg = {
	"云烛/boom.mp3","云烛/kgstart.mp3","云烛/kgend.mp3",
	"云烛/quarkringon",
	"云烛/quarkringoff",
	"云烛/quarksearchw",
...
for a = 1,#stab-2 do
	for b = 1,4 do
		...
		if b==1 then
			_ENV["switchs"..a..b][#_ENV["switchs"..a..b]+1]=Yunzhu.text("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n")[1]
		end
	end
end
		...
		orientation = "vertical",
		{
			...
			padding = "5dp",
			{
				...
				orientation = "vertical",
				{
					...
					layout_weight = 1,
					layout_width = "match_parent",
					background = getHorizontalBG({0x75161616,0x75161616},10,2,0xff047274),
					gravity = "center_vertical",
					orientation = "vertical",
					_ENV["switchs"..i.."1"]
				},
...
launchui = luajava.loadlayout({
	...
	layout_width = "match_parent",
	{
		...
		gravity="center",
		{
			...
			gravity = "center",
			layout_gravity="center_horizontal",
						background=getHorizontalBG({0x900e585f,0x900e585f},10,2,0xff047274),
			...
			onClick = function() 
			luajava.newThread(function() 切换(1)  end):start() end,
			background=getHorizontalBG({0x900e585f,0x900e585f},10,2,0xff047274),
			layout_height = "25dp",
			layout_width = "55dp",
			layout_gravity="center_horizontal",
			...
			background=getHorizontalBG({0x900e585f,0x900e585f},10,2,0xff047274),
		{
			...
			onClick = function() 
			luajava.newThread(function() 切换(2)   end):start() end,
			background=getHorizontalBG({0x900e585f,0x900e585f},10,2,0xff047274),
			layout_height = "25dp",
			layout_width = "55dp",
			layout_gravity="center_horizontal",
...
searchUI:setHintTextColor(0xff447F75)
gg.playMusic("/sdcard/云烛/图片/云烛/boom.mp3")
ringon=获取图片("/sdcard/云烛/图片/云烛/quarkringon")
ringoff=获取图片("/sdcard/云烛/图片/云烛/quarkringoff")
			...
			--elevation = "4dp",
				...
				--以上是多板块
				{
					...
					--elevation = "4dp",
						...
						layout_height="20dp",
						layout_width="20dp",
						src=获取图片("/sdcard/云烛/图片/quarksearchw"),
						padding="10dp",
						background = getHorizontalBG({0x75161616,0x75161616},10,2,0xff047274),
						onTouch=hanshu,
						onClick=searching,
...
local function invoke2()
    ...
    local blurRoot = HiddenApiBypass:getDeclaredMethod(ViewRootImpl, "createBackgroundBlurDrawable", {})
      ...
      blurRoot:setBlurRadius(30)
      blurRoot:setCornerRadius(10)
...
function tosearch(besearch)
	...
	elseif #result==0 then
		...
		adtext("你是否在找:","#447F75")
		for i=1,#result do
			adtext("第"..result[i][2].."页第"..result[i][3].."板块的“"..result[i][1].."”","#447F75")
		end
		切换(result[1][2])
		if string.find(besearch,"打开")~=nil or string.find(besearch,"开启")~=nil or string.find(besearch,"启动")~=nil or string.find(besearch,"执行")~=nil then
			if #result==1 then
				...
				adtext("搜索到"..#result.."个结果，请手动执行","#447F75")
			end
		end
...
luajava.runUiThread (function ()
	...
	_ENV["jmt"..i]:setTextColor(0xff447F75)
	...
	_ENV["jmt"..当前ui]:setTextColor(0xff000000)
...
slcta = getHorizontalBG({0x900e585f,0x900e585f},10,2,0xff047274)
...
pcall(func1)
	...
	luajava.getIdValue(nid.."t"):setTextColor(0xff447F75)
...
--if not yans then yans = "#447F75" end
...
if not gtvb4 then gtvb4 = 0 gtvb5 = 0xE6E9F000 end
local jianbians = luajava.new(GradientDrawable)
jianbians:setCornerRadius(gtvb3)
jianbians:setGradientType(GradientDrawable.LINEAR_GRADIENT)
jianbians:setColors(gtvb1)
...
--	SliderColor(_ENV[nid],232323,232323,232323)
...
function Yunzhu.text2(txt,color,size)
...
if not txt then txt = "未设置文字" end
if not color then color = "0xff447F75" end
...
function Yunzhu.check2(name,func1,func2)
...
function Yunzhu.intcheck(name,func1,func2,size)
nid = name..guid()
lname=name
...
function Yunzhu.button(txt,func,txtc)
  ...
  if not txtc then txtc="#447F75" end
...
bglist={
getHorizontalBG({0x75161616,0x75161616},10,2,0xff047274),
getShape(30,{0xffD9DCE2,0xffD9DCE2},0,0x01B8B8B8),
getShape(30,{0xe800FFC1,0x8800AFFF},0,0x01B8B8B8),
	getShape(30,{0xffDCDFE6,0xffDCDFE6},0,0xff121212),
...
--悬浮窗图标链接
YZ悬浮窗 = "https://cdn-community.codemao.cn/47/community/d2ViXzEwMDFfMTAwMTQwOTNfMTAwMTQwOTNfMTcwNjUyNTg1MTY4NV80ZmRmNjBjMw.png"

stab = {
---侧边标题和图标
	{
		"Combat", "https://cdn-community.codemao.cn/47/community/d2ViXzEwMDFfMTAwMTQwOTNfMTAwMTQwOTNfMTcwNjUyNTg1MTY4NV80ZmRmNjBjMw.png"

	},
	{
		"Movement", "https://cdn-community.codemao.cn/47/community/d2ViXzEwMDFfMTAwMTQwOTNfMTAwMTQwOTNfMTcwNjUyNTg1MTY4NV80ZmRmNjBjMw.png"

	},
	{
		"Visual", "https://cdn-community.codemao.cn/47/community/d2ViXzEwMDFfMTAwMTQwOTNfMTAwMTQwOTNfMTcwNjUyNTg1MTY4NV80ZmRmNjBjMw.png"

	},
	{
		"Misc", "https://cdn-community.codemao.cn/47/community/d2ViXzEwMDFfMTAwMTQwOTNfMTAwMTQwOTNfMTcwNjUyNTg1MTY4NV80ZmRmNjBjMw.png"

	},
	{
		"Config", "https://cdn-community.codemao.cn/47/community/d2ViXzEwMDFfMTAwMTQwOTNfMTAwMTQwOTNfMTcwNjUyNTg1MTY4NV80ZmRmNjBjMw.png"

	},
	{
		"Settings", "https://cdn-community.codemao.cn/47/community/d2ViXzEwMDFfMTAwMTQwOTNfMTAwMTQwOTNfMTcwNjUyNTg1MTY4NV80ZmRmNjBjMw.png"
...
swits["第1页"]["第2板块"] = {
	Yunzhu.text2("雷达设置"),
	Yunzhu.seek("雷达X","ltlz",1,100,19),--名字，变量名字，最小，最大，初始
	Yunzhu.seek("雷达Y","ltlz",1,100,87),--名字，变量名字，最小，最大，初始
	Yunzhu.seek("雷达大小","ltlz",1,100,37),--名字，变量名字，最小，最大，初始
}-- 此板块结束


swits["第1页"]["第3板块"] = {
	Yunzhu.text2("颜色设置"),
Yunzhu.check2("冰霜M416",
		function()
		end,
		function()
		end),Yunzhu.check2("冰霜M416",
		function()
		end,
		function()
		end),
		Yunzhu.text("●卡密到期时间:2099/12/31"),
		Yunzhu.text("●设备码:114514"),
		Yunzhu.text("●当前状态:永久卡密"),
		Yunzhu.text("●服务器状态:正常(20ms)"),
}

swits["第1页"]["第4板块"] = {
	Yunzhu.text2("进阶设置"),
Yunzhu.switch("Sprint",
function()
end,
function()
